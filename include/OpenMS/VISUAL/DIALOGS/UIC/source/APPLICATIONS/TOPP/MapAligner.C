...
/**
	...
	This tool provides several different algorithms to correct for retention time shifts and distortions.

	- @p pose_clustering_affine: The pose clustering algorithm as described in doi:10.1093/bioinformatics/btm209 is used to find an affine transformation.  This algorithm can be applied to features (featureXML) and peaks (mzML), but it has mostly been developed and tested on features.
	- @p apply_given_trafo: Applies transformations that are read from the files provided with the option @p -given_transformations. These transformations might have been generated by a previous invocation of MapAligner, see the option @p -transformations.  However, the trafoXML file format is not very complicated, so it is relatively easy to write (or generate) your own files.  This algorithm can be applied to mzML, featureXML, and idXML data.
	- @p identification: An experimental algorithm based on peptide identifications, and thus applicable to files containing peptide IDs (idXML, annotated mzML, annotated featureXML). It finds peptide sequences that different input files have in common and uses them as points of correspondence. From the retention times of these peptides, smooth non-linear transformations are computed that convert each file to a consensus time scale.
