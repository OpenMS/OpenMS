	...
	*/
		...
		: public QWidget
	{
		...
		protected:
			...
			QSpinBox* addSpinBox_(QWidget* parent, int min, int max, int step) const;
			///Adds a new QGroupBox with a @p label to the QGridlayout @p grid
			QGroupBox* addBox_(QGridLayout* grid, int row, int col, const QString& label, int row_span=1, int col_span=1);
			///Adds a @p label and a @p widget to the give @p row of a QGridlayout @p grid
			void addWidget_(QLayout* grid, int row, const QString& label, QWidget* widget) const;
			///Adds a @p label and a @p layout to the give @p row of a QGridlayout @p grid
			void addLayout_(QLayout* grid, int row, const QString& label, QLayout* layout) const;
			///Appends a row and column with strech @p stretch to a QGridlayout @p grid
			void finish_(QLayout* grid, int stretch = 2) const;
