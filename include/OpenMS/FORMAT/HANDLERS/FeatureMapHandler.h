...
namespace OpenMS
{
	...
	/** 
		...
		for details. 
	*/
  template <Size D, typename TraitsT = KernelTraits, typename FeatureT = Feature >
		...
		: public SchemaHandler
  {
    public:
    	/**
			...
			*/
			...
			typedef typename ConvexHullVector::value_type ConvexHullType;
      ...
      //@{
      ///
      FeatureMapHandler(FeatureMap<FeatureType>& map, const String& filename) 
      ...
      ///
      FeatureMapHandler(const FeatureMap<FeatureType>& map, const String& filename)
      : SchemaHandler(TAG_NUM,MAP_NUM,filename),
				map_(0), 
				cmap_(&map),	
				feature_(), 
    ...
    protected:
		...
		FeatureMap<FeatureType>* map_;
		// Feature map pointer for writing
		const FeatureMap<FeatureType>* cmap_;
  ...
  void FeatureMapHandler<D,TraitsT,FeatureT>::endElement(const XMLCh* const /*uri*/, const XMLCh* const /*local_name*/, const XMLCh* const qname)
	{
  ...
  template <Size D, typename TraitsT, typename FeatureT>
  void FeatureMapHandler<D,TraitsT,FeatureT>::startElement(const XMLCh* const /*uri*/, const XMLCh* const /*local_name*/, const XMLCh* const qname, const xercesc::Attributes& attributes)
	{
		if (is_parser_in_tag_[DESCRIPTION])	// collect Experimental Settings
		{
			exp_sett_ << '<' << xercesc::XMLString::transcode(qname);
	...
	template <Size D, typename TraitsT, typename FeatureT>
  void FeatureMapHandler<D,TraitsT,FeatureT>::characters(const XMLCh* const chars, const unsigned int /*length*/)
  {
		if (is_parser_in_tag_[DESCRIPTION])	// collect Experimental Settings
		{
	...
	template <Size D, typename TraitsT, typename FeatureT>
 	void FeatureMapHandler<D,TraitsT,FeatureT>::writeTo(std::ostream& os)
	{
