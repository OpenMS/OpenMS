...
namespace OpenMS
{
  ...
  {
		private:
			/// used in generateSecondDatabase
			template< typename T1, typename T2 >
			struct snd_db_comp_ : public std::binary_function<std::pair< T1, T2 >, std::pair< T1, T2 >, bool> {
				bool operator()(const std::pair< T1, T2 > x, const std::pair< T1, T2 > y) { return (x.first < y.first); }
			};

		public:
			
			/// default constructor
			/// ### expriment oder spec auch benutzen (muß dann umgewandelt werden)
			InspectInfile();
			...
			/// Specifies a spectra file to search. You can specify the name of a directory to search every file in that directory (non-recursively). Supported spectra file formats are .mzXML, .mzData, .ms2, dta, and .pkl. Multiple spectra in one .dta file are not supported.
			const std::string& getSpectra() const;
			void setSpectra(const std::string& spectra);

			
			/// Specifies the name of a database (.trie file) to search. The .trie file contains one or more protein sequences delimited by asterisks, with no whitespace or other data. Use PrepDB.py (see above) to prepare a .trie file. Most .trie files have a corresponding .index file giving the names of the proteins. You can specify at most one database.
			const String& getDb() const;
			void setDb(const String& db);

			
			/// Specifies the name of a FASTA-format protein database to search. If you plan to search a large database, it is more efficient to preprocess it using PrepDB.py and use the "db" command instead. You can specify at most one sequence file, but you can include both one "db" and one "sequence_file".
			...
			/// Specifies an amino acid modification. The delta mass (in daltons) and affected amino acids are required. The first four characters of the name should be unique. Valid values for "type" are "fix", "cterminal", "nterminal", and "opt" (the default). Examples:
			...
			*/
			...
			/// For blind search, specifies the maximum modification size (in Da) to consider. Defaults to 200. Larger values require more time to search.
			const double getMaxptmsize() const;
			void setMaxptmsize(double maxptmsize);

			
			/// Specifies the parent mass tolerance, in Daltons. A candidate's flanking mass can differ from the tag's flanking mass by no more than ths amount.
			...
			/// Use the file to specify PTM frequencies, for use in tag generation. This is more accurate tagging than than the default behavior (where tags can contain any PTM), but requires the creation of the jump frequency file.
		...
		private:
			...
			std::string spectra_; /// Specifies a spectra file to search. You can specify the name of a directory to search every file in that directory (non-recursively). Supported spectra file formats are .mzXML, .mzData, .ms2, dta, and .pkl. Multiple spectra in one .dta file are not supported.
