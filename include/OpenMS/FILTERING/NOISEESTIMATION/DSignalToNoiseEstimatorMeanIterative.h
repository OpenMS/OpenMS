  ...
  */
  ...
  class DSignalToNoiseEstimatorMeanIterative : public DSignalToNoiseEstimator<D, PeakIterator>
  {
    ...
    public:
      ...
      inline double& getSTDEVMultiplier()
      {
        return stdev_;
      }
      /// Mutable access to the multiplier of the stdev used during iterative threshold calculation
      ...
      inline double& getAutoMaxIntensity()
      {
        return auto_max_intensity_;
      }
      /// Mutable access to the AutoMaxIntensity-Param, which holds either a percentile or a stdev-multiplier (depending on AutoMode)
      ...
      inline double& getNoiseForEmtpyWindow()
      {
        return noise_for_empty_window_;
      }
      /// Mutable access to the noise value that is used if a window contains not enough elements
      ...
      inline void setFirstDataPoint(const PeakIterator& first)
      {
        DSignalToNoiseEstimator<D, PeakIterator>::setFirstDataPoint(first);
        is_result_valid_ = false;
      }
      /// Mutable access to the last raw data point
      inline void setLastDataPoint(const PeakIterator& last)
      {
        DSignalToNoiseEstimator<D, PeakIterator>::setLastDataPoint(last);
        is_result_valid_ = false;
      }
      ...
      virtual double getSignalToNoise(PeakIterator data_point)
      {
        ...
        if (!is_result_valid_)
        {
          ...
          else
          {
            ...
            {
              scan_count++;
              rt = (*scan_end).getPosition()[0];
              // find end of scan
              while (scan_end != last_ && rt == (*scan_end).getPosition()[0])
              {
                ++scan_end;
              }
      ...
      void shiftWindow_(const PeakIterator& scan_first_, const PeakIterator& scan_last_, double &sparse_window_percent)
      {
        ...
        if (max_intensity_ == -1.0)
        {
          ...
          else
          {
            ...
            if (auto_max_intensity_ > 100)
            {
              auto_max_intensity_ = DEFAULT_MAXINTENSITY_BYPERCENT;
            }
        ...
        while (window_pos_center != scan_last_)
        {
          ...
          while ( (*window_pos_borderleft).getPosition() <  (*window_pos_center).getPosition() - window_half_size )
          {
          ...
          while (     (window_pos_borderright != scan_last_)
                      && ((*window_pos_borderright).getPosition() <= (*window_pos_center).getPosition() + window_half_size )                     )
          {
