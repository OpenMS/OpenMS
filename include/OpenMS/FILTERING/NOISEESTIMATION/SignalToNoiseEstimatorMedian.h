  ...
  protected:
    ...
    void computeSTN_(const PeakIterator& scan_first_, const PeakIterator& scan_last_)
    {
      ...
      while (window_pos_center != scan_last_)
      {
        ...
        {
					to_bin = std::max( std::min<int>( (*window_pos_borderleft).getIntensity() / bin_size, bin_count_minus_1), 0);
          --histogram[to_bin];
          --elements_in_window;
                ...
                &&((*window_pos_borderright).getMZ() <= (*window_pos_center).getMZ() + window_half_size ) )
        {
          ...
          to_bin = std::max( std::min<int>( (*window_pos_borderright).getIntensity() / bin_size, bin_count_minus_1), 0);
          ++histogram[to_bin];
          ++elements_in_window;
