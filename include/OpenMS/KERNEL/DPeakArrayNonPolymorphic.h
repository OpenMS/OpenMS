...
//
// --------------------------------------------------------------------------
// $Id: DPeakArrayNonPolymorphic.h,v 1.21 2006/06/01 15:26:18 groepl Exp $
// $Author: groepl $
// $Maintainer: Marc Sturm $
// --------------------------------------------------------------------------

#ifndef OPENMS_KERNEL_DPEAKARRAYNONPOLYMORPHIC_H
#define OPENMS_KERNEL_DPEAKARRAYNONPOLYMORPHIC_H
...
namespace OpenMS
{
	...
	*/
	...
	class DPeakArrayNonPolymorphic
			...
			public PersistentObject
	{
		...
		*/
		...
		inline DPeakArrayNonPolymorphic() : PersistentObject() {} 
		/// See std::vector documentation.
		inline DPeakArrayNonPolymorphic(const DPeakArrayNonPolymorphic& p) : Base(p), PersistentObject(p) {}
		/// See std::vector documentation.
		inline ~DPeakArrayNonPolymorphic() {}
		/// See std::vector documentation.
		DPeakArrayNonPolymorphic(typename std::vector<PeakType>::size_type n) : PersistentObject()
		{
			resize(n);
		} 
		/// See std::vector documentation.
		DPeakArrayNonPolymorphic(typename std::vector<PeakType>::size_type n, const PeakType& peak) : PersistentObject()
		{
			reserve(n);
		...
		DPeakArrayNonPolymorphic(InputIterator f, InputIterator l) : PersistentObject()
		{
		...
		*/
		...
		/** 
			...
			<code>DPeakArrayNonPolymorphic < 1, DPeak <1> ></code>.
		*/
		//@{
		...
		/// See std::vector documentation.
		bool operator == (const DPeakArrayNonPolymorphic& array) const
		{
  ...
  void DPeakArrayNonPolymorphic<1, DRawDataPoint<1> >::persistentWrite(PersistenceManager& /*pm*/, const char* /*name*/) const throw (Exception::Base);
	...
	std::ostream& operator << (std::ostream& os, const DPeakArrayNonPolymorphic<D, Peak>& array)
	{
		os << "-- DPEAKARRAY-NONPOLYMORPHIC BEGIN --"<<std::endl;
		for (typename DPeakArrayNonPolymorphic<D, Peak>::const_iterator it = array.begin(); it!=array.end(); ++it)
		{
			os << *it << std::endl;
		}
...
//---------------------------------------------------------------
	...
	void DPeakArrayNonPolymorphic<D,PeakT>::sortByNthPosition(UnsignedInt i) throw (Exception::NotImplemented)
	{ 
