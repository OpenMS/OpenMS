...
namespace OpenMS
{
  ...
  /**
    ...
    Depending on the method of extraction a mass trace could virtually represent a complete ion
    chromatogram (XIC) or merely a part of it (e.g., a chromatographic peak). The kernel class
    provides methods for computing mass trace characteristics such as its centroid m/z and retention
    time. Coeluting mass traces can be further assembled to complete isotope patterns of
...
public:
    ...
    explicit MassTrace(const std::list<PeakType> &);

    /// Detailed constructor 2
    explicit MassTrace(const std::vector<PeakType> &);
