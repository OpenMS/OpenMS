...
namespace OpenMS 
{
	...
	**/
    ...
    public:
		...
		/// Standard constructor, allocates a buffer of size 1000
       	MSExperimentExtern()
		 		: buffer_size_(1500),
     ...
     /// Equality operator
	  ...
	  template <class Container> void get2DData(Container& cont) const { exp_.get2DData(cont); }
	   
	  template <class Container> void set2DData(Container& cont) { exp_.set2DData(cont); }
	   
	  void updateRanges() { exp_.updateRanges(); }
	   
	  void sortSpectra(bool sort_mz) { exp_.sortSpectra(sort_mz); }
	...
	Iterator erase(Iterator first,Iterator last)
	{
	  ...
	  {	  
		...
		//std::cout << "operator[" << n << "]" << std::endl;
	  	...
	  	if (buffer2scan_[b] != n) storeInBuffer(n);	// scan is not in buffer, needs to be read from file			
	  ...
	  void updateBuffer() { exp_.resize(buffer_size_); }	   
	 ...
	 protected:
		...
		void copyTmpFile_(String source)
		{
			...
			// copy file
			while( !feof(inFile) && !ferror(inFile)) putc( fgetc(inFile), outFile);
