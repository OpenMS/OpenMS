...
public:
  ...
  @see goNext
	*/
	...
	inline void goNextSubTree(seqan::Iter<TIndex, seqan::VSTree<seqan::TopDown<seqan::ParentLinks<TSpec> > > >& it, double& m, std::stack<double>& allm, std::stack<std::map<double, int> >& mod_map) 
	{
    // preorder dfs
    if (!goRight(it))
    {
	...
	/**
  	...
  	@see goNext
	*/
	template <typename TIndex, typename TSpec> inline void goNextSubTree(seqan::Iter<TIndex, seqan::VSTree<seqan::TopDown<seqan::ParentLinks<TSpec> > > >& it)
  {
    // preorder dfs
    if (!goRight(it))
    {
  ...
  @see goNextSubTree
	*/
	template <typename TIndex, typename TSpec> 
	inline void goNext(seqan::Iter<TIndex, seqan::VSTree<seqan::TopDown<seqan::ParentLinks<TSpec> > > >& it, double& m, std::stack<double>& allm, std::stack<std::map<double, int> >& mod_map)
	{
  	// preorder dfs
  	if (!goDown(it))
  	{
	...
	template <typename TIndex, typename TSpec>
  inline void parseTree(seqan::Iter<TIndex, seqan::VSTree<seqan::TopDown<seqan::ParentLinks<TSpec> > > >& it, std::vector<std::pair<int, int> >& out_number, std::vector<std::pair<int, int> >& edge_length, std::vector<int>& leafe_depth)
  {
    int depth = 1;
    while (!atEnd(it))
    {
...
protected:
	...
	seqan::Iter<TIndex, seqan::VSTree<seqan::TopDown<seqan::ParentLinks<seqan::Preorder> > > >* it_; ///< seqan suffix array iterator
