...
//
// --------------------------------------------------------------------------
// $Id: DGeomHashShiftSuperimposer.h,v 1.6 2006/05/03 07:18:47 ole_st Exp $
// $Author: ole_st $
// $Maintainer: Eva Lange $
// --------------------------------------------------------------------------


#ifndef OPENMS_ANALYSIS_MAPMATCHING_DGEOMHASHSHIFTSUPERIMPOSER_H
#define OPENMS_ANALYSIS_MAPMATCHING_DGEOMHASHSHIFTSUPERIMPOSER_H

#include <OpenMS/ANALYSIS/MAPMATCHING/DBaseSuperimposer.h>
...
#if defined OPENMS_DEBUG && ! defined V_DGeomHashShiftSuperimposer
#define V_DGeomHashShiftSuperimposer(bla) // std::cout << bla << std::endl;
#else
#define V_DGeomHashShiftSuperimposer(bla)
#endif
...
namespace OpenMS
{
  ...
  **/
  ...
  class DGeomHashShiftSuperimposer
		: public DBaseSuperimposer< MapT >
  {
	 public:
    ...
    typedef MapT MapType;
	 ...
	 protected:
    typedef DBaseSuperimposer< MapT > Base;
    ...
    */
    class Shift
    {
      ...
      typedef typename MapType::TraitsType TraitsType;
    ...
    /// Copy constructor
    DGeomHashShiftSuperimposer(const DGeomHashShiftSuperimposer& source)
			: Base(source)
    {
    ...
    ///  Assignment operator
    DGeomHashShiftSuperimposer& operator = (DGeomHashShiftSuperimposer source)
    {
    ...
    /// Destructor
    virtual ~DGeomHashShiftSuperimposer()
    {
			V_DGeomHashShiftSuperimposer("~GeomHashShift");
    }
    //@}
    ...
    virtual void run()
    {
      ...
      shift_matrix_.clear();
			...
			if ( !this->feature_map_[0]->empty() && !this->feature_map_[1]->empty() )
			{
				...
				computeShift_();
				// std::cout << "DGeomHashShiftSuperimposer::run()  fertig" << std::endl;
			}
			else
			{
				std::cerr << "DGeomHashShiftSuperimposer::run():  Oops, one of the feature maps is empty!\n";
			}
    }
    ...
    void parseParam_()
    {
#define V_parseParam_(bla) V_DGeomHashShiftSuperimposer(bla)
...
#undef V_parseParam_
    ...
    {
#define V_computeFeatureBuckets_(bla) V_DGeomHashShiftSuperimposer(bla)
      ...
      for ( Size map_index = 0; map_index < 2; ++map_index )
      {
        ...
        MapType const     & fm     = getFeatureMap(map_index);
        ...
        for ( typename MapType::ConstIterator fm_iter = fm.begin();
              fm_iter != fm.end();
              ++fm_iter
            )
        {
...
#undef V_computeFeatureBuckets_
    ...
    */
    ...
    {
#define V_computeShiftBuckets_(bla) V_DGeomHashShiftSuperimposer(bla)
          ...
          for ( Size model_bucket_index_RT
								...
								)
            {
							...
							FeatureBucketType const & model_feature_bucket
								= feature_bucket_[MODEL]( model_bucket_index_RT, model_bucket_index_MZ );
...
#undef V_computeShiftBuckets_
    ...
    */
    void computeShift_()
    {
#define V_computeShift_(bla) V_DGeomHashShiftSuperimposer(bla)
...
#endif  // OPENMS_ANALYSIS_MAPMATCHING_DGeomHashShiftSuperimposer_H
