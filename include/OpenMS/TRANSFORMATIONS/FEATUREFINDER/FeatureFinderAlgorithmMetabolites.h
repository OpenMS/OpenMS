...
#ifndef OPENMS_TRANSFORMATIONS_FEATUREFINDER_FEATUREFINDERALGORITHMMETABOLITES_H
#define OPENMS_TRANSFORMATIONS_FEATUREFINDER_FEATUREFINDERALGORITHMMETABOLITES_H
...
{
	/** 
		@brief A highly experimental FeatureFinderAlgorithm for metabolomics data.

    @ref FeatureFinderAlgorithmMetabolites_Parameters are explained on a separate page.
		
		@ingroup FeatureFinder
	*/
	template<class PeakType, class FeatureType> class FeatureFinderAlgorithmMetabolites 
			...
			public FeatureFinderDefs
	{
		...
		protected:
			/// Helper structure that stores precalculated information for each peak used in FeatureFinderAlgorithmMetabolites
			struct PeakInfo
			{
			...
			///Helper structure for seeds used in FeatureFinderAlgorithmMetabolites
			struct Seed
			{
				///Spectrum index
				UInt spectrum;
			...
			///Helper struct for mass traces used in FeatureFinderAlgorithmMetabolites
			struct MassTrace
			{
			...
			///Helper struct for a collection of mass traces used in FeatureFinderAlgorithmMetabolites
			struct MassTraces
			...
			///Helper structure for a theoretical isotope pattern used in FeatureFinderAlgorithmMetabolites
			...
			///Helper structure for a found isotope pattern used in FeatureFinderAlgorithmMetabolites
		...
		public:			
			/// default constructor 
			FeatureFinderAlgorithmMetabolites() 
			{
				//intensity
			...
			virtual void run()
			{
				...
				for (UInt c=(UInt)param_.getValue("isotopic_pattern:charge_low"); c<=(UInt)param_.getValue("isotopic_pattern:charge_high"); ++c)
				{
					...
					for (UInt i=0; i<seeds.size(); ++i)
					{
					  ...
					  //check if feature quality is high enough (average relative deviation and correlation of the whole feature)
						...
						{
							f.setMetaValue(String("mz_") + j, traces[j].getAvgMZ());
							f.setMetaValue(String("it_") + j, traces[j].max_peak->getIntensity());
				...
				//------------------------------------------------------------------
				//Step 5:
				//Split features into mass traces
				//------------------------------------------------------------------
				FeatureMapType features;
				for (UInt i=0; i<this->features_->size(); ++i)
				{
					Feature& f(this->features_->at(i));
					for (UInt j=0; j<f.getConvexHulls().size(); ++j)
					{
						Feature f2;
						f2.setIntensity((DoubleReal)(f.getMetaValue(String("it_") + j)));
						f2.setMZ((DoubleReal)(f.getMetaValue(String("mz_") + j)));
						f2.setRT(f.getRT());
						f2.getConvexHulls().push_back(f.getConvexHulls()[j]);
			...
			{
				return new FeatureFinderAlgorithmMetabolites();
			}
		...
		protected:
			...
			*/
			void findIsotope_(CoordinateType pos, UInt spectrum_index, IsotopePattern& pattern, UInt pattern_index, UInt& peak_index)
			{
			...
			DoubleReal isotopeScore_(const TheoreticalIsotopePattern& isotopes, IsotopePattern& pattern, bool consider_mz_distances)
			{
		...
		private:
			...
			/// Not implemented
			FeatureFinderAlgorithmMetabolites& operator=(const FeatureFinderAlgorithmMetabolites&);
			/// Not implemented
			FeatureFinderAlgorithmMetabolites(const FeatureFinderAlgorithmMetabolites&);

	};

} // namespace OpenMS

#endif // OPENMS_TRANSFORMATIONS_FEATUREFINDER_FEATUREFINDERALGORITHMMETABOLITES_H
