...
namespace OpenMS
{
	...
	void CompleteLinkage::cluster(const DistanceMatrix<double>& originalDist, DistanceMatrix<double>& actualDist, vector< vector<UInt> >& clusters, const String filepath /*= ""*/, const double threshold /*=1*/) const throw (Exception::UnableToCreateFile,ClusterFunctor::InsufficientInput)
	{
		...
		//pick minimum-distance pair i,j and merge them
			...
			for (UInt i = 0; i < min.first; ++i)
		 	{
			 	actualDist.setValueQuick(i,min.first,getMaxDist_(i,min.first,clusters,originalDist));
			}
			for (UInt j = min.first+1; j < actualDist.dimensionsize(); ++j)
		 	{
			 	actualDist.setValueQuick(min.first,j,getMaxDist_(min.first,j,clusters,originalDist));
			}
