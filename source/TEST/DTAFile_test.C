...
CHECK(template<typename SpectrumType> void store(const String& filename, const SpectrumType& spectrum) const throw(Exception::UnableToCreateFile))
	...
	spec.getPrecursorPeak().getPos()[0] = 582.40666;
	...
	TEST_REAL_EQUAL(spec.getPrecursorPeak().getPos()[0],582.40666)
	...
	TEST_REAL_EQUAL(it->getPos()[0], 11.4)
	TEST_REAL_EQUAL(it->getIntensity(), 11.5)
	++it;
	
	TEST_REAL_EQUAL(it->getPos()[0], 12.4)
	TEST_REAL_EQUAL(it->getIntensity(), 12.5)
	++it;

	TEST_REAL_EQUAL(it->getPos()[0], 13.4)
	...
	TEST_REAL_EQUAL(it2->getPos()[0], 11.4)
	TEST_REAL_EQUAL(it2->getIntensity(), 11.5)
	++it2;
	
	TEST_REAL_EQUAL(it2->getPos()[0], 12.4)
	TEST_REAL_EQUAL(it2->getIntensity(), 12.5)
	++it2;

	TEST_REAL_EQUAL(it2->getPos()[0], 13.4)
