...
//
// --------------------------------------------------------------------------
// $Id: DSavitzkyGolaySVDFilter_test.C,v 1.6 2006/04/13 08:40:46 elange Exp $
// $Author: elange $
...
START_TEST(DSavitzkyGolaySVDFilter<D>, "$Id: DSavitzkyGolaySVDFilter_test.C,v 1.6 2006/04/13 08:40:46 elange Exp $")
...
DSavitzkyGolaySVDFilter<1>* dsg_ptr = 0;
CHECK(DSavitzkyGolaySVDFilter())
  dsg_ptr = new DSavitzkyGolaySVDFilter<1>;
...
CHECK(DSavitzkyGolaySVDFilter(const Param& p))
  Param p;
  p.setValue("PolynomOrder",2);
  p.setValue("FrameLength",3);


  DSavitzkyGolaySVDFilter<1> sgolay(p);
...
CHECK(DSavitzkyGolaySVDFilter& operator=(const DSavitzkyGolaySVDFilter& s))
  RawDataArray2D* sgolay_data=0;
  DSavitzkyGolaySVDFilter<2> sgolay;
  sgolay.setOrder(4);
  sgolay.setWindowSize(5);
  sgolay(*sgolay_data);

  DSavitzkyGolaySVDFilter<2> sgolay_copy=sgolay;
...
CHECK(DSavitzkyGolaySVDFilter(const DSavitzkyGolaySVDFilter& s))
  RawDataArray2D* sgolay_data=0;
  DSavitzkyGolaySVDFilter<2> sgolay;
  sgolay.setOrder(4);
  sgolay.setWindowSize(5);
...
CHECK(const Param& getParam() const)
  ...
  p.setValue("PolynomOrder",2);
  p.setValue("FrameLength",3);
  const DSavitzkyGolaySVDFilter<1> sgolay(p);
...
CHECK(const unsigned int& getOrder() const)
  DSavitzkyGolaySVDFilter<1> sgolay;
