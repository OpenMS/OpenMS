...
START_TEST(MapMatcherRegression<ElementT>, "$Id: MapMatcherRegression_test.C 1586 2007-03-01 17:59:10Z elange $")
...
CHECK((MapMatcherRegression(const MapMatcherRegression& source)))
  ...
  feat4.setRT(9.0);
	...
	MapMatcherRegression<> mmatcher_copy(mmatcher);
	Grid grid_copy = mmatcher_copy.getGrid();
	ElementPairVector pairs_copy = mmatcher_copy.getElementPairs();
...
CHECK((MapMatcherRegression& operator = (const MapMatcherRegression& source)))
