...
//
// --------------------------------------------------------------------------
// $Id: DTA2DFile_test.C,v 1.9 2006/05/30 15:46:43 marc_sturm Exp $
// $Author: marc_sturm $
...
START_TEST(DTAFile, "$Id: DTA2DFile_test.C,v 1.9 2006/05/30 15:46:43 marc_sturm Exp $")
...
CHECK(void store(const String& filename, const MSExperiment<>& exp) const throw (Exception::UnableToCreateFile))
  ...
  f.load("data/DTA2DFile_test_1.dta2d",e);
	...
	DPeakArrayNonPolymorphic<2,DRawDataPoint<2> > array2;
	e2.get2DData(array2);
	TEST_EQUAL(array2.size(), 9);
	ABORT_IF(array2.size() != 9)

	DPeakArrayNonPolymorphic<2,DRawDataPoint<2> >::ConstIterator it3 = array2.begin();
