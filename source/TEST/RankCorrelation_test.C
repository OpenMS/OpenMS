...
START_TEST(Correlation, "$Id RankCorrelation_test.C 139 2006-07-14 10:08:39Z ole_st $")
...
RankCorrelation* ptr = 0;
CHECK((RankCorrelation()))
	ptr = new RankCorrelation();
...
CHECK((double evaluate(const IndexSet& set, const BaseModel<1>& model, UnsignedInt dim)))
	...
	TEST_REAL_EQUAL(result,0.954702)
...
CHECK((double evaluate(const IndexSet& set, const BaseModel<2>& model)))
	...
	TEST_REAL_EQUAL(result,0.955305);
	TEST_REAL_EQUAL(pval,0.0280274);
