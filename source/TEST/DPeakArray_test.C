...
START_TEST(DPeakArray<D>, "$Id: DPeakArray_test.C 146 2006-07-14 15:06:21Z marc_sturm $")
...
CHECK(DPeakArray(typename std::vector<PeakType>::size_type n, const PeakType& peak))
	...
	DPeakArray<2, DPickedPeak<2> > dpanp(3,peak);
...
CHECK(DPeakArray& operator[] const)
...
CHECK(iterator insert(iterator pos, InputIterator f, InputIterator l))
	DPeakArray<2, DPickedPeak<2> > tmp(pl);
	TEST_REAL_EQUAL(pl.size(), 3)
