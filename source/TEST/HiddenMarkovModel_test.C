...
CHECK([EXTRA](HMMState()))
	state_ptr = new HMMState();
	TEST_NOT_EQUAL(state_ptr, 0)
RESULT

CHECK([EXTRA](virtual ~HMMState()))
	delete state_ptr;
RESULT

state_ptr = 0;

CHECK([EXTRA](HMMState(const String& name, bool hidden = true)))
...
CHECK([EXTRA](void setName(const String& name)))
...
CHECK([EXTRA](const std::set<HMMState*>& getPredecessorStates() const))
	TEST_EQUAL(state_ptr->getPredecessorStates().size(), 0)
RESULT

CHECK([EXTRA](const std::set<HMMState*>& getSuccessorStates() const))
	TEST_EQUAL(state_ptr->getSuccessorStates().size(), 0)
RESULT

CHECK([EXTRA](void addPredecessorState(HMMState* state)))
...
CHECK([EXTRA](void deletePredecessorState(HMMState* state)))
	state_ptr->deletePredecessorState(state_ptr2);
	TEST_EQUAL(state_ptr->getPredecessorStates().size(), 0);
RESULT

CHECK([EXTRA](void addSuccessorState(HMMState* state)))
...
CHECK([EXTRA](void deleteSuccessorState(HMMState* state)))
...
CHECK([EXTRA](HMMState(const HMMState& state)))
...
CHECK([EXTRA](HMMState& operator = (const HMMState&)))
	HMMState copy;
	copy = *state_ptr;
	TEST_EQUAL(copy.getName(), state_ptr->getName())
	TEST_EQUAL(copy.getSuccessorStates().size(), state_ptr->getSuccessorStates().size())
...
CHECK(void addNewState(const String &name))
