...
START_SECTION((MassTrace(const std::list<PeakType>& tmp_lst)))
...
START_SECTION((MassTrace(const std::vector<PeakType>& tmp_vec)))
...
START_SECTION((DoubleReal MassTrace::computePeakArea()))
...
START_SECTION((Size MassTrace::findMaxByIntPeak(bool use_smoothed_ints = false)))
...
START_SECTION((DoubleReal MassTrace::estimateFWHM(bool use_smoothed_ints = false)))
...
START_SECTION((void MassTrace::findLocalExtrema(const Size& num_neighboring_peaks, std::vector<Size>& chrom_maxes, std::vector<Size>& chrom_mins)))
...
START_SECTION((MassTrace(const MassTrace& mt)))
...
START_SECTION((MassTrace& MassTrace::operator= (const MassTrace& rhs)))
...
START_SECTION((ConvexHull2D MassTrace::getConvexhull() const))
