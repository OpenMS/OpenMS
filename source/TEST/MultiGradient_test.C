...
//
// --------------------------------------------------------------------------
// $Id: MultiGradient_test.C,v 1.6 2006/03/28 08:03:34 marc_sturm Exp $
// $Author: marc_sturm $
...
CHECK(interpolateColorAt(position) (stairs mode) )
	...
	TEST_EQUAL(mg.interpolatedColorAt(0)==Qt::white,true);
	TEST_EQUAL(mg.interpolatedColorAt(25)==Qt::white,true);
	TEST_EQUAL(mg.interpolatedColorAt(100)==Qt::black,true);
	mg.insert(50,Qt::red);
	TEST_EQUAL(mg.interpolatedColorAt(0)==Qt::white,true);
	TEST_EQUAL(mg.interpolatedColorAt(49)==Qt::white,true);
	TEST_EQUAL(mg.interpolatedColorAt(50)==Qt::red,true);
	TEST_EQUAL(mg.interpolatedColorAt(51)==Qt::red,true);
	TEST_EQUAL(mg.interpolatedColorAt(99)==Qt::red,true);
	TEST_EQUAL(mg.interpolatedColorAt(100)==Qt::black,true);	
RESULT
