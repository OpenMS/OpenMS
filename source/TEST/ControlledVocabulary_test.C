...
START_TEST(ControlledVocabulary, "$Id: DPeak_test.C 1619 2007-03-02 12:51:03Z andreas_bertsch $")
...
CHECK(const CVTerm& getTerm(const String& id) const throw (Exception::InvalidValue))
	TEST_EQUAL(cv.getTerm("OpenMS:6").id,"OpenMS:6")
	TEST_EQUAL(cv.getTerm("OpenMS:6").name,"Ka")
	TEST_EQUAL(cv.getTerm("OpenMS:6").parents.size(),1)
	TEST_EQUAL(cv.getTerm("OpenMS:6").parents[0],"OpenMS:2")
