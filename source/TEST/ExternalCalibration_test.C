...
START_TEST(ExternalCalibration, "$Id$")

/////////////////////////////////////////////////////////////

ExternalCalibration* ptr = 0;
CHECK((ExternalCalibration()))
  ptr = new ExternalCalibration;
...
CHECK((template<typename PeakType> void calibrate(MSExperiment< RawDataPointType > &calib_spectra, MSExperiment< PeakType > &exp, std::vector< double > &exp_masses)))
  ...
  MzDataFile file;
  file.load("data/ExternalCalibration_test_calibrants.mzData",calib_exp);
  file.load("data/ExternalCalibration_test.mzData",exp);
  file.load("data/ExternalCalibration_test_output.mzData",res_exp);
...
CHECK((template<typename PeakType> void calibrate(MSExperiment< PickedPeakType > &calib_spectra, MSExperiment< PeakType > &exp, std::vector< double > &exp_masses)))
  ...
  MzDataFile file;
  file.load("data/ExternalCalibration_test_calibrants2.mzData",calib_exp);
  file.load("data/ExternalCalibration_test2.mzData",exp);
  file.load("data/ExternalCalibration_test_output2.mzData",res_exp);
  vector<double> ref_masses;
  TextFile ref_file;
