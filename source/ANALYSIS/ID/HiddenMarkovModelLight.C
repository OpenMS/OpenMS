...
namespace OpenMS 
{
	...
	HMMStateLight::HMMStateLight(Size id, bool hidden)
		: hidden_(hidden),
			id_(id)
	{
	}
	...
	void HMMStateLight::setIdentifier(Size id)
	{
		id_ = id;
	}

	Size HMMStateLight::getIdentifier() const
	{
		return id_;
	}
	...
	HMMStateLight* HiddenMarkovModelLight::getState(Size id)
	{
		return id_to_state_[id];
	}

	const HMMStateLight* HiddenMarkovModelLight::getState(Size id) const
	{
		return id_to_state_[id];
	}
	
	void HiddenMarkovModelLight::writetoYGFFile(const String& filename)
	{
		set<HMMStateLight*> states;
		HashMap<HMMStateLight*, vector<HMMStateLight*> > transitions;
		
		stack<HMMStateLight*> s;
		for (HashMap<HMMStateLight*, double>::ConstIterator it = init_train_prob_.begin(); it != init_train_prob_.end(); ++it)
		{
			s.push(it->first);
		}
    ...
    out << "  <graph id=\"G\" edgedefault=\"directed\">" << endl;
		...
		for (HashMap<HMMStateLight*, vector<HMMStateLight*> >::ConstIterator it = transitions.begin(); it != transitions.end(); ++it)
		{
	...
	void HiddenMarkovModelLight::readFromFile(const String& filename)
	{
		...
		char buf[10000];
		HMMStateLight* s = 0;
		HashMap<HMMStateLight*, vector<Size> > pre_states;
		HashMap<HMMStateLight*, vector<Size> > succ_states;
		while (in.getline(buf, 10000, '\n'))
		{
			...
			{
				s = new HMMStateLight((Size)split[1].toInt());
			}
			if (split_0 == "hidden_:")
			{
				if (split[1] == "false")
		...
		while (in.getline(buf, 10000, '\n'))
		{
			...
			vector<String> split;
			line.split(' ', split);
			synonym_trans_names_[split[0].toInt()][split[1].toInt()] = make_pair<Size, Size>(split[2].toInt(), split[3].toInt());
		}
	...
	void HiddenMarkovModelLight::addSynonymTransition(Size name1, Size name2, Size synonym1, Size synonym2)
	{
		synonym_trans_names_[synonym1][synonym2] = make_pair<Size, Size>(name1, name2);
	}
	...
	void HiddenMarkovModelLight::setTransitionProbability(Size s1, Size s2, double trans_prob)
	{
  ...
  double HiddenMarkovModelLight::getTransitionProbability(Size s1, Size s2) const
  {
	...
	void HiddenMarkovModelLight::train()
	{
		...
		for (HashMap<HMMStateLight*, double>::ConstIterator it1 = train_emission_prob_.begin(); it1 != train_emission_prob_.end(); ++it1)
		{
	...
	void HiddenMarkovModelLight::calculateForwardPart_()
	{
		...
		for (HashMap<HMMStateLight*, double>::Iterator it = init_train_prob_.begin(); it != init_train_prob_.end(); ++it)
		{
		...
		#ifdef HIDDEN_MARKOV_MODEL_DEBUG
		for (HashMap<HMMStateLight*, double>::ConstIterator it = forward_.begin(); it != forward_.end(); ++it)
		{
	...
	void HiddenMarkovModelLight::calculateBackwardPart_()
	{
		...
		for (HashMap<HMMStateLight*, double>::Iterator it = train_emission_prob_.begin(); it != train_emission_prob_.end(); ++it)
		{
	...
	{
		for (HashMap<HMMStateLight*, HashMap<HMMStateLight*, double> >::ConstIterator it1 = count_trans_.begin(); it1 != count_trans_.end(); ++it1)
		{
			double sum(0);
	...
	void HiddenMarkovModelLight::setTrainingEmissionProbability(Size state, double prob)
	{
	...
	void HiddenMarkovModelLight::enableTransition(Size s1, Size s2)
	{
	...
	void HiddenMarkovModelLight::disableTransition(Size s1, Size s2)
	{
	...
	{
		for (HashMap<HMMStateLight*, set<HMMStateLight*> >::ConstIterator it = enabled_trans_.begin(); it != enabled_trans_.end(); ++it)
		{
	...
	void HiddenMarkovModelLight::calculateEmissionProbabilities(HashMap<HMMStateLight*, double>& emission_probs)
	{
		HashMap<HMMStateLight*, double> states = init_train_prob_;

		while (states.size() != 0)
		{
			HashMap<HMMStateLight*, double> tmp = states;
	...
	void HiddenMarkovModelLight::dump()
	{
		...
		for (HashMap<HMMStateLight*, HashMap<HMMStateLight*, double> >::ConstIterator it = trans_.begin(); it != trans_.end(); ++it)
		{
			for (HashMap<HMMStateLight*, double>::ConstIterator it1 = it->second.begin(); it1 != it->second.end(); ++it1)
			{
   ...
   set<HMMStateLight*> succ;
	...
	{
		for (HashMap<Size, HashMap<Size, std::pair<Size,  Size> > >::ConstIterator it = synonym_trans_names_.begin(); it != synonym_trans_names_.end(); ++it)
		{
			for (HashMap<Size, pair<Size, Size> >::ConstIterator it2 = it->second.begin(); it2 != it->second.end(); ++it2)
			{
	...
	void HiddenMarkovModelLight::estimateUntrainedTransitions()
	{
		throw Exception::NotImplemented(__FILE__, __LINE__, __PRETTY_FUNCTION__);
		/*
		for (HashMap<HMMStateLight*, HashMap<HMMStateLight*, Size> >::ConstIterator it = training_steps_count_.begin(); it != training_steps_count_.end(); ++it)
		{
			for (HashMap<HMMStateLight*, Size>::ConstIterator it1 = it->second.begin(); it1 != it->second.end(); ++it1)
			{
		...
		for (Size i = 0; i != residues.size(); ++i)
		{
			...
			for (Size j = 0; j != residues.size(); ++j)
			{
				...
				if (training_steps_count_[id_to_state_[aa1 + aa2 + "bxyz"]][s2] == 0)
				{
					...
					{
						Size tmp = training_steps_count_[id_to_state_[aa1 + residues[k] + "bxyz"]][s2];
						if (tmp != 0)
						{
					...
					for (Size k = 0; k != residues.size(); ++k)
					{
						Size tmp = training_steps_count_[id_to_state_[residues[k] + aa2 +"bxyz"]][s2];
						if (tmp != 0)
						{
      ...
      for (Size j = 0; j != residues.size(); ++j)
      {
        ...
        if (training_steps_count_[id_to_state_[aa1 + aa2 + "axyz"]][s2] == 0)
        {
          ...
          for (Size k = 0; k != residues.size(); ++k)
          {
            Size tmp = training_steps_count_[id_to_state_[aa1 + residues[k] + "axyz"]][s2];
            if (tmp != 0)
            {
          ...
          for (Size k = 0; k != residues.size(); ++k)
          {
            Size tmp = training_steps_count_[id_to_state_[residues[k] + aa2 +"axyz"]][s2];
            if (tmp != 0)
            {
	...
	void HiddenMarkovModelLight::addIdToName(Size id, const String& name)
	{
		id_to_name_[id] = name;
	}
	...
	void HiddenMarkovModelLight::write(ostream& out)
	{
		...
		// synonym transitions
		for (HashMap<HMMStateLight*, HashMap<HMMStateLight*, std::pair<HMMStateLight*, HMMStateLight*> > >::ConstIterator it1 = synonym_trans_.begin(); it1 != synonym_trans_.end(); ++it1)
		{
