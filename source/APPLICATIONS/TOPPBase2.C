...
namespace OpenMS
{
  ...
  TOPPBase2::TOPPBase2(const String& tool_name)
	...
	TOPPBase2::~TOPPBase2()
  {
	...
	TOPPBase2::ExitCodes TOPPBase2::main(int argc , char** argv)
	{
		//----------------------------------------------------------
		//parse command line
		//----------------------------------------------------------
	...
	void TOPPBase2::printUsage_() const
	{
		printToolUsage_();
	...
	void TOPPBase2::printHelpOpt_() const
	{
		printToolHelpOpt_();
	...
	void TOPPBase2::writeLog_(const String& text) const
	{
		cout << text << endl;
		enableLogging_();
	...
	void TOPPBase2::writeDebug_(const String& text, UnsignedInt min_level) const
	{
	...
	void TOPPBase2::writeDebug_(const String& text, const Param& param, UnsignedInt min_level) const
	{
		if (debug_level_>=(SignedInt)min_level)
		{
	...
	String TOPPBase2::getParamAsString_(const String& key, const String& default_value) const
	{
	...
	SignedInt TOPPBase2::getParamAsInt_(const String& key, SignedInt default_value) const
	{
		const DataValue& tmp = getParam_(key);
		if (!tmp.isEmpty())
		{
	...
	double TOPPBase2::getParamAsDouble_(const String& key, double default_value) const
	{
		const DataValue& tmp = getParam_(key);
		if (!tmp.isEmpty())
		{
	...
	bool TOPPBase2::getParamAsBool_(const String& key, bool default_value) const
	{
		const DataValue& tmp = getParam_(key);
	...
	DataValue const& TOPPBase2::getParam_(const String& key) const
	{
