...
/**
	...
	<ol>
		<li> 
			Set the parameters of the svm: The RTEvaluation application will train 
			the svm with the training data and store the svm model
		</li>
		<li>
			Give a range of parameters for which a CV should be performed:
			The RTEvaluation application will perform a CV to find the best 
	...
	: public TOPPBase
{
	...
	protected:
		void registerOptionsAndFlags_()
		{
			...
			registerOutputFile_("out","<file>","","output file: the results");
			...
			registerIntOption_("number_of_outer_partitions","<int>",5,"number of partitions in outer CV",false);
			setMinInt_("number_of_outer_partitions", 2);
			registerIntOption_("number_of_outer_runs","<int>",2,"number of runs in outer CV",false);
			setMinInt_("number_of_outer_runs", 1);
														  ...
														  std::vector<DoubleReal>&  labels)
	  {
		...
		ExitCodes main_(Int , const char**)
		{
		  ...
		  PeptideHit temp_peptide_hit;
			...
			DoubleReal test_quality_sum = 0.;
			UInt number_of_outer_partitions = 0;
			UInt number_of_outer_runs = 0;
			...
			outputfile.open(outputfile_name.c_str(), ios_base::out);
			...
			number_of_outer_partitions = getIntOption_("number_of_outer_partitions");			
			number_of_outer_runs = getIntOption_("number_of_outer_runs");			
			...
			{	
				...
				for (Size i = 0; i < number_of_outer_runs; ++i)
				{
					...
					for (Size j = 0; j < number_of_outer_partitions; ++j)
					{
						...
						SVMWrapper::mergePartitions(samples, j, temp_training_sample);
					
						cv_quality = svm.performCrossValidation(temp_training_sample,
																									start_values,
																		 							step_sizes,
																		 							end_values,
						...
						cout << "Test samples: " << temp_test_sample.labels.size() << "," 
							<< temp_test_sample.sequences.size() << endl;
		
						vector<DoubleReal> temp_predictions;
		
						svm.predict(temp_test_sample, temp_predictions);					
						DoubleReal test_quality = Math::pearsonCorrelationCoefficient(temp_predictions.begin(), 
																						temp_predictions.end(), 
																						temp_test_sample.labels.begin(), 
																						temp_test_sample.labels.end());
					...
					outputfile << (test_quality_sum / number_of_outer_partitions) << endl;					
				}
			}
