	...
	: public TOPPBase
{
	...
	protected:
		...
		ExitCodes main_(int , const char**)
		{
			...
			DoubleReal AccurateMass, MinMass, MaxMass;
		...
		if(out_opt==2)
		fp_out<<"mass_error"<<SEP<<"#proteins in database"<<SEP<<"# tryptic peptides"<<SEP<<"# unique peptide weights"<<SEP<<"# idenfiable proteins"<<SEP<<"average window_size"<<endl;
		...
		while(mass_iter>0)
		{
			...
			for(UInt i = 0; i < protein_data.size(); ++i)
			{
				...
				for(UInt j = 0; j < temp_peptides.size(); ++j)
				{
					...
					AccurateMass = temp_peptides[j].getMonoWeight();
					MinMass = AccurateMass - mass_iter*AccurateMass/1000000000;
					MaxMass = AccurateMass + mass_iter*AccurateMass/1000000000;
					...
					if (temp_peptides[j].size() >= min_size)
					{
						if (temp_peptides[j].hasSubsequence(M) == TRUE)
						{
							...
							MIN.push_back(MinMass);
							MAX.push_back(MaxMass);
			...
			fp_out<<endl;
			fp_out<<"MW_count"<<SEP<<"Overlapping ions in search space"<<endl;
		...
		for(UInt a = 0; a < PROTEINS.size()-1; ++a)
		{
			...
			cout<<PROTEINS.size()<<endl<<ProCount<<endl;
		}
			...
			if(out_opt>1)
			fp_out<<mass_iter<<SEP<<protein_data.size()<<SEP<<MAX.size()<<SEP<<ZeroCount<<SEP<<PROTEINS.size()-ProCount<<SEP<<total<<endl;
			ProCount = 0;
			ZeroCount = 0;
		}
			return EXECUTION_OK;
