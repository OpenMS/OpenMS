...
<CENTER>
	...
	- @ref OpenMS::MapAlignmentAlgorithmIdentification "identification": An alignment algorithm based on peptide identifications, and thus applicable to files containing peptide IDs (idXML, annotated mzML, annotated featureXML). It finds peptide sequences that different input files have in common and uses them as points of correspondence. From the retention times of these peptides, smooth non-linear transformations are computed that convert each file to a consensus time scale.
