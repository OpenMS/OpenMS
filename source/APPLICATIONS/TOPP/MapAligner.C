...
/**
	...
	- pose_clustering_affine: The pose clustering algorithm as described in doi:10.1093/bioinformatics/btm209 is used to find an affine transformation.  This algorithm can be applied to features (featureXML) and peaks (mzML), but it has mostly been developed and tested on features.
	- apply_given_trafo: The transformations are read from the files provided with the option -given_transformations. These transformations might have been generated by a previous invocation of MapAligner, see the option -transformations.  But the TransformationXML File format is not very complicated, so it is relatively easy to write (or generate) your own ones.  This algorithm can be applied to mzML, featureXML, and idXML data.
	- identification: An experimental algorithm based on identifications.
	- spectrum_alignment: An experimental algorithm based on spectrum alignment.
  .
