...
<CENTER>
  ...
  One input file will serve as reference for all other input files (except for type 'apply_given_trafo', which does no alignment.
	...
	- @ref OpenMS::MapAlignmentAlgorithmSpectrumAlignment "spectrum_alignment": An experimental algorithm based on spectrum alignment; applicable only to peak maps (mzML). The first map is taken as reference.
