...
//
// --------------------------------------------------------------------------
// $Id: DBExporter.C,v 1.6 2006/06/01 11:53:24 marc_sturm Exp $
// $Author: marc_sturm $
       ...
       << "Usage:" << endl
		...
		ExitCodes main_(int argc , char** argv)
		{
			...
			if (query!="")
			{
				...
				while (con.lastResult().next())
				{
					ids.push_back(con.lastResult().value(0).toInt());
				}
			}
			...
			if (ids.size()>=1)
			{
				...
				if (ids.size()==1)
				{
					...
					//write to file
					f.store(out, dynamic_cast<MSExperiment<>&>(*exp_new));
				}
				else
				{
					...
					for (vector<UID>::iterator it = ids.begin(); it!=ids.end(); ++it)
					{
						...
						ss << *it;
						ss << "_" << out;
						f.store(ss.str(), dynamic_cast<MSExperiment<>&>(*exp_new));
					}
				}		
			}		
