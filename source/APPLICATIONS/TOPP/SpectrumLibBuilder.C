...
//
// --------------------------------------------------------------------------
// $Id: SpectrumLibBuilder.C,v 1.4 2006/03/28 08:03:30 marc_sturm Exp $
// $Author: marc_sturm $
...
//-------------------------------------------------------------
const char* tool_name = "SpectrumLibBuilder";
...
{
	cerr << endl
       << tool_name << " -- build a library ofr MS spectra for similarity-based identification." << endl 
       << "  Starting with a library of high-quality MS spectra, this component computes all pairwise" << endl
       << "  similarities between spectra. The spectra are then clustered and a representative" << endl
			 << "  for each cluster is chosen. These representatives are then stored (along with their" << endl
       << "  identification, if available) for subsequent identification of unknown spectra through the SpectrumMatcher."
       << endl
       << "Usage:" << endl
			 << " " << tool_name << "  [-in <file>] [-out <file>] [-ini <file>] [-log <file>] [-n <int>] [-d <level>]" << endl
			 << "  -in <file>   input file in analysisXML format (default read from TOPP.ini)" << endl
       ...
       << "  --help-opt   shows help on all options accepted in the INI file." << endl
			 << endl;
}
