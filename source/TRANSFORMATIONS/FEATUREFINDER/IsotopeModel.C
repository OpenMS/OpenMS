...
namespace OpenMS
{
  ...
  EmpiricalFormula IsotopeModel::getFormula()
  {
    CoordinateType mass = mean_ * charge_;
		...
		if (C_num > std::numeric_limits<Int>::max() ||
				...
				S_num < -std::numeric_limits<Int>::max() ) return EmpiricalFormula();
  ...
  void IsotopeModel::setSamples(const EmpiricalFormula& formula)
  {
    ...
    result.clear ();
		...
		SignedSize rMax = std::min ( SignedSize( left.size() + right.size() - 1 ),				 SignedSize (2*normal_widening_width/interpolation_step_*max_isotope_+1) );
    ...
    {
			for ( SignedSize j = std::min ( rMax - i, SignedSize(right.size()) ) - 1; j >= 0 ; --j )
      {
