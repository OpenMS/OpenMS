...
//
// --------------------------------------------------------------------------
// $Id: SimpleExtender.C,v 1.38 2006/04/25 12:41:44 ole_st Exp $
// $Author: ole_st $
  ...
  const IndexSet& SimpleExtender::extend(const UnsignedInt seed_index)
  {
  	if (!first_seed_seen_)
  	{
		...
		{			
			...
			if (nr_peaks_seen_ == 5)  intensity_threshold_ = intensity_factor_ * current_intensity;
			
			if (current_intensity <  intensity_threshold_) 	continue; 
