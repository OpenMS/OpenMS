...
namespace OpenMS
{
	...
	MvAvgExtender::~MvAvgExtender()
	{}

	const IndexSet& MvAvgExtender::extend(const IndexSet& seed_region)
	{
    if (!first_seed_seen_)
    {
            ...
            ++citer)
    {
        ...
        if (p.getIntensity() > max_intensity)
        {
    		...
    		IndexWithPriority iwp(*citer,pi);
				traits_->getPeakFlag(*citer) = FeaFiTraits::INSIDE_FEATURE;
				region_.add(*citer);
...
//     } // end of while ( !boundary_.empty() )
    ...
    std::cout << "MvAvgExtender: Feature region size: " << region_.size() << std::endl;
	 ...
	 * 
	 * */
	bool MvAvgExtender::isTooFarFromCentroid_(UnsignedInt current_peak)
	{
...
//     return false;
	...
	void MvAvgExtender::moveMzUp_(UnsignedInt current_index)
	{
	...
	void MvAvgExtender::moveMzDown_(UnsignedInt current_index)
	{
	...
	void MvAvgExtender::moveRtUp_(UnsignedInt current_index)
	{
	...
	void MvAvgExtender::moveRtDown_(UnsignedInt current_index)
	{
    try // moving up in retention time
    {
	...
	MvAvgExtender::ProbabilityType MvAvgExtender::computePeakPriority_(PeakType& p)
	{
	...
	void MvAvgExtender::checkNeighbour_(UnsignedInt current_index)
	{
