...
Also, TOPPAS can only chain OpenMS TOPP tools; there is no built-in node for calling external tools. We generally recommend to run only OpenMS-specific tools in TOPPAS
and hand the resulting data to other tools, or use other workflow systems, such as Nextflow or Galaxy which can fully integrate other tools.
