...
Once the environment is active, you can install all required python packages using
    ...
    cd doc/pyopenms/docs
    pip install -r requirements.txt
    python install_pyopenms.py


To build the docs run the line below (works on all operating systems), but make sure that your working dir is `doc/pyopenms/docs` (see above):
