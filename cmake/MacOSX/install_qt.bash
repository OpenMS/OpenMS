...
do
  ...
  then
    ...
    prefix=$(otool -L $2/$lib.framework/$lib | grep QtCore | tr -d ' \t'|sed 's/QtCore.*//')

    # fix reference to core
    $3 -change ${prefix}QtCore.framework/Versions/4/QtCore \
      @executable_path/../lib/QtCore.framework/Versions/4/QtCore \
      $2/$lib.framework/Versions/4/$lib
  fi
done

# fix the QtGui dependencies for QtOpenGL and QtSvg
prefix=$(otool -L $2/QtOpenGL.framework/QtOpenGL | grep QtGui | tr -d ' \t'|sed 's/QtGui.*//')

#fix reference of QtOpenGL to QtGui
$3 -change ${prefix}QtGui.framework/Versions/4/QtGui \
