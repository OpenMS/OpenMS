...
class DependencyFixer
  def initialize(options, usage)
    ...
    @install_name_tool = options[:install_name_tool] || `which install_name_tool`.strip
    @debug = options[:verbose] || false
    @currentIndent = 0
    @executableId = options[:path_prefix] || "@executable_path/"
    @extract_fw = options[:extract_fw] || false
    @auto_relative = options.fetch(:auto_relative, true)
...
if __FILE__ == $0
  ...
  opts.each do |opt, arg|
    case opt
      ...
      when '--install-name-tool'
        options[:install_name_tool] = arg
      when '--lib-path'
        options[:lib_dir] = Pathname.new(arg).realpath
      when '--bin-path'
        options[:bin_dir] = Pathname.new(arg).realpath
      when '--plugin-path'
        options[:plugin_dir] = Pathname.new(arg).realpath
      when '--path-prefix'
        options[:path_prefix] = arg
      when '--extract-from-framework'
        options[:extract_fw] = true
      when '--no-auto-relative'
        options[:auto_relative] = false
      when '--no-copy'
        options[:nocopy] = true
      when '--verbose'
  ...
  DependencyFixer.new(options, usage).run
end
