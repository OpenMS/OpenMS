...
namespace OpenMS
{
  ...
  void Plot2DCanvas::paintEvent(QPaintEvent * e)
  {
    ...
    if (getLayerCount() == 0)
    {
      QPainter painter;
      painter.begin(this);
      painter.fillRect(0, 0, this->width(), this->height(), QColor(QString::fromStdString(String(param_.getValue("background_color").toString()))));
      painter.end();
      e->accept();
      return;
    }
    ...
    if (update_buffer_)
    {
      ...
      buffer_.fill(QColor(QString::fromStdString(String(param_.getValue("background_color").toString()))).rgb());
  ...
  void Plot2DCanvas::drawCoordinates_(QPainter & painter, const PeakIndex & peak)
  {
    ...
    { // if intensity is not mapped to X or Y, add it
      ...
      lines << QString::fromStdString(int_mapper.getDim(DIM::X).formattedValue(int_point.getX()));
    }
    ...
    else if (auto* lc = dynamic_cast<const LayerDataChrom*>(&layer))
    {
      ...
      if (!map_precursor_to_chrom_idx.empty())
      {
        ...
        for (auto mit = map_precursor_to_chrom_idx.cbegin(); mit != map_precursor_to_chrom_idx.cend(); ++mit)
        {
          ...
          QString precursor_string = QString("Precursor m/z: (")  + QString::fromStdString(String(mit->first.getCharge())) + ") " + QString::number(mit->first.getMZ());
          if (mit->first.metaValueExists("peptide_sequence"))
          {
            precursor_string = QString::number(mit->first.getMZ()) + " : " + QString::fromStdString(String(mit->first.getMetaValue("peptide_sequence"))) + " (" + QString::number(mit->first.getCharge()) + "+)";
          }
  ...
  void Plot2DCanvas::showCurrentLayerPreferences()
  {
    ...
    feature_icon->setCurrentIndex(feature_icon->findText(QString::fromStdString(String(layer.param.getValue("dot:feature_icon").toString()))));
