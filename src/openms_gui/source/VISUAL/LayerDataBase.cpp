...
namespace OpenMS
{
  ...
  std::string LayerDataBase::getDecoratedName() const
  {
    std::string n = name_ + name_suffix_;
    if (modified)
    {
      n += '*';
    }
...
*/
  ...
  LayerAnnotatorBase::LayerAnnotatorBase(const FileTypeList& supported_types, const std::string& file_dialog_text, QWidget* gui_lock) :
  ...
  bool LayerAnnotatorBase::annotateWithFileDialog(LayerDataBase& layer, LogWindow& log, const std::string& current_path) const
  {
  ...
  bool LayerAnnotatorBase::annotateWithFilename(LayerDataBase& layer, LogWindow& log, const std::string& fname) const
  {
    if (fname.empty())
    {
      return false;
    ...
    {
      log.appendNewHeader(LogWindow::LogState::NOTICE, "Error",StringUtils::toStr("Filename '" + fname + "' has unsupported file type. No annotation performed."));
      return false;
    }
  ...
  std::unique_ptr<LayerAnnotatorBase> LayerAnnotatorBase::getAnnotatorWhichSupports(const std::string& filename)
  {
    return getAnnotatorWhichSupports(FileHandler::getType(filename));
  }

  bool LayerAnnotatorPeptideID::annotateWorker_(LayerDataBase& layer, const std::string& filename, LogWindow& /*log*/) const
  {
  ...
  bool LayerAnnotatorAMS::annotateWorker_(LayerDataBase& layer, const std::string& filename, LogWindow& log) const
  {
    FeatureMap fm;
    ...
    std::string engine = "no protein identification section found";
    ...
    QMessageBox::warning(nullptr, "Error", (toQString(StringUtils::toStr("FeatureXML is currently only supported for files generated by the AccurateMassSearch tool (got '") + engine + "', expected 'AccurateMassSearch'.")));
    return false;
  }
