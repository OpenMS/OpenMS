...
*/
  ...
  bool LayerAnnotatorBase::annotateWithFilename(LayerDataBase& layer, LogWindow& log, const std::string& fname) const
  {
    ...
    {
      log.appendNewHeader(LogWindow::LogState::NOTICE, "Error",std::string("Filename '" + fname + "' has unsupported file type. No annotation performed."));
      return false;
    }
  ...
  bool LayerAnnotatorAMS::annotateWorker_(LayerDataBase& layer, const std::string& filename, LogWindow& log) const
  {
    ...
    QMessageBox::warning(nullptr, "Error", (toQString(std::string("FeatureXML is currently only supported for files generated by the AccurateMassSearch tool (got '") + engine + "', expected 'AccurateMassSearch'.")));
    return false;
  }
