...
namespace OpenMS
{
        ...
        && table_widget_->horizontalHeaderItem(Clmn::PEAK_ANNOTATIONS)->text() == Clmn::HEADER_NAMES[Clmn::PEAK_ANNOTATIONS])
    {
      ...
      if (item_pepid)// might be null for MS1 spectra
      {
        ...
        for (const PeptideHit::PeakAnnotation& pa : hit.getPeakAnnotations())
        {
          ...
          item->setData(Qt::DisplayRole, QString::fromStdString(pa.annotation));
  ...
  void SpectraIDViewTab::updateProteinEntries_(int selected_spec_row_idx)
  {
    ...
    for (Size i = 0; i < layer_->getPeakData()->getProteinIdentifications()[0].getHits().size(); ++i)
    {
      ...
      if (accs.empty() || accs.find(protein.getAccession()) != accs.end())
      {
        ...
        protein_table_widget_->setAtBottomRow(QString::fromStdString(protein.getAccession()), ProteinClmn::ACCESSION, bg_color, Qt::blue);
        protein_table_widget_->setAtBottomRow(QString::fromStdString(protein.getSequence()), ProteinClmn::FULL_PROTEIN_SEQUENCE, bg_color);
        protein_table_widget_->setAtBottomRow("show", ProteinClmn::SEQUENCE, bg_color, Qt::blue);
        protein_table_widget_->setAtBottomRow(QString::fromStdString(protein.getDescription()), ProteinClmn::DESCRIPTION, bg_color);
  ...
  void SpectraIDViewTab::updateEntries_()
  {
    ...
    {
      headers << QString::fromStdString(ck);
    }
    ...
    for (Size i = 0; i < layer_->getPeakData()->getMSExperiment().size(); ++i)
    {
      ...
      else
      {
        ...
        for (Size ph_idx = 0; ph_idx != peptide_id.getHits().size(); ++ph_idx)
        {
          ...
          table_widget_->setAtBottomRow(QString::fromStdString(seq), Clmn::SEQUENCE, bg_color);
       ...
       // accession, start and end in protein (note that one peptide might match twice into same protein)
        ...
        table_widget_->setAtBottomRow(QString::fromStdString(accessions), Clmn::ACCESSIONS, bg_color);
        table_widget_->setAtBottomRow(QString::fromStdString(starts), Clmn::START, bg_color);
        table_widget_->setAtBottomRow(QString::fromStdString(ends), Clmn::END, bg_color);
        ...
        if (!precursors.empty())
        {
          ...
          if (create_rows_for_commmon_metavalue_->isChecked())
          {
            ...
            if (has_peak_annotations)
            {
              ...
              {
                annotation += QString::fromStdString(String(pa.mz)) + "|" +
                  QString::fromStdString(String(pa.intensity)) + "|" +
                  QString::fromStdString(String(pa.charge)) + "|" +
                  QString::fromStdString(pa.annotation) + ";";
              }
            ...
            for (const auto& ck : common_keys)
            {
              ...
              {
                table_widget_->setAtBottomRow(QString::fromStdString(dv), current_col, bg_color);
              }
  ...
  void SpectraIDViewTab::fillRow_(const MSSpectrum& spectrum, const int spec_index, const QColor& background_color)
  {
    ...
    if (!precursors.empty())
    {
      ...
      table_widget_->setAtBottomRow(QString::fromStdString(ListUtils::concatenate(first_precursor.getActivationMethodsAsString(), ",")), Clmn::DISSOCIATION, background_color);
