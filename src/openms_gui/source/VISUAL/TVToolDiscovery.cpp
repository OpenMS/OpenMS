...
namespace OpenMS
{
  ...
  Param TVToolDiscovery::getParamFromIni_(const String& tool_path, bool plugins)
  {
    ...
    QStringList args{"-write_ini", QString::fromStdString(path)};
    Param tool_param;
    String executable;
    ...
    // Write tool ini to temporary file
    ++running_processes;
    auto return_state = proc.run(QString::fromStdString(executable), args, QString::fromStdString(working_dir), true, ExternalProcess::IO_MODE::NO_IO);
    --running_processes;
  ...
  bool TVToolDiscovery::setPluginPath(const String& plugin_path, bool create)
  {
    if (!File::exists(plugin_path))
    {
      if (create)
      {
        QDir path = QDir(QString::fromStdString(plugin_path));
