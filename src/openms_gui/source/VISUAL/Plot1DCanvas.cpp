...
namespace OpenMS
{
  ...
  void Plot1DCanvas::paint(QPainter* painter, QPaintEvent* e)
  {
    ...
    painter->fillRect(0, 0, this->width(), this->height(),
                      QColor(QString::fromStdString(String(param_.getValue("background_color").toString()))));
  ...
  void Plot1DCanvas::drawHighlightedPeak_(Size layer_index, const PeakIndex& peak, QPainter& painter, bool draw_elongation)
  {
    ...
    painter.setPen(QPen(QColor(QString::fromStdString(String(param_.getValue("highlighted_peak_color").toString()))), 2));
    ...
    if (draw_elongation)
    {
      ...
      Painter1DBase::drawDashedLine(begin, top_end, &painter, QString::fromStdString(String(param_.getValue("highlighted_peak_color").toString())));
    }
  }
...
/*    const MSSpectrum& spectrum = getCurrentLayer().getCurrentSpectrum();
  ...
  */
  ...
  void Plot1DCanvas::drawCoordinates_(QPainter& painter, const PeakIndex& peak)
  {
    ...
    QStringList lines;
    lines << QString::fromStdString(unit_mapper_.getDim(DIM::X).formattedValue(xy_point.getX()));
    lines << QString::fromStdString(unit_mapper_.getDim(DIM::Y).formattedValue(xy_point.getY()));
  ...
  void Plot1DCanvas::drawDeltas_(QPainter& painter, const PeakIndex& start, const PeakIndex& end)
  {
    ...
    auto dim_text = [](const DimBase& dim, double start_pos, double end_pos, bool ratio /*or difference*/)
    {
      ...
      if (ratio)
      {
        result = QString::fromStdString(dim.formattedValue(end_pos / start_pos, " ratio "));
      }
      else
      {
        result = QString::fromStdString(dim.formattedValue(end_pos - start_pos, " delta "));
  ...
  void Plot1DCanvas::showCurrentLayerPreferences()
  {
    ...
    peak_color->setColor(QColor(QString::fromStdString(String(layer.param.getValue("peak_color").toString()))));
    icon_color->setColor(QColor(QString::fromStdString(String(layer.param.getValue("icon_color").toString()))));
    annotation_color->setColor(QColor(QString::fromStdString(String(layer.param.getValue("annotation_color").toString()))));
    bg_color->setColor(QColor(QString::fromStdString(String(param_.getValue("background_color").toString()))));
    selected_color->setColor(QColor(QString::fromStdString(String(param_.getValue("highlighted_peak_color").toString()))));
  ...
  void Plot1DCanvas::contextMenuEvent(QContextMenuEvent* e)
  {
    ...
    else // !annot_item
    {
      ...
      context_menu->addAction(QString::fromStdString(layer_name))->setEnabled(false);
      ...
      context_menu->addAction(QString::fromStdString(String("Add peak annotation ") + String(getNonGravityDim().getDimNameShort())), [&]() {
        const auto xy_point = getCurrentLayer().peakIndexToXY(near_peak, unit_mapper_);
        QString label = QString::fromStdString(getNonGravityDim().formattedValue(gr_.swap().gravityValue(xy_point)));
        addPeakAnnotation(near_peak, label, QString::fromStdString(String(getCurrentLayer().param.getValue("peak_color").toString())));
      })->setEnabled(near_peak.isValid());
  ...
  void Plot1DCanvas::addUserPeakAnnotation_(PeakIndex near_peak)
  {
    ...
    if (ok && !text.isEmpty())
    {
      addPeakAnnotation(near_peak, text, QColor(QString::fromStdString(String(getCurrentLayer().param.getValue("peak_color").toString()))));
    }
  }
