...
namespace OpenMS
{
  namespace Internal
  {
            ...
            [&](const String& out) {writeLog_(QString::fromStdString(out));})
    {
      ...
      py_pyprophet->validate(QString::fromStdString(py_selector->getLastPython()));
    ...
    void SwathTabWidget::on_btn_runPyProphet_clicked()
    {
      ...
      for (const auto& file : inputs)
      {
        ...
        {
          QMessageBox::warning(this, "Error", QString::fromStdString(String("Required input file '" + file.first + "' not found. Please run OpenSwathWorkflow first to create it")));
          return;
        }
      ...
      if (library.isEmpty())
      {
        QMessageBox::warning(this, "Error", QString::fromStdString(String("The assay library is not specified. Please go to the 'database' tab and specify it.")));
        return;
      }
      ...
      {
        QMessageBox::warning(this, "Error", QString::fromStdString(String("Could not find 'pyprophet' in the python installation '" + ui->py_selector->getLastPython() + "'. Please make sure it is installed. Visit http://openswath.org/en/latest/docs/tric.html for details.")));
        return;
      }
      ...
      {
        QMessageBox::warning(this, "Error", QString::fromStdString(String("Could not find 'feature_alignment.py' from the msproteomicstool package in the python installation '" + ui->py_selector->getLastPython() + "'. Please make sure it is installed. Visit http://openswath.org/en/latest/docs/tric.html for details.")));
        return;
      }
      ...
      { 
        ...
        for (size_t i_loop = 0; i_loop < call.getLoopCount(); ++i_loop)
        {
          ...
          {
            QMessageBox::warning(this, "Error", QString::fromStdString(String("Running pyprophet/TRIC failed at step " + String(step) + "/" + String(calls.size()) + ". Please see log for details")));
            return;
          }
    ...
    void SwathTabWidget::on_pushButton_clicked()
    {
      ...
      for (int i = 0; i < tbl.rowCount(); ++i)
      {
        ...
        {
          ++selected_rows;
          args << QString::fromStdString(infileToChrom(raw_files[i])) << "!" << osw_files[i];
          if (!File::exists(osw_files[i])) missing_osw_files << QString::fromStdString(File::basename(osw_files[i]));
        }
      }
      if (selected_rows == 0)
      {
      ...
      if (QMessageBox::question(this, "Confirm", QString::fromStdString(String("Confirm opening ") + selected_rows + " raw files in TOPPView"), 
                             ...
                             == QMessageBox::StandardButton::Ok)
      {
        ...
        qp->start(QString::fromStdString(tv), args);
        if (!qp->waitForStarted(2000))
        {
          QMessageBox::warning(this, "Error", QString::fromStdString(String("Could not open TOPPView executable from '" + tv + "'")));
          return;
        }
