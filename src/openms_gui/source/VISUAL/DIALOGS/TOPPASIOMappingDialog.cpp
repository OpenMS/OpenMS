  ...
  void TOPPASIOMappingDialog::fillComboBoxes_()
  {
    ...
    if (target_dir)
    {
      ...
      for (const auto& info : source_output_dirs)
      {
        ...
        ui_->source_combo->addItem(QString::fromStdString(item_name), source_output_dirs.indexOf(info));
      }
    ...
    else if (source_tool)
    {
      ...
      ui_->source_label->setText(QString::fromStdString(source_tool->getName()));
      if (!source_tool->getType().empty())
      {
        ui_->source_type_label->setText("(" + QString::fromStdString(source_tool->getType()) + ")");
      }
      else
      {
      ...
      for (TOPPASToolVertex::IOInfo info : source_output_files)
      {
        ...
        ui_->source_combo->addItem(QString::fromStdString(item_name), source_output_files.indexOf(info));
      }
    }
    ...
    if (target_tool)
    {
      ...
      ui_->target_label->setText(QString::fromStdString(target_tool->getName()));
      if (!target_tool->getType().empty())
      {
        ui_->target_type_label->setText("(" + QString::fromStdString(target_tool->getType()) + ")");
      }
      else
      {
      ...
      for (TOPPASToolVertex::IOInfo info : target_input_files)
      {
        ...
        ui_->target_combo->addItem(QString::fromStdString(item_name), target_input_files.indexOf(info));
      }
    }
