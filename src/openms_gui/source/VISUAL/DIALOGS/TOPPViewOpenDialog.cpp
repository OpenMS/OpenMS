...
namespace OpenMS
{
    ...
    ui_(new Ui::TOPPViewOpenDialogTemplate)
  {
    ...
    //set title
    setWindowTitle(QString::fromStdString(static_cast<const std::string&>(String("Open data options for ") + data_name)));
  }
  ...
  void TOPPViewOpenDialog::setMergeLayers(const std::map<Size, String> & layers)
  {
    ...
    if (!layers.empty())
    {
      ...
      {
        ui_->merge_combo_->insertItem(i++, QString::fromStdString(static_cast<const std::string&>(it.second)), (int)(it.first));
      }
    }
    else
    {
