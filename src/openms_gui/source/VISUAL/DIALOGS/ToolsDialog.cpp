...
namespace OpenMS
{
            ...
            layer_type_(layer_type)
  {
    ...
    auto layer_label_name = new QLabel(QString::fromStdString(layer_name));
  ...
  QStringList ToolsDialog::createToolsList_()
  {
    ...
    for (const auto& name : tool_scanner_->getPlugins())
    {
      ...
      if (std::find(tool_types.begin(), tool_types.end(), layer_type_) != tool_types.end())
      {
        list << QString::fromStdString(String(name));
      }
    }
  ...
  void ToolsDialog::createINI_()
  {
    ...
    tool_desc_->setText(QString::fromStdString(String(arg_param_.getSectionDescription(tool_name))));
