...
namespace OpenMS
{
  namespace Internal
  {
    ...
    void TOPPViewPrefDialog::setParam(const Param& param)
    {
      ...
      // general tab
      ui_->default_path->setText(QString::fromStdString(String(param_.getValue("default_path").toString())));
      ui_->default_path_current->setChecked(param_.getValue("default_path_current").toBool());
      ui_->plugins_path->setText(QString::fromStdString(String(param_.getValue("plugins_path").toString())));
      ...
      ui_->map_default->setCurrentIndex(ui_->map_default->findText(QString::fromStdString(String(param_.getValue("default_map_view").toString()))));
      ui_->map_cutoff->setCurrentIndex(ui_->map_cutoff->findText(QString::fromStdString(String(param_.getValue("intensity_cutoff").toString()))));
      ui_->on_file_change->setCurrentIndex(ui_->on_file_change->findText(QString::fromStdString(String(param_.getValue("on_file_change").toString()))));

      // 1D view
      ui_->color_1D->setColor(QColor(QString::fromStdString(String(param_.getValue("1d:peak_color").toString()))));
      ui_->selected_1D->setColor(QColor(QString::fromStdString(String(param_.getValue("1d:highlighted_peak_color").toString()))));
      ui_->icon_1D->setColor(QColor(QString::fromStdString(String(param_.getValue("1d:icon_color").toString()))));

      // 2D view
      ui_->peak_2D->gradient().fromString(param_.getValue("2d:dot:gradient"));
      ui_->feature_icon_2D->setCurrentIndex(ui_->feature_icon_2D->findText(QString::fromStdString(String(param_.getValue("2d:dot:feature_icon").toString()))));
      ...
      ui_->unit->setCurrentIndex(ui_->unit->findText(QString::fromStdString(String(param_.getValue("idview:align:is_relative_tolerance") == "true" ? "ppm" : "Da"))));
    }
