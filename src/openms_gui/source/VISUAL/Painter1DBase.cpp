...
namespace OpenMS
{
  ...
  {
    const QColor col {QColor(toQString(std::string(layer->param.getValue("annotation_color").toString())))};
  ...
  void Painter1DPeak::paint(QPainter* painter, Plot1DCanvas* canvas, int layer_index)
  {
    ...
    QPen pen(QColor(toQString(std::string(layer_->param.getValue("peak_color").toString()))), 1);
    pen.setStyle(canvas->peak_penstyle_[layer_index]);
    painter->setPen(pen);

    // draw dashed elongations for pairs of peaks annotated with a distance
    const QColor color = toQString(std::string(canvas->param_.getValue("highlighted_peak_color").toString()));
