...
namespace OpenMS
{
  ...
  {
    const QColor col {QColor(toQString(StringUtils::toStr(layer->param.getValue("annotation_color").toString())))};
  ...
  void Painter1DPeak::paint(QPainter* painter, Plot1DCanvas* canvas, int layer_index)
  {
    ...
    QPen pen(QColor(toQString(StringUtils::toStr(layer_->param.getValue("peak_color").toString()))), 1);
    pen.setStyle(canvas->peak_penstyle_[layer_index]);
    painter->setPen(pen);

    // draw dashed elongations for pairs of peaks annotated with a distance
    const QColor color = toQString(StringUtils::toStr(canvas->param_.getValue("highlighted_peak_color").toString()));
  ...
  void Painter1DPeak::drawMZAtInterestingPeaks_(QPainter& painter, Plot1DCanvas* canvas, MSSpectrum::ConstIterator v_begin, MSSpectrum::ConstIterator v_end) const
  {
    ...
    for (size_t i = 0; i < spec.size(); ++i)
    {
      ...
      QString label = toQString(StringUtils::number(mz, 4));
