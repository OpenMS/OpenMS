...
This tool can apply retention time transformations to different types of data (mzML, featureXML/featureparquet, consensusXML/consensusparquet, and idXML/idparquet files).
...
protected:
  ...
  ExitCodes main_(int, const char**) override
  {
    ...
    if (!in.empty()) // load input
    {
      ...
      else if (in_type == FileTypes::FEATUREXML || in_type == FileTypes::FEATUREPARQUET)
      {
        FeatureMap map;
        FileHandler().loadFeatures(in, map, {FileTypes::FEATUREXML, FileTypes::FEATUREPARQUET}, log_type_);
        applyTransformation_( trafo, map);
        FileHandler().storeFeatures(out, map,
            {in_type == FileTypes::FEATUREPARQUET ? FileTypes::FEATUREPARQUET
                                                  : FileTypes::FEATUREXML}, log_type_);
      }
      else if (in_type == FileTypes::CONSENSUSXML || in_type == FileTypes::CONSENSUSPARQUET)
      {
        ConsensusMap map;
        FileHandler().loadConsensusFeatures(in, map, {FileTypes::CONSENSUSXML, FileTypes::CONSENSUSPARQUET}, log_type_);
        applyTransformation_( trafo, map);
        FileHandler().storeConsensusFeatures(out, map,
            {in_type == FileTypes::CONSENSUSPARQUET ? FileTypes::CONSENSUSPARQUET
                                                    : FileTypes::CONSENSUSXML}, log_type_);
      }
      else if (in_type == FileTypes::IDXML || in_type == FileTypes::IDPARQUET)
      {
        vector<ProteinIdentification> proteins;
        PeptideIdentificationList peptides;
        FileHandler().loadIdentifications(in, proteins, peptides, {FileTypes::IDXML, FileTypes::IDPARQUET}, log_type_);
        bool store_original_rt = getFlag_("store_original_rt");
        MapAlignmentTransformer::transformRetentionTimes(peptides, trafo,
                                                         store_original_rt);
        // no "data processing" section in idXML/idparquet
        FileHandler().storeIdentifications(out, proteins, peptides,
            {in_type == FileTypes::IDPARQUET ? FileTypes::IDPARQUET
                                             : FileTypes::IDXML}, log_type_);
      }
    }
