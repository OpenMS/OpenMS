...
@note End-of-search summary: after each run ProSE prints a consolidated report to the terminal covering configuration, database/index stats, per-file identification results (matched spectra, unique peptides and proteins, target/decoy counts, achieved FDR, charge and missed-cleavage histograms, HyperScore distribution), per-run tolerance estimation, per-phase timing, open-search modification discovery, and an output-file manifest. Use '-summary_out <file.yaml>' to additionally write the same data as a machine-readable YAML file for pipeline ingestion; use '-no_summary' to suppress the terminal block.
