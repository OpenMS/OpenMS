...
@note End-of-search summary report: after each run ProSE prints a consolidated report to the command line covering database/index statistics, per-file identification metrics (PSM count, ID rate, unique peptides/proteins, charge and missed-cleavage histograms, HyperScore distribution), per-run tolerance estimation, phase timing, and an output manifest. Use @c -summary_out to capture the same information as a machine-readable YAML file suitable for downstream pipeline ingestion. Use @c -no_summary to suppress the console output (the YAML file is still written if @c -summary_out is set).
