...
@note End-of-search report: after all files are processed, ProSE prints a human-readable summary to the console (suppress with @c -no_summary) and can write the same information as machine-readable YAML to @c -summary_out. The YAML captures per-file identification statistics, shared configuration/database/index facts, timing, and the output manifest — useful for downstream pipeline inspection or scripted post-processing.
