...
namespace OpenMS
{
  ...
  namespace
  {
    // This tool reads and binds native SQLite values. Keep that adapter local:
    // SqliteConnector_impl.h and its non-exported helpers belong to libOpenMS.
    namespace Sql
    {
      // Own the connection in this executable. Never pass a native SQLite handle
      // across the OpenMS boundary: an installed SDK may embed a different SQLite.
      class Connection
      {
      public:
        enum class Mode
        ...
        Connection(const std::string& filename, Mode mode)
        {
          const int flags = mode == Mode::READ_ONLY ? SQLITE_OPEN_READONLY : SQLITE_OPEN_READWRITE;
          sqlite3* db = nullptr;
          ...
          if (rc != SQLITE_OK)
          {
            throw Exception::SqlOperationFailed(__FILE__, __LINE__, OPENMS_PRETTY_FUNCTION,
                                                "Could not open sqlite db '" + filename + "' in mode " + StringUtils::toStr(int(mode)));
          }
        }
        ...
        bool tableExists(const std::string& name)
        {
          ...
          const std::string sql = "SELECT 1 FROM sqlite_master WHERE type='table' AND name='" + name + "';";
          if (sqlite3_prepare_v2(db_.get(), sql.c_str(), static_cast<int>(sql.size()), &stmt, nullptr) != SQLITE_OK)
          {
            throw Exception::IllegalArgument(__FILE__, __LINE__, OPENMS_PRETTY_FUNCTION, sqlite3_errmsg(db_.get()));
          }
        ...
        void executeStatement(const std::string& sql)
        {
          ...
          {
            const std::string message = error ? error : sqlite3_errmsg(db_.get());
            sqlite3_free(error);
            throw Exception::IllegalArgument(__FILE__, __LINE__, OPENMS_PRETTY_FUNCTION, message);
          }
        }
      ...
      void prepareStatement(Connection& conn, sqlite3_stmt** stmt, const std::string& sql)
      {
      ...
      SqlState nextRow(sqlite3_stmt* stmt, SqlState current = SqlState::SQL_ROW)
      {
        ...
        { // querying a new row after the last invocation gave 'SQL_DONE' might loop around
          ...
          throw Exception::SqlOperationFailed(
            ...
            "Sql operation requested on SQL_DONE/SQL_ERROR state. This should never happen. Please file a bug report!");
        }
        int rc = sqlite3_step(stmt);
        if (rc == SQLITE_ROW) { return SqlState::SQL_ROW; }
        if (rc == SQLITE_DONE) { return SqlState::SQL_DONE; }
        if (rc == SQLITE_ERROR)
        {
    ...
    std::vector<String> sqliteColumnsWithPrefix_(Sql::Connection& conn,
                                                 const String& table_name,
                                                 const String& prefix)
    {
      sqlite3_stmt* stmt = nullptr;
      Sql::prepareStatement(conn, &stmt, "PRAGMA table_info('" + table_name + "');");
    ...
    {
      Sql::Connection conn(osw_path, Sql::Connection::Mode::READ_ONLY);
      sqlite3_stmt* stmt = nullptr;
      std::vector<ScoreRow> rows;
      std::vector<String> feature_names;
      ...
      if (level == Level::MS1 || level == Level::MS2 || level == Level::MS1MS2)
      {
        ...
        Sql::prepareStatement(conn, &stmt, select_sql);
        Sql::SqlState state = Sql::nextRow(stmt);
    ...
    {
      Sql::Connection conn(osw_path, Sql::Connection::Mode::READWRITE);
      ...
      try
      {
        ...
        if (transition_level)
        {
          Sql::prepareStatement(conn, &insert_stmt,
