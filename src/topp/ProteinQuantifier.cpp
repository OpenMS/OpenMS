...
With @p file_and_channel_level_output the protein abundances are instead reported per (file, channel) cell. These cells are computed with the same peptide-level policy as the sample-level values (all peptidoforms and all charge states of a peptide are accumulated into one peptide - see the note on @p best_charge_and_fraction below for the one exception), but the peptide selection and the aggregation are applied <em>per file</em>. Two consequences are worth knowing:
...
Both statements above assume @p best_charge_and_fraction is off (the default). That option changes only the sample-level values - it reduces each peptide to a single (fraction, file, charge, channel) observation - while the (file, channel) cells keep accumulating all charge states. With it enabled the two granularities describe different quantities and do not decompose for any setting of @p top:N and @p top:aggregate.
