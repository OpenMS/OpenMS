...
With @p file_and_channel_level_output the protein abundances are instead reported per (file, channel) cell. These cells are computed with the same peptide-level policy as the sample-level values (all peptidoforms and all charge states of a peptide are accumulated into one peptide), but the peptide selection and the aggregation are applied <em>per file</em>. Two consequences are worth knowing:
