...
LabeledPMF<VARIABLE_KEY> mult_or_div(const LabeledPMF<VARIABLE_KEY> & lhs, const LabeledPMF<VARIABLE_KEY> & rhs) {
  ...
  if (already_in_order) {
    ...
    if (MULT)
      return LabeledPMF<VARIABLE_KEY>( new_variable_order, PMF(new_first_support, semi_outer_product(lhs_view_and_first_sup.first, rhs_view_and_first_sup.first, intersection_size)) );
    else
      return LabeledPMF<VARIABLE_KEY>( new_variable_order, PMF(new_first_support, semi_outer_quotient(lhs_view_and_first_sup.first, rhs_view_and_first_sup.first, intersection_size)) );
  }
  else {
    ...
    if (MULT)
      return LabeledPMF<VARIABLE_KEY>( new_variable_order, PMF(new_first_support, semi_outer_product(lhs_part, rhs_part, intersection_size)) );
    else
      return LabeledPMF<VARIABLE_KEY>( new_variable_order, PMF(new_first_support, semi_outer_quotient(lhs_part, rhs_part, intersection_size)) );
  }
}
