...
public:
  ...
  void print(std::ostream & os, unsigned int depth=0) {
    for (unsigned int i=0; i<3*depth; ++i)
      os << " ";
    os << "prior&support " << _prior_ready << _support_from_below_ready << " likelihood&support " << _likelihood_ready << _support_from_above_ready << " min/max possible support " << _minimum_possible_first_support << " " << _maximum_possible_last_support << " prior/likelihood " << _prior << " " << _likelihood << std::endl;
