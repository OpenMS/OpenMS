...
public:
    ...
    */
    ...
    /**
      ...
      @param[in,out] A Input pointer to Eigen::MatrixXd A of size m x n (Note: due to an in-place algorithm, A will be modified!)
      @param[in,out] b Input vector b of size m (Note: due to an in-place algorithm, b will be modified!)‚
      @param[out] x Output vector with non-negative least square solution of size n. Contents will be overwritten!‚
      @return status of solution (either NonNegativeLeastSquaresSolver::SOLVED, NonNegativeLeastSquaresSolver::ITERATION_EXCEEDED)
