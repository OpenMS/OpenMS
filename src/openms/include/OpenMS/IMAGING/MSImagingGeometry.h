...
public:
  ...
  */
  ...
  /// @brief Sentinel returned by regionOf() when (x,y) belongs to no region. required for pyopenms
  static constexpr Size NO_REGION = std::numeric_limits<Size>::max();
  /// @throws InvalidValue duplicate id / NO_REGION id, overlap
  void addRegion(const MSImagingRegion& region);
  /// @throws ElementNotFound unknown id
