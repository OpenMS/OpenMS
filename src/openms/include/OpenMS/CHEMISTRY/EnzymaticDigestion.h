  ...
  */
  ...
  public:
    ...
    /**
       ...
       */
    Size digestUnmodified(const std::string_view& sequence, std::vector<std::string_view>& output, Size min_length = 1, Size max_length = 0) const;

    /**
     @brief Performs the enzymatic digestion of an unmodified sequence.

     By returning only positions into the original string this is very fast and compared to the std::string_view output
  ...
  protected:
    ...
    */
    ...
    /**
       ...
       @return number of digestion products NOT matching the length restrictions
    */
    Size digestAfterTokenize_(const std::vector<int>& fragment_positions, const std::string_view& sequence, std::vector<std::string_view>& output, Size min_length = 0, Size max_length = -1) const;
    Size digestAfterTokenize_(const std::vector<int>& fragment_positions, const std::string_view& sequence, std::vector<std::pair<Size, Size>>& output, Size min_length = 0, Size max_length = -1) const;
