    ...
    MzReductionFunctionType func_mz_reduction) const
{
    ...
    if (needs_im_filtering)
    {
      ...
      {
        auto [im_unit, im_values] = spec_ref.get().maybeGetIMData();
    ...
    for (Int64 i = 0; i < (Int64)spec_idx_to_range_idx.size(); ++i)
    {
      ...
      if (needs_im_filtering)
      {
        std::tie(im_unit, im_values) = spec.maybeGetIMData();
      }
      ...
      for (size_t range_idx : spec_idx_to_range_idx[i])
      {
        ...
        if (!im_range.isEmpty())
        {
          ...
          {
            size_t peak_idx = std::distance(spec.cbegin(), it);
            if (peak_idx < im_values.size() && im_range.containsMobility(im_values[peak_idx]))
            {
    ...
    MzReductionFunctionType func_mz_reduction) const
{
    ...
    for (Int64 i = 0; i < (Int64)spec_idx_to_range_idx.size(); ++i)
    {
      ...
      for (size_t range_idx : spec_idx_to_range_idx[i])
      {
        ...
        {
          // IM filtering enabled: filter peaks by IM value
          // Create filtered vector of peaks within IM range
          std::vector<Peak1D> filtered_peaks;
          filtered_peaks.reserve(std::distance(start_it, end_it));
    ...
    MzReductionFunctionType func_mz_reduction) const
{
    ...
    // For aggregate with precursor filtering, we need dynamic results since spectra may be skipped
    std::vector<std::vector<MSExperiment::CoordinateType>> result(ranges.size());
    std::vector<std::vector<std::pair<size_t, CoordinateType>>> temp_results(ranges.size());
    ...
    for (Int64 i = 0; i < (Int64)spec_idx_to_range_idx.size(); ++i)
    {
        ...
        for (size_t range_idx : spec_idx_to_range_idx[i])
        {
            ...
            #pragma omp critical
            {
                temp_results[range_idx].emplace_back(i, value);
            }
        }
    }

    // Sort by spectrum index and extract values
    for (size_t i = 0; i < result.size(); ++i)
    {
        std::sort(temp_results[i].begin(), temp_results[i].end(),
                  [](const auto& a, const auto& b) { return a.first < b.first; });
        result[i].reserve(temp_results[i].size());
        for (const auto& [idx, val] : temp_results[i])
        {
