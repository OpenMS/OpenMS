    ...
    struct ContainerAdd_<ContainerValueType, true>
    {
      ...
      static void addData_(SpectrumType* spectrum, const ContainerValueType* item)
      {
        if (item->metaValueExists("num_of_masstraces"))
        {
          ...
          {
            std::string meta_name =StringUtils::toStr("masstrace_intensity_") + i;
            if (!item->metaValueExists(meta_name))
            {
              throw Exception::Precondition(__FILE__, __LINE__, OPENMS_PRETTY_FUNCTION,StringUtils::toStr("Meta value '") + meta_name + "' expected but not found in container.");
            }
