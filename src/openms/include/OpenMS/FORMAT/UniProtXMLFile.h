...
namespace OpenMS
{
  ...
  */
  ...
  /**
    ...
    Captures only the fields needed to build a PEFF descriptor line; isoforms
    (sequences without a "length" attribute) and splice-variant features are
    intentionally not represented because UniPEFF's PEFF emission ignores them.
  */
  struct OPENMS_DLLAPI UniProtEntry
  {
    ...
    std::string sequence;                      ///< canonical &lt;sequence&gt; text (whitespace stripped); isoform sequences are skipped
  ...
  /**
    ...
    XML release schema; isoforms (sequences without the "length" attribute on
    &lt;sequence&gt;) and the &lt;comment type="alternative products"&gt; subtree are
    skipped, matching UniPEFF's PEFF-emission behaviour. Gzip is detected by
    magic-byte sniffing in the inherited XMLFile parser, so no special handling
    is required for .xml.gz inputs.
