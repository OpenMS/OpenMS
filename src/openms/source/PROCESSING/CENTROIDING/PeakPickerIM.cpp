...
double PeakPickerIM::computeOptimalSamplingRate(const vector<MSSpectrum>& spectra)
{
  ...
  for (size_t s = 0; s < spectra.size(); ++s)
  {
    ...
    {
#ifdef DEBUG_PICKER
      OPENMS_LOG_DEBUG << "Skipping trace " << s << " because it has too few points (" << summed_trace.size() << ").\n";
#endif
  ...
  {
    if (diff <= threshold) { small_mz_diffs.push_back(diff); }
  }
...
void PeakPickerIM::sumFrame_(const MSSpectrum& input_spectrum, MSSpectrum& output_spectrum, double tolerance, bool use_ppm)
{
  ...
  for (Size i = 1; i < input_spectrum.size(); ++i)
  {
    ...
    bool within_tolerance = use_ppm ? ((delta_mz / current_mz) * 1e6 <= tolerance) : (delta_mz <= tolerance);

    if (within_tolerance) { current_intensity += next_intensity; }
...
vector<MSSpectrum> PeakPickerIM::extractIonMobilityTraces(const MSSpectrum& picked_spectrum, const MSSpectrum& raw_spectrum)
{
  ...
  for (size_t i = 0; i < picked_spectrum.size(); ++i)
  {
    ...
    while (right_idx < static_cast<SignedSize>(raw_spectrum.size()) && raw_spectrum[right_idx].getMZ() <= upper_bound)
    {
...
MSSpectrum PeakPickerIM::computeCentroids_(const vector<MSSpectrum>& mobilogram_traces, const vector<MSSpectrum>& picked_traces)
{
  ...
  for (size_t i = 0; i < picked_traces.size(); ++i)
  {
    ...
    for (Size j = 0; j < picked_trace.size(); ++j)
    {
...
#ifdef DEBUG_PICKER
      OPENMS_LOG_DEBUG << "Picked peak " << j << " IM centroid: " << centroid_im << " ion mobility FWHM: " << fwhm << " --> IM bounds: [" << im_lower
                       << ", " << im_upper << "]\n";
#endif
      ...
      // --- Expand Right ---
      SignedSize right_idx = center_idx + 1;
      while (right_idx < static_cast<SignedSize>(raw_trace.size()) && raw_trace[right_idx].getMZ() <= im_upper)
      {
        Peak1D new_peak;
...
#ifdef DEBUG_PICKER
      OPENMS_LOG_DEBUG << "Picked IM peak " << j << ": collected " << raw_peaks_within_bounds.size() << " raw m/z points between IM [" << im_lower
                       << ", " << im_upper << "]\n";
#endif
...
#ifdef DEBUG_PICKER
        OPENMS_LOG_DEBUG << "[INFO] Only one raw peak found. Added directly to centroided_frame. m/z: " << single_peak.getMZ()
      ...
      for (size_t i = 0; i < raw_mz_peaks.size(); ++i)
      {
        ...
        if (i > 0 && current_mz < mz_values.back()) { is_sorted = false; }
      ...
      if (! is_sorted)
      {
        ...
        std::sort(indices.begin(), indices.end(), [&mz_values](size_t i1, size_t i2) { return mz_values[i1] < mz_values[i2]; });
      ...
      if (spline.eval(mz_left) > half_height) { mz_mid = mz_left; }
      else
      {
        do
        {
          ...
          if (int_mid < half_height) { mz_left = mz_mid; }
          else { mz_center = mz_mid; }
      ...
      if (spline.eval(mz_right) > half_height) { mz_mid = mz_right; }
      else
      {
        do
        {
          ...
          if (int_mid < half_height) { mz_right = mz_mid; }
          else { mz_center = mz_mid; }
...
void removeAllFloatDataArraysExcept(OpenMS::MSSpectrum& spectrum, const String& keep_name)
{
  ...
  auto new_end = std::remove_if(float_arrays.begin(), float_arrays.end(), [&keep_name](const MSSpectrum::FloatDataArray& array) {
...
PeakPickerIM::PeakPickerIM(): DefaultParamHandler("PeakPickerIM")
{
  // --- PickIMTraces parameters ---
  defaults_.setValue("pickIMTraces:sum_tolerance_mz", 1.0, "Tolerance for summing adjacent m/z peaks (ppm)");
  defaults_.setValue("pickIMTraces:sum_tolerance_im", 0.0006,
                     "Tolerance for summing adjacent ion mobility peaks (in 1/K0 units). For CCS data, use larger values (e.g., 1.0).");
  ...
  defaults_.setValue("pickIMCluster:im_tolerance_cluster", 0.1,
                     "Ion mobility tolerance for clustering (in 1/K0 units). For CCS data, use larger values (e.g., 10-20).");
  ...
  // --- PickIMSage parameters ---
  defaults_.setValue("pickIMSage:ppm_tolerance_sage", 50.0, "m/z tolerance in ppm for Sage algorithm");
  defaults_.setValue("pickIMSage:im_tolerance_sage", 0.1, "Ion mobility tolerance for Sage algorithm");
...
void PeakPickerIM::updateMembers_()
{
  ...
  ppm_tolerance_sage_ = (double)param_.getValue("pickIMSage:ppm_tolerance_sage");
  im_tolerance_sage_ = (double)param_.getValue("pickIMSage:im_tolerance_sage");
}
...
namespace
{
  ...
  struct Point
  {
    ...
    Point(double mz_val, double im_val, float int_val, OpenMS::Size idx): mz(mz_val), im(im_val), intensity(int_val), original_index(idx)
    {
    }
  };
} // namespace
...
void PeakPickerIM::pickIMTraces(MSSpectrum& spectrum)
{
  ...
  if (! validateIMFormatForPicking(spectrum)) { return; }
  ...
  mobilogram_traces.erase(std::remove_if(mobilogram_traces.begin(), mobilogram_traces.end(), [](const auto& trace) { return trace.empty(); }),
...
void PeakPickerIM::pickIMCluster(OpenMS::MSSpectrum& spectrum) const
{
  ...
  std::sort(mz_sorted_indices.begin(), mz_sorted_indices.end(), [&](OpenMS::Size a, OpenMS::Size b) { return points[a].mz < points[b].mz; });
  ...
  std::sort(intensity_sorted_indices.begin(), intensity_sorted_indices.end(), [&](OpenMS::Size a, OpenMS::Size b) {
  ...
  for (OpenMS::Size current_intensity_rank = 0; current_intensity_rank < points.size(); ++current_intensity_rank)
  {
    ...
    OpenMS::SignedSize left_idx = static_cast<OpenMS::SignedSize>(seed_sorted_pos);
    ...
    while (changed)
    {
      ...
      while (left_idx >= 0)
      {
        ...
        if (! used[candidate_original_idx])
        {
          ...
          double potential_mz_min = (cluster_mz_min < candidate_point.mz ? cluster_mz_min : candidate_point.mz);
          double potential_mz_max = (cluster_mz_max > candidate_point.mz ? cluster_mz_max : candidate_point.mz);
          double potential_im_min = (cluster_im_min < candidate_point.im ? cluster_im_min : candidate_point.im);
          double potential_im_max = (cluster_im_max > candidate_point.im ? cluster_im_max : candidate_point.im);

          // Fixed m/z tolerance calculation with epsilon
          bool mz_ok = (potential_mz_max - potential_mz_min) <= (potential_mz_min * ppm_factor + 1e-9);
          bool im_ok = (potential_im_max - potential_im_min) <= im_tolerance_cluster_ + 1e-9;
      ...
      while (right_idx < static_cast<OpenMS::SignedSize>(points.size()))
      {
        ...
        if (! used[candidate_original_idx])
        {
          ...
          else
          {
            if (! mz_ok)
            {
              right_idx = static_cast<OpenMS::SignedSize>(points.size());
              break;
            }
            else { right_idx++; }
          }
    ...
    if (! current_cluster_indices.empty())
    {
      ...
      {
        averaged_points.emplace_back(sum_mz_intensity / sum_intensity, sum_im_intensity / sum_intensity, static_cast<float>(sum_intensity),
...
void PeakPickerIM::pickIMElutionProfiles(MSSpectrum& input) const
{
  ...
  // Validate IM format - returns false if we should skip processing
  if (! validateIMFormatForPicking(input)) { return; }
...
void PeakPickerIM::pickIMSage(MSSpectrum& spectrum) const
{
  if (! validateIMFormatForPicking(spectrum)) return;

  const auto& fda = spectrum.getFloatDataArrays();
  size_t im_data_index = 0;
  bool found = false;
  for (size_t i = 0; i < fda.size(); ++i)
  {
    if (fda[i].getName() == Constants::UserParam::ION_MOBILITY)
    {
      im_data_index = i;
      found = true;
      break;
  ...
  const auto& im_data = fda[im_data_index];
  std::vector<Point> points;
  points.reserve(spectrum.size());
  for (size_t i = 0; i < spectrum.size(); ++i)
  {
    points.emplace_back(spectrum[i].getMZ(), im_data[i], static_cast<float>(spectrum[i].getIntensity()), i);
  }
  ...
  std::sort(intensity_sorted_indices.begin(), intensity_sorted_indices.end(),
            [&](OpenMS::Size a, OpenMS::Size b) { return points[a].intensity > points[b].intensity; });

  std::vector<OpenMS::Size> original_to_sorted_pos(points.size());
  for (OpenMS::Size i = 0; i < mz_sorted_indices.size(); ++i)
  {
    original_to_sorted_pos[mz_sorted_indices[i]] = i;
  }
  ...
  std::vector<bool> used(points.size(), false);
  std::vector<Point> averaged_points;
  double ppm_factor = ppm_tolerance_sage_ * 1e-6;

  for (OpenMS::Size seed_original_idx : intensity_sorted_indices)
  {
    if (used[seed_original_idx]) continue;

    used[seed_original_idx] = true;
    const auto& seed = points[seed_original_idx];
    double seed_mz = seed.mz;
    double seed_im = seed.im;
    double total_intensity = seed.intensity;
    OpenMS::Size num_used = 1;

    OpenMS::Size seed_sorted_pos = original_to_sorted_pos[seed_original_idx];

    // Search left
    for (int i = static_cast<int>(seed_sorted_pos) - 1; i >= 0; --i)
    {
      OpenMS::Size candidate_idx = mz_sorted_indices[i];
      const auto& candidate = points[candidate_idx];
      if (std::fabs(seed_mz - candidate.mz) > (seed_mz * ppm_factor + 1e-9)) break;
      if (! used[candidate_idx] && std::fabs(seed_im - candidate.im) <= im_tolerance_sage_ + 1e-9)
      {
        total_intensity += candidate.intensity;
    ...
    // Search right
    for (OpenMS::Size i = seed_sorted_pos + 1; i < points.size(); ++i)
    {
      OpenMS::Size candidate_idx = mz_sorted_indices[i];
      const auto& candidate = points[candidate_idx];
      if (std::fabs(seed_mz - candidate.mz) > (seed_mz * ppm_factor + 1e-9)) break;
    ...
    if (total_intensity > 0.0) { averaged_points.emplace_back(seed_mz, seed_im, static_cast<float>(total_intensity), 0); }

    if (num_used == points.size()) break;
  }
  ...
  std::vector<float> new_im_data(averaged_points.size());
  for (size_t i = 0; i != averaged_points.size(); ++i)
  {
    const auto& p = averaged_points[i];
    spectrum[i].setMZ(p.mz);
  ...
  spectrum.updateRanges();
  spectrum.getFloatDataArrays()[im_data_index].setName(Constants::UserParam::ION_MOBILITY_CENTROID);
  spectrum.getFloatDataArrays()[im_data_index].assign(new_im_data.begin(), new_im_data.end());
  spectrum.setIMFormat(IMFormat::CENTROIDED);
  removeAllFloatDataArraysExcept(spectrum, Constants::UserParam::ION_MOBILITY_CENTROID);
}
