...
namespace OpenMS::Math
{
              ...
              for (PeptideIdentification pep : peptide_ids)
              {
                ...
                if (prot.getIdentifier() == pep.getIdentifier())
                {
                  ...
                  if (top_hits_only)
                  {
                    ...
                    {
                      double score = PosteriorErrorProbabilityModel::transformScore_(supported_engine, hits[0], score_type);
                  ...
                  else
                  {
                    for (PeptideHit const & hit : hits)
                    {
                      ...
                      {
                        double score = PosteriorErrorProbabilityModel::transformScore_(supported_engine, hit, score_type);
                        if (!std::isnan(score)) // issue #740: ignore scores with 0 values, otherwise you will get the error "unable to fit data"
                        {
                          scores.push_back(score);
                        }
      ...
      bool & data_might_not_be_well_fit)
    {
      ...
      // Extract score type once from the peptide identification list
      String id_score_type = peptide_ids.getScoreType();
            ...
            if (prot.getIdentifier() == pep.getIdentifier())
            {
              ...
              for (PeptideHit & hit : hits)
              {
                if (!split_charge || (hit.getCharge() == charge))
                {
                  ...
                  score = PosteriorErrorProbabilityModel::transformScore_(engine, hit, id_score_type);
