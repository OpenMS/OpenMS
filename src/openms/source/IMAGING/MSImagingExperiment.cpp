...
MSSpectrum& MSImagingExperiment::getSpectrum(UInt x, UInt y)
{
  ...
  {
    throw Exception::InvalidValue(__FILE__, __LINE__, OPENMS_PRETTY_FUNCTION, "Pixel references missing spectrum", String(idx));
  }
...
IonImage MSImagingExperiment::extractIonImage(double mz, double tolerance_ppm) const
{
  ...
  {
    throw Exception::InvalidValue(__FILE__, __LINE__, OPENMS_PRETTY_FUNCTION, "mz and tolerance_ppm must be finite and non-negative",
...
void MSImagingExperiment::validate() const
{
  ...
  for (const auto& p : geometry_.getPixels())
  {
    ...
    {
      throw Exception::InvalidValue(__FILE__, __LINE__, OPENMS_PRETTY_FUNCTION, "Pixel references missing spectrum", String(p.spectrum_index));
    }
  }
}
...
void MSImagingExperiment::extractIntoImage_(IonImage& image,
                                            double mz_lo,
                                            double mz_hi,
