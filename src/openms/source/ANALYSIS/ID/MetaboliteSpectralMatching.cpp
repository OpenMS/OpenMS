...
MetaboliteSpectralMatching::MetaboliteSpectralMatching(): DefaultParamHandler("MetaboliteSpectralMatching"), ProgressLogger()
{
...
double MetaboliteSpectralMatching::computeHyperScore(double fragment_mass_error,
                                                     bool fragment_mass_tolerance_unit_ppm,
                                                     const MSSpectrum& exp_spectrum,
                                                     const MSSpectrum& db_spectrum,
                                                     double mz_lower_bound)
{
  return computeHyperScore_(fragment_mass_error, fragment_mass_tolerance_unit_ppm, exp_spectrum, db_spectrum, nullptr, mz_lower_bound);
}
...
double MetaboliteSpectralMatching::computeHyperScore(double fragment_mass_error,
                                                     ...
                                                     double mz_lower_bound)
{
  return computeHyperScore_(fragment_mass_error, fragment_mass_tolerance_unit_ppm, exp_spectrum, db_spectrum, &annotations, mz_lower_bound);
}


double MetaboliteSpectralMatching::computeHyperScore_(double fragment_mass_error,
                                                      bool fragment_mass_tolerance_unit_ppm,
                                                      const MSSpectrum& exp_spectrum,
                                                      const MSSpectrum& db_spectrum,
                                                      vector<PeptideHit::PeakAnnotation>* annotations,
                                                      double mz_lower_bound)
{
  ...
  if (fragment_mass_tolerance_unit_ppm) { mz_offset = min_exp_mz * fragment_mass_error * 1e-6; }
  mz_lower_bound = max(mz_lower_bound, min_exp_mz - mz_offset);
  double max_exp_mz = exp_spectrum.back().getMZ(); // highest experimental m/z
  if (fragment_mass_tolerance_unit_ppm) { mz_offset = max_exp_mz * fragment_mass_error * 1e-6; }
  ...
  for (auto db_it = db_spectrum.MZBegin(mz_lower_bound); db_it != db_spectrum.MZEnd(mz_upper_bound); ++db_it)
  {
    double db_mz = db_it->getMZ();

    if (fragment_mass_tolerance_unit_ppm) { mz_offset = db_mz * fragment_mass_error * 1e-6; }
  ...
  if ((annotations != nullptr) && ! db_spectrum.getStringDataArrays().empty() && ! db_spectrum.getIntegerDataArrays().empty())
  {
    for (const auto& match : peak_matches)
    {
  ...
  if (matched_ions_count < 3) { return matched_ions_term; }
  ...
  else { matched_ions_term = log(boost::math::factorial<double>(boost::math::max_factorial<double>::value)); }
...
void MetaboliteSpectralMatching::run(PeakMap& msexp, PeakMap& spec_db, MzTab& mztab_out, String& out_spectra)
{
  ...
  if (! out_spectra.empty()) { FileHandler().storeExperiment(out_spectra, msexp, {FileTypes::MZML}); }
  ...
  for (Size spec_idx = 0; spec_idx < msexp.size(); ++spec_idx)
  {
    ...
    for (Size prec_idx = 0; prec_idx < msexp[spec_idx].getPrecursors().size(); ++prec_idx)
    {
      ...
      {
        String id_to_log = msexp[spec_idx].metaValueExists("GNPS_Spectrum_ID") ? msexp[spec_idx].getMetaValue("GNPS_Spectrum_ID").toString()
      ...
      for (Size search_idx = start_idx; search_idx < end_idx; ++search_idx)
      {
        ...
        {
          metabolite_name = spec_db[search_idx].getMetaValue(Constants::UserParam::MSM_METABOLITE_NAME).toString();
        }
        ...
        if ((ion_mode_ == "positive" && spec_db[search_idx].getPrecursors()[0].getCharge() < 0)
            || (ion_mode_ == "negative" && spec_db[search_idx].getPrecursors()[0].getCharge() > 0))
        {
          continue;
        }
        ...
        if (hyperscore > 0)
        {
          ...
          else { primary_id_value = spec_db[search_idx].getNativeID(); }
      ...
      if (report_mode_ == "top3")
      {
        ...
        {
          OPENMS_LOG_DEBUG << "score: " << partial_results[result_idx].getMatchingScore() << " "
                           << partial_results[result_idx].getMatchingSpectrumIndex() << endl;
      ...
      {
        if (! partial_results.empty()) { matching_results.push_back(partial_results[0]); }
      }
...
void MetaboliteSpectralMatching::exportMzTab_(const vector<SpectralMatch>& overall_results, MzTab& mztab_out)
{
  ...
  for (Size id_idx = 0; id_idx < overall_results.size(); ++id_idx)
  {
    ...
    mztab_row_record.exp_mass_to_charge = mass_to_charge; // TODO: distinguish the experimental precursor mass and spectral library precursor mass
                                                          // (later should probably go into cv_opt_ column)
    ...
    // set smallmolecule_abundance_stdev_sub; not applicable for a single feature intensity, however must be filled. Otherwise, the mzTab export
    // fails.
    ...
    // set smallmolecule_abundance_std_error_sub; not applicable for a single feature intensity, however must be filled. Otherwise, the mzTab export
    // fails.
