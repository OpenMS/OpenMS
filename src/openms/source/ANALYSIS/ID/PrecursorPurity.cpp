...
namespace OpenMS
{
  ...
  std::map<std::string, PrecursorPurity::PurityScores> PrecursorPurity::computePrecursorPurities(const PeakMap& spectra, double precursor_mass_tolerance, bool precursor_mass_tolerance_unit_ppm, bool ignore_missing_precursor_spectra)
  {
    std::map<std::string, PrecursorPurity::PurityScores> purityscores;
    std::pair<std::map<std::string, PrecursorPurity::PurityScores>::iterator, bool> insert_return_value;
    ...
    for (int i = 0; i < spectra_size; ++i)
    {
      if (spectra[i].getMSLevel() == 2)
      {
        ...
        if (parent_spectrum_it == spectra.end() && !ignore_missing_precursor_spectra)
        {
          ...
          return std::map<std::string, PrecursorPurity::PurityScores>();
        }
