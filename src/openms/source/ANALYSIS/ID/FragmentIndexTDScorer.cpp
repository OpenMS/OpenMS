...
namespace OpenMS
{
  ...
  void FragmentIndexTDScorer::setDB(FragmentIndex* db)
  {
    db_ = db;
  }
  ...
  void FragmentIndexTDScorer::simpleScoring(MSSpectrum& specturm, InitHits& initHits)
  {
    ...
    if (specturm.empty() || (specturm.getMSLevel() != 2))
      return;
  ...
  void FragmentIndexTDScorer::peakQuery(InitHits& candidates, const OpenMS::Peak1D& peak, pair<size_t, size_t> candidates_range, int16_t isotope_error, uint16_t precursor_charge)
  {
  ...
  void FragmentIndexTDScorer::closedSearch(MSSpectrum& spectrum, double mz, InitHits& initHits, uint16_t charge)
  {
    for (int16_t isotope_error = min_isotope_error_; isotope_error <= max_isotope_error_; isotope_error++)
    {
      ...
      {
        peakQuery(candidates_iso_error, peak, candidates_range, isotope_error, charge);
      }
  ...
  void FragmentIndexTDScorer::openSearch(MSSpectrum& spectrum, double precursor_mass, InitHits& initHits, uint16_t charge)
  {
    ...
    {
      peakQuery(hits_per_window, *peak, candidates_range, 0, charge);
    }
    trimHits(hits_per_window);
    initHits += hits_per_window;
  }

  void FragmentIndexTDScorer::extractHits(OpenMS::FragmentIndexTDScorer::InitHits& candidates, const vector<FragmentIndex::Hit>& hits, uint32_t charge, int16_t isotope_error,
  ...
  FragmentIndexTDScorer::FragmentIndexTDScorer() : DefaultParamHandler("FragmentIndexTDScorer")
  {
    ...
    defaults_.setValue("max_processed_hits", 10, "The number of initial hits for which we calculate a score");
  ...
  const FragmentIndex* FragmentIndexTDScorer::getDb() const
  {
    return db_;
  }
} // namespace OpenMS