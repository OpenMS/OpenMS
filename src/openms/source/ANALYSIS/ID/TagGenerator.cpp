...
namespace OpenMS
{
  ...
  void TagGenerator::localSelection()
  {
    size_t lower_peak = 0;
    size_t upper_peak = 0;
    for(double window_da = 0; window_da < spectrum_.getPrecursors()[0].getUnchargedMass(); window_da += 35){ //window increment = 35 Da
  ...
  void TagGenerator::generateDirectedAcyclicGraph(double fragment_tolerance)
  {
    ...
    for(size_t i = selected_peaks_.size(); i > 0; i--){
      if(selected_peaks_[i-1]){
        TagGeneratorNode newNode{spectrum_[i-1]};
        newNode.calculateConfidence(spectrum_, max);
        for(auto dag_iter = dag_.end() - 1; dag_iter != dag_.begin()-1 && dag_iter != dag_.end() - 11; dag_iter--){  // only look at the 5 peak to the right
          newNode.generateConnection(*dag_iter, fragment_tolerance);
        }
        dag_.push_back(std::move(newNode));

      }
    }
  }

  void TagGenerator::generateAllQuadPeaks(std::vector<TagGeneratorNode::QuadPeak>& quad_peaks)
  {
    for(TagGeneratorNode node: dag_){
      vector<TagGeneratorNode::QuadPeak> quad_peaks_per_node;
      node.generateAllQuadPeaks(quad_peaks_per_node, 2);
