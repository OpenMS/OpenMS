...
namespace OpenMS
{
          ...
          for (int d = 1; d >= -1; d -= 2)
          {
            ...
            for (int t = -1; t < 2; t++)
            {
              ...
              if (std::abs(nibt - (int)mz_bin_index) >= tol_div_factor && nibt > 0 && nibt < (int)mz_bins_.size() && mz_bins_[nibt])
              {
                ...
                if (next_iso_bin == 0 || mz_intensities[next_iso_bin] < mz_intensities[nibt])
                {
                  next_iso_bin = nibt;
                }
              }
  ...
  void SpectralDeconvolution::generatePeakGroupsFromSpectrum_()
  {
    ...
    {
      deconvolved_spectrum_ = *target_dspec_for_decoy_calculation_;
    }
...
#pragma omp parallel for default(none) shared(tol, selected)
    for (int i = 0; i < (int)deconvolved_spectrum_.size(); i++)
    {
      ...
      float cos = getIsotopeCosineAndIsoOffset(peak_group.getMonoMass(), peak_group.getIsotopeIntensities(), offset, avg_, -peak_group.getMinNegativeIsotopeIndex(), window_width, allowed_iso_error_,
                                               target_decoy_type_);
      ...
      if (!mass_determined || peak_group.empty() || peak_group.getQscore() <= 0 || peak_group.getMonoMass() < current_min_mass_ || peak_group.getMonoMass() > current_max_mass_)
        continue;
...
#pragma omp critical
    ...
    filtered_peak_groups.reserve(selected_count + (target_decoy_type_ != PeakGroup::TargetDecoyType::charge_decoy ? 0 : target_dspec_for_decoy_calculation_->size()));
...
#pragma omp parallel for default(none) shared(filtered_peak_groups, tol, selected, harmonic_charges_)
    for (int i = 0; i < deconvolved_spectrum_.size(); i++)
    {
      ...
      for (auto hz : harmonic_charges_)
      {
        ...
        pg.updateQscore(nps, deconvolved_spectrum_.getOriginalSpectrum(), avg_, min_isotope_cosine_[ms_level_ - 1], tol, (z1 + z2) * hz < 2 * low_charge_, allowed_iso_error_, true);
...
#pragma omp critical
                                                            ...
                                                            int window_width, int allowed_isotope_error, PeakGroup::TargetDecoyType target_decoy_type)
  {
    ...
    int left = right;
    if (target_decoy_type == PeakGroup::TargetDecoyType::isotope_decoy && allowed_isotope_error >= 0 && right < allowed_isotope_error)
      return 0;
    ...
    std::sort(offset_cos.begin(), offset_cos.end(), [](const std::pair<int, float>& p1, const std::pair<int, float>& p2) { return p1.second > p2.second; });
  ...
  float SpectralDeconvolution::getCosine(const std::vector<float>& a, int a_start, int a_end, const IsotopeDistribution& b, int offset, int min_iso_len, bool decoy)
  {
    ...
    for (int j = a_start; j < a_end; j++)
    {
      ...
      else if (i >= b.size() || i < 0 || b[i].getIntensity() <= 0)
      {
        continue;
      }
      else
    ...
    for (const auto& e : peak_to_pgs)
    {
      ...
      for (auto i : pg_is)
      {
        ...
        for (auto j : pg_is)
        {
          ...
          if (dspec[i].getChargeSNR(repz1) > dspec[j].getChargeSNR(repz2))
          {
            continue;
          }
  ...
  void SpectralDeconvolution::removeOverlappingPeakGroups(DeconvolvedSpectrum& dspec, double tol, PeakGroup::TargetDecoyType target_decoy_type)
  {
    ...
    for (Size i = 0; i < dspec.size(); i++)
    {
      ...
      if (mass - start_mass > mass * tol * 1.5)
      {
        ...
        {
          if ((target_decoy_type == PeakGroup::TargetDecoyType::non_specific || dspec[local_max_index].getTargetDecoyType() == target_decoy_type) && last_local_max_index != local_max_index)
