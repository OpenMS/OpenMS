...
namespace OpenMS
{
  FLASHIdentAlgorithm::FLASHIdentAlgorithm() :
    DefaultParamHandler("FLASHIdentAlgorithm"),
    ProgressLogger()
  {
  ...
  void FLASHIdentAlgorithm::updateMembers_()
  {
  ...
  // static
  void FLASHIdentAlgorithm::preprocessSpectra_(PeakMap& exp, double fragment_mass_tolerance, bool fragment_mass_tolerance_unit_ppm)
  {
    // filter MS2 map
...
void FLASHIdentAlgorithm::postProcessHits_(const PeakMap& exp, 
      std::vector<std::vector<FLASHIdentAlgorithm::AnnotatedHit_> >& annotated_hits, 
...
#pragma omp critical (peptide_ids_access)
  ...
  FLASHIdentAlgorithm::ExitCodes FLASHIdentAlgorithm::search(const String& in_mzML, const String& in_db, vector<ProteinIdentification>& protein_ids, vector<PeptideIdentification>& peptide_ids) const
  {
...
#pragma omp parallel for schedule(static) default(none) shared(annotated_hits, spectrum_generator, multimap_mass_2_scan_index, fixed_modifications, variable_modifications, fasta_db, digestor, processed_petides, count_proteins, count_peptides, precursor_mass_tolerance_unit_ppm, fragment_mass_tolerance_unit_ppm, peptide_motif_regex, spectra, annotated_hits_lock)
    ...
    FLASHIdentAlgorithm::postProcessHits_(spectra, 
      annotated_hits, 
      protein_ids, 
      peptide_ids, 
      report_top_hits_,
