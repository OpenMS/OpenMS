...
namespace OpenMS
{
  ...
  FLASHDeconvAlgorithm::FLASHDeconvAlgorithm() : DefaultParamHandler("FLASHDeconvAlgorithm"), ProgressLogger()
  {
    ...
    defaults_.insert("deconv:", SpectralDeconvolution().getDefaults());
    ...
    mf_defaults.setValue("mass_error_ppm", -1.0, "Feature tracing mass ppm tolerance. When negative, MS1 tolerance for mass deconvolution will be used (e.g., 16 ppm is used when -deconv:tol 16).");
    ...
    defaults_.insert("isoquant:", TopDownIsobaricQuantifier().getDefaults());
    defaultsToParam_();
  }
  ...
  void FLASHDeconvAlgorithm::updateMembers_()
  {
    ...
    tols_ = param_.getValue("deconv:tol");
    min_cos_ = param_.getValue("deconv:min_cos");
...
#pragma omp section
  ...
  void FLASHDeconvAlgorithm::run(MSExperiment& map, std::vector<DeconvolvedSpectrum>& deconvolved_spectra, std::vector<FLASHDeconvHelperStructs::MassFeature>& deconvolved_features)
  {
    ...
    Param quant_param = param_.copy("isoquant:", true);
