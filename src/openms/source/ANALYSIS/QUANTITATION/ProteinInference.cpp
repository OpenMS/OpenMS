    ...
    for (size_t i = 0; i < protein_ident.getHits().size(); ++i)
    {
      ...
      for (size_t i_cm = 0; i_cm < consensus_map.size(); ++i_cm)
      {
             ...
             ++it_pepid)
        {
          ...
          if (sortByUnique_(peptide_hits_local, consensus_map[i_cm].getPeptideIdentifications().isHigherScoreBetter())) // we found a unique peptide
          {
        ...
        {
          if (sortByUnique_(peptide_hits, consensus_map[i_cm].getPeptideIdentifications().isHigherScoreBetter())) //found a unique peptide for current ConsensusElement
          {
