    ...
    for (Size i = 0; i < transition_exp.getTransitions().size(); i++)
    {
      ...
      if (chromatogram_map.find(transition->getNativeID()) == chromatogram_map.end())
      {
        OPENMS_LOG_DEBUG << "Error: Transition " + transition->getNativeID() + " from group " +
          transition->getTransGroupRef() + " does not have a corresponding chromatogram" << std::endl;
        if (strict_)
        {
          throw Exception::IllegalArgument(__FILE__, __LINE__, OPENMS_PRETTY_FUNCTION,
                                           "Error: Transition " + transition->getNativeID() + " from group " +
                                           transition->getTransGroupRef() + " does not have a corresponding chromatogram");
        }
        continue;
      }
      ...
      {
        expected_rt = PeptideRefMap_[transition->getTransGroupRef()]->rt;
      ...
      if (transition_group_map.find(transition->getTransGroupRef()) == transition_group_map.end())
      {
        MRMTransitionGroupType transition_group;
        transition_group.setTransitionGroupID(transition->getTransGroupRef());
        transition_group_map[transition->getTransGroupRef()] = transition_group;
      }

      // Now add the transition and the chromatogram to the group
      MRMTransitionGroupType& transition_group = transition_group_map[transition->getTransGroupRef()];
