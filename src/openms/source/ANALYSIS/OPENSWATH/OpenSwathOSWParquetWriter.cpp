...
namespace
{
  ...
  {
    if (! has_value || ! std::isfinite(value)) { ParquetFile::appendOrThrow(builder.AppendNull(), column); }
    else { ParquetFile::appendOrThrow(builder.Append(value), column); }
  }

  void appendOptionalInt_(arrow::Int64Builder& builder, bool has_value, int64_t value, const char* column)
  {
    if (! has_value) { ParquetFile::appendOrThrow(builder.AppendNull(), column); }
    else { ParquetFile::appendOrThrow(builder.Append(value), column); }
  }
  ...
  {
    double value = 0.0;
    if (extractMetaDouble_(feature, key, value)) { appendOptionalFloat_(builder, true, value, column); }
    else if (use_fallback && std::isfinite(fallback_value)) { appendOptionalFloat_(builder, true, fallback_value, column); }
  ...
  std::vector<String> getSeparateScore_(const Feature& feature, const std::string& score_name)
  {
    ...
    if (! feature.getMetaValue(score_name).isEmpty())
    {
      ...
      else if (feature.getMetaValue(score_name).valueType() == DataValue::INT_LIST)
      {
        ...
        for (int score : int_scores)
          separated_scores.emplace_back(score);
      }
      else if (feature.getMetaValue(score_name).valueType() == DataValue::DOUBLE_LIST)
      {
        ...
        for (double score : double_scores)
          separated_scores.emplace_back(score);
      }
      else { separated_scores.push_back(feature.getMetaValue(score_name).toString()); }
    }
  ...
  void appendOptionalFloatFromList_(arrow::DoubleBuilder& builder, const std::vector<String>& values, Size index, const char* column)
  {
    double value = 0.0;
    if (index < values.size() && parseOptionalDouble_(values[index], value)) { appendOptionalFloat_(builder, true, value, column); }
    else { ParquetFile::appendOrThrow(builder.AppendNull(), column); }
  }
  ...
  std::vector<RunEntry> readRuns_(const String& runs_parquet)
  {
    ...
    if (! File::exists(runs_parquet)) { return runs; }
  ...
  void
  writeMetadata_(const String& base_dir, const std::vector<RunEntry>& runs, const std::vector<RunCounts>& run_counts, const RunCounts& total_counts)
  {
    const String metadata_path = base_dir + "/metadata.json";
    std::ofstream out(metadata_path.c_str(), std::ios::out | std::ios::trunc);
    if (! out.is_open()) { throw Exception::FileNotWritable(__FILE__, __LINE__, OPENMS_PRETTY_FUNCTION, metadata_path); }
    ...
    {
      throw Exception::InvalidValue(__FILE__, __LINE__, OPENMS_PRETTY_FUNCTION, "Run count metadata mismatch", String(run_counts.size()));
    }
    ...
    for (Size i = 0; i < runs.size(); ++i)
    {
      ...
      out << "    {\"id\": " << run.run_id << ", \"filename\": \"" << jsonEscape_(run.filename) << "\""
          << ", \"counts\": {\"features\": " << counts.features << ", \"feature_precursor\": " << counts.feature_precursor
...
{
#ifndef WITH_PARQUET
  throw Exception::MissingDependency(__FILE__, __LINE__, OPENMS_PRETTY_FUNCTION, "OpenMS was built without Parquet support");
#else
  ...
  if (! File::exists(base_dir)) { File::makeDir(base_dir); }
  if (! File::exists(runs_dir)) { File::makeDir(runs_dir); }
  ...
  if (library_ready)
  {
    ...
    for (const auto& c : assay_library.compounds)
      unique_compounds.insert(c.id);
    ...
    for (const auto& t : assay_library.transitions)
      unique_transitions.insert(t.transition_name);
    ...
    if (existing_precursors != expected_precursors || existing_transitions != expected_transitions)
    {
      throw Exception::InvalidValue(
        ...
        "Existing library at '" + library_dir
          + "' appears incompatible with provided assay_library. Please rebuild the library or use a different output path.",
  ...
  if (! library_ready)
  {
    ...
    if (File::exists(library_tmp_dir)) { File::removeDirRecursively(library_tmp_dir); }
    if (File::exists(library_dir)) { File::removeDirRecursively(library_dir); }
  ...
  {
    throw Exception::InvalidValue(__FILE__, __LINE__, OPENMS_PRETTY_FUNCTION, "Run output directory already exists", run_path);
  }
  ...
  {
    if (compound_to_precursor.find(compound.id) != compound_to_precursor.end()) { continue; }
    ...
    if (parsed_numeric)
    {
      ...
      if (used_precursor_ids.find(precursor_id) != used_precursor_ids.end() || precursor_id <= 0) { precursor_id = next_precursor_id++; }
      else
      {
  ...
  for (Size idx = 0; idx < feature_map.size(); ++idx)
  {
    ...
    {
      throw Exception::MissingInformation(__FILE__, __LINE__, OPENMS_PRETTY_FUNCTION, "Feature missing PeptideRef meta value");
    }
    ...
    {
      throw Exception::InvalidValue(__FILE__, __LINE__, OPENMS_PRETTY_FUNCTION, "Feature references unknown peptide", peptide_ref);
    }
    ...
    if (has_ms1)
    {
      ...
      appendOptionalFloat_(var_ms1_im_ms1_delta_builder, extractMetaDouble_(feature, "var_im_ms1_delta_score", value), value,
      ...
      appendOptionalFloat_(var_ms1_mi_contrast_builder, extractMetaDouble_(feature, "var_ms1_mi_contrast_score", value), value,
                           "var_ms1_mi_contrast_score");
      appendOptionalFloat_(var_ms1_mi_combined_builder, extractMetaDouble_(feature, "var_ms1_mi_combined_score", value), value,
                           "var_ms1_mi_combined_score");
      appendOptionalFloat_(var_ms1_iso_corr_builder, extractMetaDouble_(feature, "var_ms1_isotope_correlation", value), value,
                           "var_ms1_isotope_correlation_score");
      appendOptionalFloat_(var_ms1_iso_overlap_builder, extractMetaDouble_(feature, "var_ms1_isotope_overlap", value), value,
                           "var_ms1_isotope_overlap_score");
      appendOptionalFloat_(var_ms1_xcorr_coelution_builder, extractMetaDouble_(feature, "var_ms1_xcorr_coelution", value), value,
                           "var_ms1_xcorr_coelution");
      appendOptionalFloat_(var_ms1_xcorr_coelution_contrast_builder, extractMetaDouble_(feature, "var_ms1_xcorr_coelution_contrast", value), value,
                           "var_ms1_xcorr_coelution_contrast");
      appendOptionalFloat_(var_ms1_xcorr_coelution_combined_builder, extractMetaDouble_(feature, "var_ms1_xcorr_coelution_combined", value), value,
      ...
      appendOptionalFloat_(var_ms1_xcorr_shape_contrast_builder, extractMetaDouble_(feature, "var_ms1_xcorr_shape_contrast", value), value,
                           "var_ms1_xcorr_shape_contrast");
      appendOptionalFloat_(var_ms1_xcorr_shape_combined_builder, extractMetaDouble_(feature, "var_ms1_xcorr_shape_combined", value), value,
    ...
    appendOptionalFloat_(var_ms2_iso_corr_builder, extractMetaDouble_(feature, "var_isotope_correlation_score", value), value,
                         "var_ms2_isotope_correlation_score");
    appendOptionalFloat_(var_ms2_iso_overlap_builder, extractMetaDouble_(feature, "var_isotope_overlap_score", value), value,
    ...
    appendOptionalFloat_(var_ms2_library_dotprod_builder, extractMetaDouble_(feature, "var_library_dotprod", value), value,
                         "var_ms2_library_dotprod");
    appendOptionalFloat_(var_ms2_library_manhattan_builder, extractMetaDouble_(feature, "var_library_manhattan", value), value,
    ...
    appendOptionalFloat_(var_ms2_library_rootmeansquare_builder, extractMetaDouble_(feature, "var_library_rootmeansquare", value), value,
    ...
    appendOptionalFloat_(var_ms2_massdev_weighted_builder, extractMetaDouble_(feature, "var_massdev_score_weighted", value), value,
    ...
    appendOptionalFloat_(var_ms2_mi_weighted_builder, extractMetaDouble_(feature, "var_mi_weighted_score", value), value,
    ...
    appendOptionalFloat_(var_ms2_xcorr_coelution_builder, extractMetaDouble_(feature, "var_xcorr_coelution", value), value,
                         "var_ms2_xcorr_coelution");
    appendOptionalFloat_(var_ms2_xcorr_coelution_weighted_builder, extractMetaDouble_(feature, "var_xcorr_coelution_weighted", value), value,
    ...
    appendOptionalFloat_(var_ms2_xcorr_shape_weighted_builder, extractMetaDouble_(feature, "var_xcorr_shape_weighted", value), value,
    ...
    appendOptionalFloat_(var_ms2_elution_model_fit_builder, extractMetaDouble_(feature, "var_elution_model_fit_score", value), value,
    ...
    appendOptionalFloat_(var_ms2_im_xcorr_coelution_builder, extractMetaDouble_(feature, "var_im_xcorr_coelution", value), value,
    ...
    for (Size i = 0; i < subordinates.size(); ++i)
    {
      ...
      {
        if (! sub_it.metaValueExists("native_id")) { continue; }
        ...
        if (it != transition_to_id.end()) { transition_id_value = it->second; }
        else if (parseOptionalInt64_(native_id, transition_id_value))
        {
          ...
          {
            throw Exception::InvalidValue(__FILE__, __LINE__, OPENMS_PRETTY_FUNCTION, "Transition references unknown numeric id", native_id);
          }
        }
        else { throw Exception::InvalidValue(__FILE__, __LINE__, OPENMS_PRETTY_FUNCTION, "Transition references unknown id", native_id); }
        ...
        appendOptionalFloat_(ft_start_position_at_5_builder, has_peak_shape && extractMetaDouble_(sub_it, "start_position_at_5", value), value,
                             "start_position_at_5");
        appendOptionalFloat_(ft_end_position_at_5_builder, has_peak_shape && extractMetaDouble_(sub_it, "end_position_at_5", value), value,
                             "end_position_at_5");
        appendOptionalFloat_(ft_start_position_at_10_builder, has_peak_shape && extractMetaDouble_(sub_it, "start_position_at_10", value), value,
                             "start_position_at_10");
        appendOptionalFloat_(ft_end_position_at_10_builder, has_peak_shape && extractMetaDouble_(sub_it, "end_position_at_10", value), value,
                             "end_position_at_10");
        appendOptionalFloat_(ft_start_position_at_50_builder, has_peak_shape && extractMetaDouble_(sub_it, "start_position_at_50", value), value,
                             "start_position_at_50");
        appendOptionalFloat_(ft_end_position_at_50_builder, has_peak_shape && extractMetaDouble_(sub_it, "end_position_at_50", value), value,
                             "end_position_at_50");
        appendOptionalFloat_(ft_total_width_builder, has_peak_shape && extractMetaDouble_(sub_it, "total_width", value), value, "total_width");
        appendOptionalFloat_(ft_tailing_factor_builder, has_peak_shape && extractMetaDouble_(sub_it, "tailing_factor", value), value,
                             "tailing_factor");
        appendOptionalFloat_(ft_asymmetry_factor_builder, has_peak_shape && extractMetaDouble_(sub_it, "asymmetry_factor", value), value,
                             "asymmetry_factor");
        appendOptionalFloat_(ft_slope_of_baseline_builder, has_peak_shape && extractMetaDouble_(sub_it, "slope_of_baseline", value), value,
                             "slope_of_baseline");
        appendOptionalFloat_(ft_baseline_delta_2_height_builder, has_peak_shape && extractMetaDouble_(sub_it, "baseline_delta_2_height", value),
                             value, "baseline_delta_2_height");
        appendOptionalFloat_(ft_points_across_baseline_builder, has_peak_shape && extractMetaDouble_(sub_it, "points_across_baseline", value), value,
                             "points_across_baseline");
        appendOptionalFloat_(ft_points_across_half_height_builder, has_peak_shape && extractMetaDouble_(sub_it, "points_across_half_height", value),
                             value, "points_across_half_height");
      }
      else if (sub_it.metaValueExists("FeatureLevel") && sub_it.getMetaValue("FeatureLevel") == "MS1" && sub_it.getIntensity() > 0.0)
      {
        ...
        if (precursor_id.size() < 2) { continue; }
        int64_t isotope_value = 0;
        if (! parseOptionalInt64_(precursor_id[1], isotope_value)) { continue; }
    ...
    if (enable_uis_scoring)
    {
      ...
      for (Size i = 0; i < id_target_transition_names.size(); ++i)
      {
        ...
        appendOptionalFloatFromList_(ft_var_im_xcorr_coelution_contrast_builder, id_target_im_contrast_coelution, i,
        ...
        appendOptionalFloatFromList_(ft_var_im_xcorr_coelution_combined_builder, id_target_im_sum_contrast_coelution, i,
      ...
      for (Size i = 0; i < id_decoy_transition_names.size(); ++i)
      {
        ...
        appendOptionalFloatFromList_(ft_var_im_xcorr_coelution_contrast_builder, id_decoy_im_contrast_coelution, i,
                                     "var_im_xcorr_coelution_contrast");
        ...
        appendOptionalFloatFromList_(ft_var_im_xcorr_coelution_combined_builder, id_decoy_im_sum_contrast_coelution, i,
                                     "var_im_xcorr_coelution_combined");
  ...
  {
    auto features_schema = arrow::schema({arrow::field("feature_id", arrow::int64()),
    ...
    auto features_table
      = arrow::Table::Make(features_schema, {ParquetFile::finishArray(feature_id_builder, "feature_id"),
    ...
    write_tasks.emplace_back(
      std::async(std::launch::async, [features_table, run_path]() { ParquetFile::writeTable(features_table, run_path + "/features.parquet"); }));
  }

  {
    auto ft_schema = arrow::schema({arrow::field("feature_id", arrow::int64()),
                                    arrow::field("run_id", arrow::int64()),
    ...
    auto ft_table
      = arrow::Table::Make(ft_schema, {ParquetFile::finishArray(ft_feature_id_builder, "feature_id"),
    ...
    write_tasks.emplace_back(
      std::async(std::launch::async, [ft_table, run_path]() { ParquetFile::writeTable(ft_table, run_path + "/feature_transition.parquet"); }));
  }
  ...
  {
    auto fp_schema = arrow::schema({arrow::field("feature_id", arrow::int64()), arrow::field("run_id", arrow::int64()),
                                    arrow::field("precursor_isotope", arrow::int32()), arrow::field("precursor_area_intensity", arrow::float64()),
                                    arrow::field("precursor_apex_intensity", arrow::float64())});
    auto fp_table = arrow::Table::Make(fp_schema, {ParquetFile::finishArray(fp_feature_id_builder, "feature_id"),
    ...
    write_tasks.emplace_back(
      std::async(std::launch::async, [fp_table, run_path]() { ParquetFile::writeTable(fp_table, run_path + "/feature_precursor.parquet"); }));
  }
  ...
  catch (...)
  {
    ...
    if (File::exists(run_path)) { File::removeDirRecursively(run_path); }
    throw;
  }
  ...
  // partially-written run directory.
  {
    ...
    auto runs_schema = arrow::schema({arrow::field("run_id", arrow::int64()), arrow::field("filename", arrow::utf8())});
    auto runs_table
      = arrow::Table::Make(runs_schema, {ParquetFile::finishArray(run_id_builder, "run_id"), ParquetFile::finishArray(filename_builder, "filename")});
    ...
    if (File::exists(runs_parquet))
    {
      ...
      for (int64_t row = 0; row < existing_rows; ++row)
      {
        ...
        {
          throw Exception::InvalidValue(__FILE__, __LINE__, OPENMS_PRETTY_FUNCTION, "Run id already present in runs.parquet", String(run_id_clean));
        }
      }
      auto combined_result = arrow::ConcatenateTables({existing_table, runs_table});
      if (! combined_result.ok())
      {
        throw Exception::InvalidValue(__FILE__, __LINE__, OPENMS_PRETTY_FUNCTION, "Failed to append runs table", combined_result.status().ToString());
      }
      ParquetFile::writeTable(combined_result.ValueOrDie(), runs_parquet);
    }
    else { ParquetFile::writeTable(runs_table, runs_parquet); }
  }
  const String runs_parquet = runs_dir + "/runs.parquet";
  ...
  if (! output_is_dir)
  {
    ...
    if (File::exists(output_zip_abs) && ! preserve_existing_) { File::remove(output_zip_abs); }
