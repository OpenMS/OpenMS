...
namespace OpenMS
{
  ...
  void MRMFeatureFilter::calculateFilterValuesPercRSD(MRMFeatureQC& filter_rsd, const MRMFeatureQC& filter_mean, const MRMFeatureQC& filter_var) const
  {
    ...
    for (size_t cg_qc_it = 0; cg_qc_it < filter_rsd.component_group_qcs.size(); ++cg_qc_it) {
      ...
      filter_rsd.component_group_qcs.at(cg_qc_it).retention_time_u = (filter_mean.component_group_qcs.at(cg_qc_it).retention_time_u != 0) ? std::sqrt(filter_var.component_group_qcs.at(cg_qc_it).retention_time_u) / filter_mean.component_group_qcs.at(cg_qc_it).retention_time_u * 100 : 0;
