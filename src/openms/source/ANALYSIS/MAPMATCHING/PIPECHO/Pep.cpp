...
namespace PipEcho {
  ...
  public:
     ...
     */
    predictors_t(double cutoff, std::function<double(const Pep::acceptor_t&)> get);
  ...
  Pep::Pep(const std::vector<Acceptor*> acceptors) {
    auto push_acceptor_t =
      [](
         Acceptor* acceptor,
         std::vector<Acceptor::scored_t> &src,
         bool is_target,
         group_t &dest
         ) -> void
      {
  ...
  bool Pep::run() {
    ...
    if (acceptors.size() > MIN_TOTAL_ACCEPTORS && decoy_count > MIN_DECOYS) {
      std::vector<group_t> groups(CROSS_VALIDATION_GROUPS);
      ...
      for (std::size_t i{}; i<(TRAINING_ROUNDS - 1); ++i) {
        if (!round(groups, &acceptor_t::pep_score)) {
          return false;
        }
      }
  ...
  void Pep::group_acceptors(std::vector<group_t>& groups) {
    ...
    std::vector<group_t> target_groups(CROSS_VALIDATION_GROUPS);

    for (std::size_t i=0; i<CROSS_VALIDATION_GROUPS; ++i) {
      group_t group(targets_per_group);
    ...
    for (std::size_t i : vw::iota(std::size_t{}, CROSS_VALIDATION_GROUPS)) {
      group_t group(decoys_per_group + targets_per_group);
                  ...
                  std::function<double(const acceptor_t&)> getter)
  {
    for (std::size_t i : std::views::iota(std::size_t{}, groups.size())) {
      ...
      if (!train_predict(others, group, cutoff, getter)) {
        return false;
      }
    }
  ...
  bool predictors_t::predict(Pep::group_t& group) {
    ...
    for (std::size_t i{}; i<predictions.size(); ++i) {
      ...
      if (item == predictions[i].probabilities.end()) {
        OPENMS_LOG_WARN << "WARNING: Predictions map does not contain expected labels: ";
        ...
        for (auto key : view) OPENMS_LOG_WARN << key << " ";
