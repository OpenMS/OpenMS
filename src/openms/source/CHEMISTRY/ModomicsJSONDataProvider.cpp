...
namespace OpenMS
{
  ...
  namespace // anonymous namespace for file-local helpers
  {
    ...
    struct ParsedEntry_
    {
      ...
      std::string alternative_1;
      std::string alternative_2;
    ...
    void entryIsWellFormed_(const nlohmann::json::value_type& entry)
    {
      ...
      {
        std::string msg = "\"name\" entry missing for ribonucleotide";
        throw Exception::MissingInformation(__FILE__, __LINE__,
                                                OPENMS_PRETTY_FUNCTION, msg);
      }
      if (!entry.contains("short_name"))
      {
        std::string msg = "\"short_name\" entry missing for ribonucleotide";
        throw Exception::MissingInformation(__FILE__, __LINE__,
                                                OPENMS_PRETTY_FUNCTION, msg);
      }
      if (!entry.contains("reference_moiety"))
      {
        std::string msg = "\"reference_moiety\" entry missing for ribonucleotide";
        throw Exception::MissingInformation(__FILE__, __LINE__,
                                                OPENMS_PRETTY_FUNCTION, msg);
      }
      if (!entry.contains("formula"))
      {
        std::string msg = "\"formula\" entry missing for ribonucleotide";
        throw Exception::MissingInformation(__FILE__, __LINE__,
                                                OPENMS_PRETTY_FUNCTION, msg);
      }
    }
    ...
    {
      std::string code = entry.at("short_name").get<std::string>();
      ...
      {
        if (StringUtils::hasPrefix(code, 'd')) // handle deoxyribose, possibly with methyl mod
        {
          return EmpiricalFormula("C5H10O4");
        }
        else if (StringUtils::hasSuffix(code, 'm')) // mod. attached to the ribose, not base
        {
          return EmpiricalFormula("C6H12O5");
        }
        else if (StringUtils::hasSuffix(code, "m*")) // check if we have both a sulfer and a 2'-O methyl
        {
          return EmpiricalFormula("C6H12O5");
        }
        else if (StringUtils::hasSuffix(code, "Ar(p)") ||  StringUtils::hasSuffix(code, "Gr(p)"))
        {
    ...
    ParsedEntry_ parseEntry_(const nlohmann::json::value_type& entry)
    {
      ...
      {
        std::string msg = "\"reference_moiety\" must be a JSON array";
      ...
      else if (entry["reference_moiety"].size() == 4) // if all moieties are possible it might be a terminal
      {
        ...
        if (StringUtils::hasSuffix(code, "pN"))
        {
          ribo->setTermSpecificity(Ribonucleotide::FIVE_PRIME);
        }
        else if (StringUtils::hasSuffix(code, "p") && StringUtils::hasPrefix(code, "N"))
        {
      ...
      {
        std::string msg = "we don't support bases with multiple reference moieties or multicharacter moieties.";
        throw Exception::InvalidValue(__FILE__, __LINE__,
                                                OPENMS_PRETTY_FUNCTION, msg, entry["reference_moiety"].dump());
      }
      ...
      // Handle ambiguities
      if (StringUtils::hasSuffix(code, '?') || StringUtils::hasSuffix(code, "?*")) // ambiguity code -> fill the map
      {
        if (!entry.contains("alternatives"))
        {
          std::string msg = "Ambiguous mod without alternative found in " + code;
        ...
        {
          std::string msg = "\"alternatives\" must be an array with at least 2 entries for " + code;
          throw Exception::ParseError(__FILE__, __LINE__, OPENMS_PRETTY_FUNCTION, code, msg);
        }
  ...
  ModomicsJSONDataProvider::ModomicsJSONDataProvider(const std::string& filename)
    : filename_(filename)
  {
  }
