...
namespace OpenMS::ims
{
  ...
  Weights::alphabet_mass_type Weights::getParentMass(const std::vector<unsigned int> & decomposition) const
  {
    ...
    {
      throw Exception::InvalidParameter(__FILE__, __LINE__, OPENMS_PRETTY_FUNCTION,std::string("The passed decomposition has the wrong size. Expected ") + StringUtils::toStr(alphabet_masses_.size()) + std::string(" but got ") + StringUtils::toStr(decomposition.size()) + std::string("."));
    }
