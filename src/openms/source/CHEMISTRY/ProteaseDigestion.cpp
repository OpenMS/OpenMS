...
{
  const DigestionEnzymeProtein* new_enzyme = ProteaseDB::getInstance()->getEnzyme(enzyme_name);
  if (enzyme_ != new_enzyme)
  {
    enzyme_ = new_enzyme;
...
Size ProteaseDigestion::digest(const AASequence& protein, vector<AASequence>& output, Size min_length, Size max_length) const
{
  ...
  std::transform(idcs.begin(), idcs.end(), std::back_inserter(output), [&protein](std::pair<size_t, size_t>& start_end) {
...
Size ProteaseDigestion::digest(const AASequence& protein, vector<std::pair<size_t, size_t>>& output, Size min_length, Size max_length) const
{
  ...
  if (! (specificity_ == Specificity::SPEC_FULL || specificity_ == Specificity::SPEC_SEMI))
  {
  ...
  if (max_length == 0 || max_length > protein.size()) { max_length = protein.size(); }
  ...
  for (Size i = 1; i < count; ++i)
  {
    ...
    if (l >= min_length && l <= max_length) { output.emplace_back(begin, pep_positions[i]); }
    else { ++wrong_size; }
  ...
  if (pep_positions.size() > 1 && mc != 0) // there is at least one cleavage site (in addition to last position)!
  {
    ...
    for (Size mcs = 1; ((mcs <= mc) && (mcs < count - 1)); ++mcs)
    {
      ...
      for (Size j = 1; j < count - mcs; ++j)
      {
        ...
        if (l >= min_length && l <= max_length) { output.emplace_back(begin, pep_positions[j + mcs]); }
        else { ++wrong_size; }
  ...
  if (specificity_ == SPEC_SEMI) { wrong_size = wrong_size + semiSpecificDigestion_(pep_positions, output, min_length, max_length); }
