...
namespace OpenMS
{
  ...
  void TheoreticalSpectrumGeneratorXLMS::addPeak_(PeakSpectrum & spectrum, DataArrays::IntegerDataArray & charges, DataArrays::StringDataArray & ion_names, double pos, double intensity, Residue::ResidueType res_type, Size frag_index, int charge, std::string ion_type) const
  {
    ...
    if (add_metainfo_)
    {
      ...
      ion_names.emplace_back("[" + ion_type + "$" + std::string(Residue::residueTypeToIonLetter(res_type)) + StringUtils::toStr(frag_index) + "]" + IonNaming::chargeSuffix(charge));
    }
    if (add_charges_)
    {
  ...
  void TheoreticalSpectrumGeneratorXLMS::addLinearIonLosses_(PeakSpectrum & spectrum, DataArrays::IntegerDataArray& charges, DataArrays::StringDataArray& ion_names, double mono_weight, Residue::ResidueType res_type, Size frag_index, double intensity, int charge, std::string ion_type, LossIndex & losses) const
  {
    ...
    if (losses.has_H2O_loss)
    {
      ...
      if (mass_with_loss > 0.0)
      {
        ...
        if (add_metainfo_)
        {
          ...
          ion_names.emplace_back("[" + ion_type + "$" + std::string(Residue::residueTypeToIonLetter(res_type)) + StringUtils::toStr(frag_index) + "-H2O1]" + IonNaming::chargeSuffix(charge));
        }
        if (add_charges_)
        {
          charges.push_back(charge);
    ...
    if (losses.has_NH3_loss)
    {
      ...
      if (mass_with_loss > 0.0)
      {
        ...
        {
          // remove final bracket, insert loss name and add the bracket again
          ion_names.emplace_back("[" + ion_type + "$" + std::string(Residue::residueTypeToIonLetter(res_type)) + StringUtils::toStr(frag_index) + "-H3N1]" + IonNaming::chargeSuffix(charge));
        }
        if (add_charges_)
        {
          charges.push_back(charge);
  ...
  void TheoreticalSpectrumGeneratorXLMS::addPrecursorPeaks_(PeakSpectrum & spectrum, DataArrays::IntegerDataArray & charges, DataArrays::StringDataArray & ion_names, double precursor_mass, int charge) const
  {
    ...
    if (add_metainfo_)
    {
      ion_names.emplace_back(std::string("[M+H]") + IonNaming::chargeSuffix(charge));
    }
    if (add_charges_)
    {
      charges.push_back(charge);
    ...
    if (add_metainfo_)
    {
      ion_names.emplace_back(std::string("[M+H]-H2O") + IonNaming::chargeSuffix(charge));
    }
    if (add_charges_)
    {
      charges.push_back(charge);
    ...
    if (add_metainfo_)
    {
      ion_names.emplace_back(std::string("[M+H]-NH3") + IonNaming::chargeSuffix(charge));
    }
    if (add_charges_)
    {
      charges.push_back(charge);
  ...
  void TheoreticalSpectrumGeneratorXLMS::addKLinkedIonPeaks_(PeakSpectrum & spectrum, DataArrays::IntegerDataArray & charges, DataArrays::StringDataArray & ion_names, AASequence & peptide, Size link_pos, double precursor_mass, bool frag_alpha, int charge) const
  {
    ...
    if (add_metainfo_)
    {
      ...
      // the ion name spells out the charge, see PeptideHit::PeakAnnotation
      ion_name = "[" + peptide[l_pos].getOneLetterCode() + "-linked-" + ion_type + "]" + IonNaming::chargeSuffix(charge);
  ...
  void TheoreticalSpectrumGeneratorXLMS::addXLinkIonLosses_(PeakSpectrum & spectrum, DataArrays::IntegerDataArray& charges, DataArrays::StringDataArray& ion_names, double mono_weight, double intensity, int charge, std::string ion_name, LossIndex & losses) const
  {
    ...
    if (losses.has_H2O_loss)
    {
      ...
      if (mass_with_loss > 0.0)
      {
        ...
        {
          // remove final bracket, insert loss name and add the bracket again
          ion_names.emplace_back(StringUtils::prefix(ion_name, ion_name.size()-1) + "-H2O1]" + IonNaming::chargeSuffix(charge));
        }
        if (add_charges_)
        {
          charges.push_back(charge);
    ...
    if (losses.has_NH3_loss)
    {
      ...
      if (mass_with_loss > 0.0)
      {
        ...
        {
          // remove final bracket, insert loss name and add the bracket again
          ion_names.emplace_back(StringUtils::prefix(ion_name, ion_name.size()-1) + "-H3N1]" + IonNaming::chargeSuffix(charge));
        }
        if (add_charges_)
        {
          charges.push_back(charge);
