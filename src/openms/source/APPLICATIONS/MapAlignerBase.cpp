...
namespace OpenMS
{
  ...
  void TOPPMapAlignerBase::registerOptionsAndFlagsMapAligners_(const std::string& file_formats,
                                                               ReferenceParameterKind ref_params)
  {
  ...
  TOPPBase::ExitCodes TOPPMapAlignerBase::checkParameters_()
  {
    //-------------------------------------------------------------
    // parameter handling
    //-------------------------------------------------------------
    ...
    if (!in_spectra.empty() || !out_spectra.empty())
    {
      ...
      {
        writeLogError_("Error: The number of spectra input and output spectra files has to be equal (parameters 'in_spectra_files'/'out_spectra_files')");
        return ILLEGAL_PARAMETERS;
      }
    }
  ...
  void TOPPMapAlignerBase::transformSpectraFiles_(const StringList& in_spectra_files,
