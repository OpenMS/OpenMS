...
namespace OpenMS
{
  ...
  {
    // Create ExternalIDHandler for SVM-related operations
    ExternalIDHandler ext_handler;
    ...
    if (with_external_ids)
    {
      ext_handler.filterClassifiedFeatures(features, ext_handler.getSVMProbsInternal().empty() ? 0.0 : double(param_.getValue("svm:min_prob")));
    ...
    if (with_external_ids && !ext_handler.getSVMProbsInternal().empty())
    {
  ...
  void OpenMS::FeatureFinderIdentificationAlgorithm::checkNumObservations_(OpenMS::Size n_pos, OpenMS::Size n_neg, const OpenMS::String& note) const
  {
    ...
    OpenMS::ExternalIDHandler ext_handler;
    ext_handler.initSVMParameters(param_);
