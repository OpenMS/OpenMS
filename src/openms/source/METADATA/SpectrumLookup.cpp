...
namespace OpenMS
{
  ...
  SpectrumLookup::SpectrumLookup(): 
    ...
    regexp_name_list_(ListUtils::create<std::string>(regexp_names_, ' '))
  {}
  ...
  Size SpectrumLookup::findByRT(double rt) const
  {
    ...
    std::string element = "spectrum with RT " + StringUtils::toStr(rt);
    throw Exception::ElementNotFound(__FILE__, __LINE__, OPENMS_PRETTY_FUNCTION,
                                     element);
  }


  Size SpectrumLookup::findByNativeID(const std::string& native_id) const
  {
    map<std::string, Size>::const_iterator pos = ids_.find(native_id);
    if (pos == ids_.end())
    {
      std::string element = "spectrum with native ID '" + native_id + "'";
      throw Exception::ElementNotFound(__FILE__, __LINE__, OPENMS_PRETTY_FUNCTION,
                                       element);
    }
    return pos->second;
  ...
  Size SpectrumLookup::findByIndex(Size index, bool count_from_one) const
  {
    ...
    {
      std::string element = "spectrum with index " + StringUtils::toStr(index);
      throw Exception::ElementNotFound(__FILE__, __LINE__, OPENMS_PRETTY_FUNCTION,
                                       element);
    }
  ...
  Size SpectrumLookup::findByScanNumber(Size scan_number) const
  {
    ...
    if (pos == scans_.end())
    {
      std::string element = "spectrum with scan number " + StringUtils::toStr(scan_number);
      throw Exception::ElementNotFound(__FILE__, __LINE__, OPENMS_PRETTY_FUNCTION,
                                       element);
    }
    return pos->second;
  ...
  void SpectrumLookup::addReferenceFormat(const std::string& regexp)
  {
    // does the reg. exp. contain any of the recognized group names?
    bool found = false;
    for (vector<std::string>::iterator it = regexp_name_list_.begin();
         it != regexp_name_list_.end(); ++it)
    {
      if (StringUtils::hasSubstring(regexp, "?<" + (*it) + ">"))
      {
        found = true;
        break;
      }
    ...
    if (!found)
    {
      std::string msg = "The regular expression describing the reference format must contain at least one of the following named groups (in the format '?<GROUP>'): " + regexp_names_;
                                          ...
                                          const boost::smatch& match) const
  {
    ...
    {
      std::string value = match["INDEX0"].str();
      if (!value.empty())
      {
        Size index = StringUtils::toInt32(value);
        return findByIndex(index, false);
      }
    }
    if (match["INDEX1"].matched)
    {
      std::string value = match["INDEX1"].str();
      if (!value.empty())
      {
        Size index = StringUtils::toInt32(value);
    ...
    if (match["SCAN"].matched)
    {
      ...
      if (!value.empty())
      {
        Size scan_number = StringUtils::toInt32(value);
        return findByScanNumber(scan_number);
      }
    }
    if (match["ID"].matched)
    {
      std::string value = match["ID"].str();
      if (!value.empty())
      {
    ...
    if (match["RT"].matched)
    {
      ...
      if (!value.empty())
      {
        double rt = StringUtils::toDouble(value);
        return findByRT(rt);
      }
    }
    std::string msg = "Unexpected format of spectrum reference '" + spectrum_ref +
  ...
  bool SpectrumLookup::isNativeID(const std::string& id)
  {
    return SpectrumNativeIDParser::isNativeID(id);
  }

  std::string SpectrumLookup::getRegExFromNativeID(const std::string& id)
  {
  ...
  void SpectrumLookup::setScanRegExp_(const std::string& scan_regexp)
  {
    if (!scan_regexp.empty())
    {
      if (!StringUtils::hasSubstring(scan_regexp, "?<SCAN>"))
      {
        std::string msg = "The regular expression for extracting scan numbers from native IDs must contain a named group '?<SCAN>'.";
