...
START_SECTION([EXTRA] void extractPeakMaps includes peaks on mz and IM window boundaries)
{
  typedef OpenMS::DataArrays::FloatDataArray FloatDataArray;
  ...
  PeakMapExtractor extractor;
  std::vector<PeakMapExtractor::ExtractionCoordinates> coordinates(1);
  coordinates[0].mz = 500.02;
  coordinates[0].ion_mobility = 1.10;
  coordinates[0].rt_start = 0.0;
  coordinates[0].rt_end = -1.0;
  coordinates[0].id = "tr1";

  std::vector<PeakMapExtractor::ExtractedPeakMap> output;
  extractor.extractPeakMaps(expptr, output, coordinates, 0.06, false, 0.20, "tophat");

  TEST_EQUAL(output.size(), 1)
  TEST_EQUAL(output[0].mz.size(), 4)
  TEST_REAL_SIMILAR(output[0].mz.front(), 499.99)
  TEST_REAL_SIMILAR(output[0].mz.back(), 500.03)
  TEST_REAL_SIMILAR(output[0].ion_mobility.front(), 1.00)
  TEST_REAL_SIMILAR(output[0].ion_mobility.back(), 1.15)
}
END_SECTION
