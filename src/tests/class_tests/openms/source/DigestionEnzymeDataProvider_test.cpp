...
START_SECTION(BuiltInProteaseDataProvider - Thermolysin and Proteinase K)
{
  BuiltInProteaseDataProvider provider;
  ...
  {
    if (enz->getName() == "Thermolysin")
    {
      found_thermo = true;
      TEST_EQUAL(enz->getRegEx(), "(?<![DE])(?=[AFILMV])|(?<=R)(?=G)")
    }
    else if (enz->getName() == "Proteinase K")
    {
      found_pk = true;
