...
// Two overlapping copies of the get_mobilogram(0) peak, offset by 14 grid points (~0.0145 IM
// units) so both apexes survive as separate local maxima. 'peak_width' forces each peak's
// boundary out to at least that width regardless of the valley between them, so with it set
// wider than the peak separation both peaks' true boundaries do overlap.
RichPeakMobilogram get_overlapping_mobilogram()
{
  static const double im_data1[] = {0.93734956, 0.93835497, 0.9393905,  0.9404265,  0.9414631, 0.9425003,  0.9435083,  0.9445466,  0.9455854,  0.94662476, 0.947635,
       0.94867545, 0.9496867,  0.95072824, 0.95177037, 0.9527832, 0.9538264,  0.95484036, 0.9558847,  0.9568997,  0.9579451,  0.9589612,
       0.96000767, 0.9610248,  0.96207243, 0.96309066, 0.9641094, 0.96515864, 0.9661785,  0.96719885, 0.96824974, 0.9692712,  0.9702931,
  ...
  {
    double intensity = 1.0; // avoid a zero baseline, which the S/N estimator dislikes
    if (k < n) intensity += int_data1[k];
    if (k >= shift && (k - shift) < n) intensity += int_data1[k - shift];
    MobilityPeak1D peak;
    const double im = (k < n) ? im_data1[k] : im_data1[n - 1] + (k - n + 1) * (im_data1[n - 1] - im_data1[0]) / (n - 1);
    peak.setPos(im);
...
START_SECTION([EXTRA] remove_overlapping_peaks actually controls whether overlapping peak boundaries are corrected)
{
  // 'peak_width' forces each peak's boundary out to at least that width, wider here than the
  // separation between the two peak apexes, so without the correction both peaks' boundaries
  // genuinely overlap (each one's raw intensity is integrated into both peaks).
  RichPeakMobilogram mobilogram = get_overlapping_mobilogram();
  RichPeakMobilogram picked, smoothed;
  PeakPickerMobilogram picker;
  Param param = picker.getParameters();
  param.setValue("method", "corrected");
  param.setValue("use_gauss", "false");
  param.setValue("peak_width", 0.03);
  param.setValue("remove_overlapping_peaks", "false");
  picker.setParameters(param);
  picker.pickMobilogram(mobilogram, picked, smoothed);

  ABORT_IF(picked.size() != 2)
  // 'false': the boundaries are left overlapping (peak 1's left border is peak 0's, not the
  // valley between them), and both peaks therefore integrate almost the same, near-total intensity.
  TEST_REAL_SIMILAR(picked.getFloatDataArrays()[PeakPickerMobilogram::IDX_LEFTBORDER][0], 0.94867545);
  TEST_REAL_SIMILAR(picked.getFloatDataArrays()[PeakPickerMobilogram::IDX_RIGHTBORDER][0], 1.01150727);
  TEST_REAL_SIMILAR(picked.getFloatDataArrays()[PeakPickerMobilogram::IDX_LEFTBORDER][1], 0.94867545);
  TEST_REAL_SIMILAR(picked.getFloatDataArrays()[PeakPickerMobilogram::IDX_RIGHTBORDER][1], 1.01253724);
  TEST_REAL_SIMILAR(picked.getFloatDataArrays()[PeakPickerMobilogram::IDX_ABUNDANCE][0], 1773352.625);
  TEST_REAL_SIMILAR(picked.getFloatDataArrays()[PeakPickerMobilogram::IDX_ABUNDANCE][1], 1773410.625);

  param.setValue("remove_overlapping_peaks", "true");
  picker.setParameters(param);
  picker.pickMobilogram(mobilogram, picked, smoothed);

  ABORT_IF(picked.size() != 2)
  // 'true': the shared boundary is pulled back to the valley between the two peaks, so peak 0's
  // right border now equals peak 1's left border, and each integrates only its own share.
  TEST_REAL_SIMILAR(picked.getFloatDataArrays()[PeakPickerMobilogram::IDX_RIGHTBORDER][0], 0.98471683);
  TEST_REAL_SIMILAR(picked.getFloatDataArrays()[PeakPickerMobilogram::IDX_LEFTBORDER][1], 0.98471683);
  TEST_REAL_SIMILAR(picked.getFloatDataArrays()[PeakPickerMobilogram::IDX_ABUNDANCE][0], 933019.0625);
  TEST_REAL_SIMILAR(picked.getFloatDataArrays()[PeakPickerMobilogram::IDX_ABUNDANCE][1], 890960.0625);
}
END_SECTION
