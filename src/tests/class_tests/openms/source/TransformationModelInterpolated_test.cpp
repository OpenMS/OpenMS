...
START_SECTION((double evaluate(double value) const))
{
  ...
  for (Size i = 0; i < data_points.rowCount(); ++i)
  {
    ...
    {
      base_data.push_back(make_pair(StringUtils::toDouble(sl[0]), StringUtils::toDouble(sl[1])));
    }
  }
  ...
  for (Size i = 0; i < gsl_results.rowCount(); ++i)
  {
    ...
    {
      gsl_target_points.push_back(StringUtils::toDouble(sl[0]));
      gsl_linear.push_back(StringUtils::toDouble(sl[1]));
      gsl_akima.push_back(StringUtils::toDouble(sl[2]));
      gsl_cspline.push_back(StringUtils::toDouble(sl[3]));
    }
  }
