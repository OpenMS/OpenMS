...
// triple SILAC
std::string labels = "[][Lys4,Arg6][Lys8,Arg10]";
int max_nr_labelled_aas = 1;

MultiplexDeltaMassesGenerator* nullPointer = nullptr;
MultiplexDeltaMassesGenerator* ptr;

START_SECTION(MultiplexDeltaMassesGenerator(std::string labels, int max_nr_labelled_aas, std::map<std::string,double> label_mass_shift))
...
START_SECTION(std::vector<std::vector<std::string> > MultiplexDeltaMassesGenerator::getSamplesLabelsList())
  std::vector<std::vector<std::string> > samples_labels = list.getSamplesLabelsList();
...
START_SECTION(std::string MultiplexDeltaMassesGenerator::getLabelShort(std::string label))
  TEST_EQUAL(list.getLabelShort("Label:13C(6)15N(2)"), "Lys8");
  TEST_EQUAL(list.getLabelShort("Dimethyl:2H(6)13C(2)"), "Dimethyl8");
END_SECTION

START_SECTION(std::string MultiplexDeltaMassesGenerator::getLabelLong(std::string label))
