...
START_SECTION(double bwNrd0(const std::vector<double>&))
{
  ...
  double bw = KernelDensityEstimation::bwNrd0(uniform_data);
  ...
  bw = KernelDensityEstimation::bwNrd0(normal_data);
  TEST_EQUAL(bw > 0.0, true)
  ...
  // Test with constant values (zero variance)
  {
    ...
    bw = KernelDensityEstimation::bwNrd0(constant_data);
  ...
  bw = KernelDensityEstimation::bwNrd0(single_point);
  ...
  bw = KernelDensityEstimation::bwNrd0(with_nan);
  TEST_EQUAL(bw > 0.0, true)
  
  // Test empty vector
  std::vector<double> empty;
  bw = KernelDensityEstimation::bwNrd0(empty);
  TEST_REAL_SIMILAR(bw, 0.0)
}
END_SECTION
...
START_SECTION(std::vector<double> linBin(const std::vector<double>&, double, double, std::size_t, const std::vector<double>*))
{
  ...
  auto bins = KernelDensityEstimation::linBin(data, 0.0, 1.0, 3, nullptr);
  ...
  bins = KernelDensityEstimation::linBin(data, 0.0, 1.0, 3, &weights);
  TEST_EQUAL(bins.size(), 3)
  sum = 0.0;
  for (double v : bins) sum += v;
  ...
  bins = KernelDensityEstimation::linBin(out_of_range, 0.0, 1.0, 5, nullptr);
  sum = 0.0;
  for (double v : bins) sum += v;
  ...
  // Test empty data
  std::vector<double> empty;
  bins = KernelDensityEstimation::linBin(empty, 0.0, 1.0, 5, nullptr);
...
START_SECTION(std::vector<double> forRt(const std::vector<double>&, std::size_t))
{
  ...
  auto fft_result = KernelDensityEstimation::forRt(signal, 4);
  ...
  fft_result = KernelDensityEstimation::forRt(signal, 8);
  ...
  fft_result = KernelDensityEstimation::forRt(constant, 8);
...
START_SECTION(std::vector<double> revRt(const std::vector<double>&, std::size_t))
{
  ...
  auto fft_result = KernelDensityEstimation::forRt(signal, 8);
  auto reconstructed = KernelDensityEstimation::revRt(fft_result, 8);
  ...
  fft_result = KernelDensityEstimation::forRt(small, 4);
  reconstructed = KernelDensityEstimation::revRt(fft_result, 4);
...
START_SECTION(std::vector<double> silvermanKernelFFT(double, std::size_t, double))
{
  ...
  auto kernel_fft = KernelDensityEstimation::silvermanKernelFFT(bw, M, range);
  ...
  kernel_fft = KernelDensityEstimation::silvermanKernelFFT(0.5, M, range);
  TEST_EQUAL(kernel_fft.size(), M)
  TEST_REAL_SIMILAR(kernel_fft[0], 1.0)
  
  // Test with larger grid
  kernel_fft = KernelDensityEstimation::silvermanKernelFFT(1.0, 64, range);
...
START_SECTION((gridKdeFFT))
{
  ...
  auto result = KernelDensityEstimation::gridKdeFFT(data, bw, gridsize, 3.0);
  ...
  result = KernelDensityEstimation::gridKdeFFT(single, 1.0, 32, 3.0);
  ...
  result = KernelDensityEstimation::gridKdeFFT(bimodal, 0.5, 128, 3.0);
  ...
  // Test empty data
  std::vector<double> empty;
  result = KernelDensityEstimation::gridKdeFFT(empty, 1.0, 32, 3.0);
  TEST_EQUAL(result.first.size() >= 32, true)
...
START_SECTION((kdeFFTEval))
{
  ...
  auto densities = KernelDensityEstimation::kdeFFTEval(data, bw, 64, 3.0);
  ...
  densities = KernelDensityEstimation::kdeFFTEval(clustered, 0.5, 128, 3.0);
  ...
  densities = KernelDensityEstimation::kdeFFTEval(single, 1.0, 64, 3.0);
  ...
  densities = KernelDensityEstimation::kdeFFTEval(identical, 0.5, 64, 3.0);
  ...
  // Test empty data
  std::vector<double> empty;
  densities = KernelDensityEstimation::kdeFFTEval(empty, 1.0, 64, 3.0);
...
START_SECTION((Integration test: Full KDE pipeline))
{
  ...
  double bw = KernelDensityEstimation::bwNrd0(data);
  TEST_EQUAL(bw > 0.0, true)
  
  // Step 2: Grid-based KDE
  auto grid_result = KernelDensityEstimation::gridKdeFFT(data, bw, 256, 3.0);
  TEST_EQUAL(grid_result.first.size() >= 256, true)
  TEST_EQUAL(grid_result.second.size() >= 256, true)
  
  // Step 3: Evaluate at data points
  auto point_densities = KernelDensityEstimation::kdeFFTEval(data, bw, 256, 3.0);
...
START_SECTION("kde reference (statsmodels) regression")
{
  ...
  for (auto& [dataset_name, rows] : datasets_by_name)
  {
    ...
    double our_bw = KernelDensityEstimation::bwNrd0(data_points);
    ...
    auto our_densities = KernelDensityEstimation::kdeFFTEval(data_points, our_bw);
