...
START_TEST(BasicProteinInferenceAlgorithm, "$Id$")
    ...
    START_SECTION(BasicProteinInferenceAlgorithm on Protein Peptide ID with grouping and user score)
    {
      ...
      TEST_EQUAL(peps.getScoreType(), "Posterior Error Probability"); // check if main score is PEP
      bpia.run(peps, prots);
      TEST_EQUAL(peps.getScoreType(), "Posterior Error Probability"); // check if main score has been reset again to PEP
