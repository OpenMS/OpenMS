...
cdef extern from "<OpenMS/ANALYSIS/OPENSWATH/TransitionTSVFile.h>" namespace "OpenMS":
    ...
    cdef cppclass TransitionTSVFile(ProgressLogger):
        ...
        void convertTargetedExperimentToTSV(char * filename, TargetedExperiment& targeted_exp) nogil except + # wrap-doc:\n Write out a targeted experiment (TraML structure) into a tsv file
    
        void convertTSVToTargetedExperiment(char * filename, FileType filetype, TargetedExperiment& targeted_exp) nogil except + # wrap-doc:\n Read in a tsv/mrm file and construct a targeted experiment (TraML structure)
        void convertTSVToTargetedExperiment(char * filename, FileType filetype, LightTargetedExperiment& targeted_exp) nogil except + # wrap-doc:\n Read in a tsv file and construct a targeted experiment (Light transition structure)
    
        void validateTargetedExperiment(TargetedExperiment targeted_exp) nogil except + # wrap-doc:\n Validate a TargetedExperiment (check that all ids are unique)
