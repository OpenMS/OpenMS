...
cdef extern from "<OpenMS/TRANSFORMATIONS/RAW2PEAK/ContinuousWaveletTransform.h>" namespace "OpenMS":
    ...
    cdef cppclass ContinuousWaveletTransform "OpenMS::ContinuousWaveletTransform":
        ...
        libcpp_vector[ Peak1D ]  getSignal() nogil except + # wrap-doc:Access to the wavelet transform of the signal
        void setSignal(libcpp_vector[ Peak1D ] & signal) nogil except + # wrap-doc:Mutable access to the wavelet transform of the signal
        libcpp_vector[ double ]  getWavelet() nogil except + # wrap-doc:Access to the wavelet
        void setWavelet(libcpp_vector[ double ] & wavelet) nogil except + # wrap-doc:Mutable access to the signal
        double  getScale() nogil except + # wrap-doc:Access to the scale of the wavelet
        void setScale(double scale) nogil except + # wrap-doc:Mutable access to the spacing of raw data
        double  getSpacing() nogil except + # wrap-doc:Access to the spacing of raw data
        void setSpacing(double spacing) nogil except + # wrap-doc:Mutable access to the spacing of raw data
        SignedSize getLeftPaddingIndex() nogil except + # wrap-doc:Access to the position where the signal starts (in the interval [0,end_left_padding_) are the padded zeros)
        ...
        SignedSize getRightPaddingIndex() nogil except + # wrap-doc:Access to the position where the signal ends (in the interval (begin_right_padding_,end] are the padded zeros)
        ...
        SignedSize getSignalLength() nogil except + # wrap-doc:Access to signal length [end_left_padding,begin_right_padding]
        ...
        int getSize() nogil except + # wrap-doc:Access to signal length including padded zeros [0,end]
