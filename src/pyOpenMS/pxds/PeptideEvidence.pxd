...
cdef extern from "<OpenMS/METADATA/PeptideEvidence.h>" namespace "OpenMS":
    cdef cppclass PeptideEvidence :
        ...
        void setStart(Int start) nogil except + # wrap-doc:Set the position of the last AA of the peptide in protein coordinates (starting at 0 for the N-terminus). If not available, set to UNKNOWN_POSITION. N-terminal positions must be marked with `N_TERMINAL_AA`
        Int getStart() nogil except + # wrap-doc:Get the position in the protein (starting at 0 for the N-terminus). If not available UNKNOWN_POSITION constant is returned
        void setEnd(Int end) nogil except + # wrap-doc:Set the position of the last AA of the peptide in protein coordinates (starting at 0 for the N-terminus). If not available, set UNKNOWN_POSITION. C-terminal positions must be marked with C_TERMINAL_AA
        Int getEnd() nogil except + # wrap-doc:Get the position of the last AA of the peptide in protein coordinates (starting at 0 for the N-terminus). If not available UNKNOWN_POSITION constant is returned
        ...
        void setProteinAccession(String s) nogil except + # wrap-doc:Set the protein accession the peptide matches to. If not available set to empty string
        String getProteinAccession() nogil except + # wrap-doc:Get the protein accession the peptide matches to. If not available the empty string is returned
