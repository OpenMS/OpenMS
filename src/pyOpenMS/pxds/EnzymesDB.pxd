...
cdef extern from "<OpenMS/CHEMISTRY/EnzymesDB.h>" namespace "OpenMS":
    ...
    cdef cppclass EnzymesDB "OpenMS::EnzymesDB":
        ...
        const DigestionEnzyme* getEnzyme(String& name) nogil except +
        const DigestionEnzyme* getEnzymeByRegEx(String& cleavage_regex) nogil except +
        void setEnzymes(String& filename) nogil except +
        void addEnzyme(DigestionEnzyme& enzyme) nogil except +
        ...
        # bool hasEnzyme(DigestionEnzyme* enzyme) nogil except + # does not make sense as the ptr wont match
