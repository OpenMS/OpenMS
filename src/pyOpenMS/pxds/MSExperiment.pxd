...
cdef extern from "<OpenMS/KERNEL/MSExperiment.h>" namespace "OpenMS":
    ...
    cdef cppclass MSExperiment(ExperimentalSettings):
        ...
        libcpp_vector[libcpp_vector[double]] aggregateFromMatrixWithIM(Matrix[double] & ranges, unsigned int ms_level, libcpp_utf8_string mz_agg) except + nogil # wrap-doc:Aggregates intensity values with ion mobility filtering. Matrix rows contain [mz_min, mz_max, rt_min, rt_max, im_min, im_max]. If im_min > im_max for a row, no IM filtering is applied for that range.
        ...
        libcpp_vector[MSChromatogram] extractXICsFromMatrixWithIM(Matrix[double] & ranges, unsigned int ms_level, libcpp_utf8_string mz_agg) except + nogil # wrap-doc:Extracts XIC chromatograms with ion mobility filtering. Matrix rows contain [mz_min, mz_max, rt_min, rt_max, im_min, im_max]. If im_min > im_max for a row, no IM filtering is applied for that range.
        libcpp_vector[libcpp_vector[double]] aggregateFromMatrixWithIMAndPrecursor(Matrix[double] & ranges, unsigned int ms_level, libcpp_utf8_string mz_agg) except + nogil # wrap-doc:Aggregates intensity values with ion mobility and precursor m/z filtering for DIA data. Matrix rows contain [mz_min, mz_max, rt_min, rt_max, im_min, im_max, prec_mz_min, prec_mz_max]. If im_min > im_max, no IM filtering. If prec_mz_min > prec_mz_max, no precursor filtering.
        libcpp_vector[MSChromatogram] extractXICsFromMatrixWithIMAndPrecursor(Matrix[double] & ranges, unsigned int ms_level, libcpp_utf8_string mz_agg) except + nogil # wrap-doc:Extracts XIC chromatograms with ion mobility and precursor m/z filtering for DIA data. Matrix rows contain [mz_min, mz_max, rt_min, rt_max, im_min, im_max, prec_mz_min, prec_mz_max]. If im_min > im_max, no IM filtering. If prec_mz_min > prec_mz_max, no precursor filtering.
