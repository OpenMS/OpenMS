...
cdef extern from "<OpenMS/ANALYSIS/MAPMATCHING/TransformationModel.h>" namespace "OpenMS":
    ...
    cdef cppclass TransformationModel:
        ...
        void weightData(libcpp_vector[TM_DataPoint]& data) nogil except + # wrap-doc:\n Weight the data by the given weight function
        bool checkValidWeight(const String& weight, libcpp_vector[String]& valid_weights) nogil except + # wrap-doc:\n Check for a valid weighting function string
        double weightDatum(double& datum, const String& weight) nogil except + # wrap-doc:\n Weight the data according to the weighting function
        double unWeightDatum(double& datum, const String& weight) nogil except + # wrap-doc:\n Apply the reverse of the weighting function to the data
        libcpp_vector[ String ] getValidXWeights() nogil except + # wrap-doc:\n Returns a list of valid x weight function stringss
        libcpp_vector[ String ] getValidYWeights() nogil except + # wrap-doc:\n Returns a list of valid y weight function strings

        void unWeightData(libcpp_vector[TM_DataPoint] & data) nogil except + # wrap-doc:\n Unweight the data by the given weight function
        double checkDatumRange(const double & datum, const double & datum_min, const double & datum_max) nogil except + # wrap-doc:\n Check that the datum is within the valid min and max bounds
