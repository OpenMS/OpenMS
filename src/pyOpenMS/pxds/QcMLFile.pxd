...
cdef extern from "<OpenMS/FORMAT/QcMLFile.h>" namespace "OpenMS":
    ...
    cdef cppclass QcMLFile(XMLHandler,XMLFile,ProgressLogger) :
        ...
        void addRunQualityParameter(String r, QualityParameter qp) nogil except + # wrap-doc:Just adds a qualityparameter to run by the name r
        void addRunAttachment(String r, Attachment at) nogil except + # wrap-doc:Just adds a attachment to run by the name r
        void addSetQualityParameter(String r, QualityParameter qp) nogil except + # wrap-doc:Just adds a qualityparameter to set by the name r
        void addSetAttachment(String r, Attachment at) nogil except + # wrap-doc:Just adds a attachment to set by the name r
        void removeAttachment(String r, libcpp_vector[ String ] & ids, String at) nogil except + # wrap-doc:Removes attachments referencing a id given in ids, from run/set r. All attachments if no attachment name is given with at
        ...
        void removeQualityParameter(String r, libcpp_vector[ String ] & ids) nogil except + # wrap-doc:Just removes qualityparameter going by one of the ID attributes given in ids
