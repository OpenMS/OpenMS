...
cdef extern from "<OpenMS/CHEMISTRY/NucleicAcidSpectrumGenerator.h>" namespace "OpenMS":
    ...
    cdef cppclass NucleicAcidSpectrumGenerator(DefaultParamHandler):
        ...
        void getSpectrum(MSSpectrum &spec, NASequence &oligo, Int min_charge, Int max_charge) nogil except + # wrap-doc:Generates a spectrum for a peptide sequence, with the ion types that are set in the tool parameters. If precursor_charge is set to 0 max_charge + 1 will be used
