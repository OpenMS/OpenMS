...
:return: true if a hit was present, false otherwise
        ...
        .def_static("updateProteinGroups", [](std::vector<OpenMS::ProteinIdentification::ProteinGroup>& groups, const std::vector<OpenMS::ProteinHit>& hits) { return OpenMS::IDFilter::updateProteinGroups(groups, hits); }, "groups"_a, "hits"_a, 
            R"doc(
...
Filter identifications by "N best" PeptideIdentification objects (better PeptideIdentification means better [best] PeptideHit than other)
        ...
        .def_static("keepBestPerPeptidePerRun", [](std::vector<OpenMS::ProteinIdentification>& prot_ids, OpenMS::PeptideIdentificationList& pep_ids, bool ignore_mods, bool ignore_charges, unsigned long nr_best_spectrum) { return OpenMS::IDFilter::keepBestPerPeptidePerRun(prot_ids, pep_ids, ignore_mods, ignore_charges, nr_best_spectrum); }, "prot_ids"_a, "pep_ids"_a, "ignore_mods"_a, "ignore_charges"_a, "nr_best_spectrum"_a, "Filters PeptideHits from PeptideIdentification by keeping only the best peptide hits for every peptide sequence on a per run basis")
...
Removes hits annotated as decoys from peptide or protein identifications. Checks for meta values named "target_decoy" and "isDecoy", and removes protein/peptide hits if the values are "decoy" and "true", respectively
        ...
        .def_static("removeUnreferencedProteins", [](std::vector<OpenMS::ProteinIdentification>& proteins, OpenMS::PeptideIdentificationList& ids) { return OpenMS::IDFilter::removeUnreferencedProteins(proteins, ids); }, "proteins"_a, "ids"_a, "Removes protein hits from the protein IDs in a 'cmap' that are not referenced by a peptide in the features or if requested in the unassigned peptide list")
        ;
...
:param power: The x*x coefficient (for quadratic models)
    ...
    nb::class_<OpenMS::PrecursorCorrection>(m, "PrecursorCorrection", "This class provides methods for precursor correction")
        ...
        .def_static("getPrecursors", [](const OpenMS::MSExperiment& exp, std::vector<OpenMS::Precursor>& precursors, std::vector<double>& precursors_rt, std::vector<unsigned long>& precursor_scan_index) { return OpenMS::PrecursorCorrection::getPrecursors(exp, precursors, precursors_rt, precursor_scan_index); }, "exp"_a, "precursors"_a, "precursors_rt"_a, "precursor_scan_index"_a)
        ...
        .def_static("correctToNearestMS1Peak", [](OpenMS::MSExperiment& exp, double mz_tolerance, bool ppm, std::vector<double>& delta_mzs, std::vector<double>& mzs, std::vector<double>& rts) { return OpenMS::PrecursorCorrection::correctToNearestMS1Peak(exp, mz_tolerance, ppm, delta_mzs, mzs, rts); }, "exp"_a, "mz_tolerance"_a, "ppm"_a, "delta_mzs"_a, "mzs"_a, "rts"_a)
        .def_static("correctToHighestIntensityMS1Peak", [](OpenMS::MSExperiment& exp, double mz_tolerance, bool ppm, std::vector<double>& delta_mzs, std::vector<double>& mzs, std::vector<double>& rts) { return OpenMS::PrecursorCorrection::correctToHighestIntensityMS1Peak(exp, mz_tolerance, ppm, delta_mzs, mzs, rts); }, "exp"_a, "mz_tolerance"_a, "ppm"_a, "delta_mzs"_a, "mzs"_a, "rts"_a)
        ;
