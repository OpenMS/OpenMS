...
mz, intensities = spectrum.get_peaks()
        ...
        .def("aggregateFromMatrix", [](const OpenMS::MSExperiment& self, const OpenMS::Matrix<double>& ranges, unsigned int ms_level, const std::string& mz_agg) { return self.aggregateFromMatrix(ranges, ms_level, mz_agg); }, "ranges"_a, "ms_level"_a, "mz_agg"_a, "Aggregates intensity values for multiple m/z and RT ranges specified in a matrix")
        .def("extractXICsFromMatrix", [](const OpenMS::MSExperiment& self, const OpenMS::Matrix<double>& ranges, unsigned int ms_level, const std::string& mz_agg) { return self.extractXICsFromMatrix(ranges, ms_level, mz_agg); }, "ranges"_a, "ms_level"_a, "mz_agg"_a, "Extracts XIC chromatograms for multiple m/z and RT ranges specified in a matrix")
        ...
        .def("spectrumRanges", [](const OpenMS::MSExperiment& self) -> OpenMS::SpectrumRangeManager { return self.spectrumRanges(); }, "Returns a reference to the spectrum range manager")
        .def("chromatogramRanges", [](const OpenMS::MSExperiment& self) -> OpenMS::ChromatogramRangeManager { return self.chromatogramRanges(); }, "Returns a reference to the chromatogram range manager")
  ...
  static_assert(sizeof(OpenMS::MSImagingGeometry::Pixel) == 16, "Pixel layout assumption: 4(x) + 4(y) + 8(spectrum_index) = 16 bytes");
  static_assert(sizeof(OpenMS::UInt) == 4, "UInt must be 4 bytes (dtype assumes uint32 for pixel coordinates)");
  static_assert(sizeof(OpenMS::Size) == 8, "Size must be 8 bytes (dtype assumes uint64 for spectrum_index)");
...
Pixels are masked-out by default; setIntensity marks them present.
    ...
    .def(
      "__deepcopy__", [](const OpenMS::IonImage& self, nb::dict) { return OpenMS::IonImage(self); }, "memo"_a)
    ...
    .def(
      ...
      [](nb::object self_obj) -> nb::object {
        ...
        if (w == 0 || h == 0) { return np.attr("empty")(nb::make_tuple(h, w), np.attr("float64")); }
        ...
        size_t shape[2] = {h, w};
        auto arr = nb::ndarray<nb::numpy, double, nb::ndim<2>, nb::c_contig>(ptr, 2, shape, self_obj);
        return nb::cast(arr);
      },
...
mask. Use setIntensity(x, y, val) when correctness of the mask matters.)doc")
    ...
    .def("__repr__", [](const OpenMS::IonImage& self) {
      std::ostringstream oss;
      oss << "IonImage(width=" << self.getWidth() << ", height=" << self.getHeight() << ")";
      return oss.str();
    });
...
Regions are either Rectangle based and defined by their x/y minima and maxima or bitmasked with a bounding box for irregular shapes.)doc")
        ...
        .def("getMaxY", &OpenMS::MSImagingRegion::getMaxY)
        .def("getBBoxWidth", &OpenMS::MSImagingRegion::getBBoxWidth)
        .def("getBBoxHeight", &OpenMS::MSImagingRegion::getBBoxHeight)
        .def("area", &OpenMS::MSImagingRegion::area)
        .def(
          "get_mask",
          [](const OpenMS::MSImagingRegion& self) {
            const size_t h = self.getBBoxHeight();
            const size_t w = self.getBBoxWidth();
            const size_t n = h * w;
            uint8_t* buf = new uint8_t[n ? n : 1];
            if (self.getShape() == OpenMS::MSImagingRegion::Shape::Rectangle)
            {
              // mask for rectangle is empty -> fill buffer with 1s
              std::fill_n(buf, n, static_cast<uint8_t>(1));
            }
            else
            {
              const auto& v = self.getMask();
          ...
          "Returns a 2D uint8 numpy array (bbox_h, bbox_w); 1 = inside region.")
        .def("__repr__", [](const OpenMS::MSImagingRegion& self) {
          std::ostringstream oss;
          oss << "MSImagingRegion(id=" << self.getId() << ", name=" << self.getName() << ")";
          return oss.str();
        });
  nb::enum_<OpenMS::MSImagingRegion::Shape>(region_cls, "Shape")
    .value("Rectangle", OpenMS::MSImagingRegion::Shape::Rectangle)
    .value("Mask", OpenMS::MSImagingRegion::Shape::Mask);
  // --- MSImagingGeometry ---
  auto geom_cls
...
structured numpy view of all pixels at once.
        ...
        .def(
          "__deepcopy__", [](const OpenMS::MSImagingGeometry& self, nb::dict) { return OpenMS::MSImagingGeometry(self); }, "memo"_a)
        ...
        .def("addPixel", &OpenMS::MSImagingGeometry::addPixel, "x"_a, "y"_a, "spectrum_index"_a, "Adds a pixel at (x, y) bound to spectrum_index.")
        ...
        .def(
          ...
          [](nb::object self_obj) -> nb::object {
            ...
            dtype_dict["offsets"] = nb::make_tuple(offsetof(OpenMS::MSImagingGeometry::Pixel, x), offsetof(OpenMS::MSImagingGeometry::Pixel, y),
            ...
            uint8_t* data_ptr = reinterpret_cast<uint8_t*>(const_cast<OpenMS::MSImagingGeometry::Pixel*>(pixels.data()));
            size_t byte_shape[1] = {n * sizeof(OpenMS::MSImagingGeometry::Pixel)};
            auto raw = nb::ndarray<nb::numpy, uint8_t, nb::c_contig>(data_ptr, 1, byte_shape, self_obj);
            return np.attr("frombuffer")(raw, py_dtype);
          },
          nb::rv_policy::reference_internal, "Zero-copy structured numpy array with fields 'x' (uint32), 'y' (uint32), 'spectrum_index' (uint64).")

        .def("__len__", &OpenMS::MSImagingGeometry::getNumberOfPixels)
        .def("__repr__",
             [](const OpenMS::MSImagingGeometry& self) {
               std::ostringstream oss;
               oss << "MSImagingGeometry(width=" << self.getWidth() << ", height=" << self.getHeight() << ", num_pixels=" << self.getNumberOfPixels()
                   << ")";
               return oss.str();
             })
        ...
        .def("getRegionSpectrumIndices", &OpenMS::MSImagingGeometry::getRegionSpectrumIndices, "id"_a)
        .def("clearRegions", &OpenMS::MSImagingGeometry::clearRegions)
        .def_ro_static("NO_REGION", &OpenMS::MSImagingGeometry::NO_REGION);
