...
array's name. Raises if the spectrum has no ion mobility array; use ``containsIMData()`` first.
        ...
        .def("select", [](OpenMS::MSSpectrum& self, const std::vector<size_t>& indices) -> OpenMS::MSSpectrum& {
            // Validate here so Python gets IndexError/ValueError instead of a RuntimeError translated
            // from Exception::Precondition. The range check is repeated in select() itself (memory
            // safety for every C++ caller). Duplicates are checked only here: C++ select() treats them
            // as undefined behaviour (entries are moved) and every in-tree C++ caller builds a unique
            // index list, so this keeps the cost off the C++ filter/sort paths.
            const size_t n = self.size();
            ...
            for (size_t i : indices)
            {
              ...
              if (seen[i])
              {
                throw nb::value_error(("duplicate index " + std::to_string(i) + ": indices must be unique").c_str());
              }
              seen[i] = true;
            }
