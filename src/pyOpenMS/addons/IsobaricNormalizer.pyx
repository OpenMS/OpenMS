    ...
    cdef initialize_from_ptr(self, _IsobaricQuantitationMethod * this_quant_method):
        self.inst = shared_ptr[_IsobaricNormalizer](new _IsobaricNormalizer(( this_quant_method )))

    def __init__from_self(self, IsobaricNormalizer in_0 ):
    ...
    def __init__(self, input_arg):
        if isinstance(input_arg, IsobaricNormalizer):
