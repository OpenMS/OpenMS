    ...
    def transform(self, *args):
        ...
        # void transform(const String&, IMSDataConsumer[Peak1D, ChromatogramPeak] *, MSRun& e) nogil except + # wrap-ignore
        # void transform(const String&, IMSDataConsumer[Peak1D, ChromatogramPeak] *, MSRun& e, bool skip_full_count, bool skip_first_pass) nogil except + # wrap-ignore

    def _transform_4(self, path, transformer, MSRun exp, bool skip_full_count, bool skip_first_pass):
        assert isinstance(exp, MSRun), 'arg exp wrong type'
    ...
    def _transform_2(self, path, transformer, MSRun exp):
        assert isinstance(exp, MSRun), 'arg exp wrong type'
        assert (isinstance(path, str) or isinstance(path, unicode) or isinstance(path, bytes) or isinstance(path, String)), 'arg path wrong type'
