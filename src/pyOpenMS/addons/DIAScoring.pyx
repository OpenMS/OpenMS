    ...
    def dia_by_ion_score(self, list spectrum , AASequence sequence , charge , float bseries_score , float yseries_score, float drift_start, float drift_end):
        ...
        assert isinstance(drift_start, float), 'arg drift_start wrong type'
        assert isinstance(drift_end, float), 'arg drift_end wrong type'
        ...
        cdef double input_drift_start = (<double>drift_start)
        cdef double input_drift_end = (<double>drift_end)
        self.inst.get().dia_by_ion_score(v1, (deref(sequence.inst.get())), (<int>charge), input_bseries_score, input_yseries_score, input_drift_start, input_drift_end)
