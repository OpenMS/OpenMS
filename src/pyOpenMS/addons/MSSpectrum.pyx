...
import numpy as np
    ...
    def set_peaks(self, peaks):
        ...
        assert len(mzs) == len(intensities), "Input vectors for set_peaks need to have the same length (mz and intensity vecgtor)"

        cdef _MSSpectrum[_Peak1D] * spec_ = self.inst.get()

        spec_.clear(0) # empty vector , keep meta data
        chrom_.reserve(<int>len(mzs)) # allocate space for incoming data
