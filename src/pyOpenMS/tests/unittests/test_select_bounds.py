...
unchanged on rejection.  The nanobind binding raises the idiomatic Python exceptions
before the call reaches C++: ``IndexError`` for an out-of-range index (matching
``__getitem__``) and ``ValueError`` for a duplicate index (undefined behaviour in C++,
where entries are moved).
"""
...
def test_out_of_range_index_raises_and_leaves_spectrum_untouched():
    s = _spectrum(3)
    before = _snapshot(s)
    with pytest.raises(IndexError):  # matches __getitem__, not a RuntimeError from C++
