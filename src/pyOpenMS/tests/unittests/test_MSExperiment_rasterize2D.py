"""
Tests for MSExperiment.rasterize2D() method in pyOpenMS.
...
def test_rasterize2D_basic_sum():
    ...
    exp.rasterize2D(output, 100.0, 200.0, 500.0, 600.0, 1, "sum")
...
def test_rasterize2D_basic_max():
    ...
    exp.rasterize2D(output, 100.0, 200.0, 500.0, 600.0, 1, "max")
    
    # Output should contain non-zero values
    assert np.any(output > 0), "Output should contain non-zero intensity values"


def test_rasterize2D_empty_experiment():
    ...
    exp.rasterize2D(output, 0.0, 100.0, 0.0, 1000.0, 1, "sum")
    
    # Output should be all zeros
    assert np.all(output == 0.0), "Empty experiment should produce all-zero output"


def test_rasterize2D_single_spectrum():
    ...
    exp.rasterize2D(output, 0.0, 100.0, 400.0, 600.0, 1, "sum")
...
def test_rasterize2D_ms_level_filtering():
    ...
    # Rasterize MS1 only
    exp.rasterize2D(output_ms1, 100.0, 150.0, 400.0, 600.0, 1, "sum")
    
    # Rasterize MS2 only
    exp.rasterize2D(output_ms2, 100.0, 150.0, 400.0, 600.0, 2, "sum")
...
def test_rasterize2D_out_of_range():
    ...
    exp.rasterize2D(output, 1000.0, 2000.0, 500.0, 600.0, 1, "sum")
    
    # Output should be all zeros
    assert np.all(output == 0.0), "Out-of-range request should produce all-zero output"


def test_rasterize2D_sum_vs_max():
    ...
    exp.rasterize2D(output_sum, 100.0, 110.0, 490.0, 510.0, 1, "sum")
    exp.rasterize2D(output_max, 100.0, 110.0, 490.0, 510.0, 1, "max")
...
def test_rasterize2D_exception_non_contiguous():
    ...
    with pytest.raises(ValueError, match="C-contiguous"):
        exp.rasterize2D(non_contiguous, 100.0, 200.0, 500.0, 600.0, 1, "sum")


def test_rasterize2D_exception_invalid_aggregation():
    ...
    with pytest.raises(ValueError, match="Invalid aggregation mode"):
        exp.rasterize2D(output, 100.0, 200.0, 500.0, 600.0, 1, "invalid")


def test_rasterize2D_exception_inverted_rt_range():
...
def test_rasterize2D_exception_inverted_mz_range():
...
def test_rasterize2D_shape_validation():
...
def test_rasterize2D_in_place_modification():
...
def test_rasterize2D_zero_initialization():
