...
if __name__ == '__main__':
  ...
  # Import config from env.py (generated by CMake)
  # Provides fallback defaults for standalone/py-build-cmake builds
  try:
      from env import (QT_QMAKE_VERSION_INFO, OPEN_MS_BUILD_TYPE, PYOPENMS_SRC_DIR,
                       OPEN_MS_VERSION, PY_NUM_THREADS, PY_NUM_MODULES)
  except ImportError:
      # Fallback defaults when env.py is not available or incomplete
      print("Warning: env.py not found, using defaults", file=sys.stderr)
      QT_QMAKE_VERSION_INFO = "Unknown"
      OPEN_MS_BUILD_TYPE = os.environ.get("CMAKE_BUILD_TYPE", "Release")
      # Assume script is run from build directory, source is in parent
      PYOPENMS_SRC_DIR = os.environ.get("PYOPENMS_SRC_DIR",
                                         os.path.dirname(os.path.abspath(__file__)))
      OPEN_MS_VERSION = os.environ.get("OPENMS_VERSION", "0.0.0")
      PY_NUM_THREADS = os.environ.get("PY_NUM_THREADS", "1")
      PY_NUM_MODULES = os.environ.get("PY_NUM_MODULES", "8")

  IS_DEBUG = OPEN_MS_BUILD_TYPE.upper() == "DEBUG"

  print("Build type is:", OPEN_MS_BUILD_TYPE)
  print("Source directory:", PYOPENMS_SRC_DIR)
