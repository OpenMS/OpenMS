...
cli = importlib.import_module("pyopenms.pytopp.tools.pytopp_peak_picker_hires_cli")
...
def _run_cli_with_args(monkeypatch, args):
    ...
    """
    monkeypatch.setattr(sys, "argv", ["pytopp_peak_picker_hires_cli.py", *args], raising=False)
    try:
        rc = cli.main()
...
def test_cli_pyprophet_end_to_end(monkeypatch, tmp_path):
    ...
    out_mzml = tmp_path / "PeakPickerHiRes_output.mzML"
