...
CREATE TABLE META_InstrumentSettings (
  ...
  ScanMode enum('UNKNOWN','SELECTEDIONDETECTION','MASSSCAN') default 'UNKNOWN',
  PRIMARY KEY  (id),
  KEY fid_MetaInfo (fid_MetaInfo),
  KEY fid_Spectrum (fid_Spectrum)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
CREATE TABLE META_MassAnalyzer (
  ...
  TandemScanningMethod enum('UNKNOWN','PRODUCTIONSCAN','PRECURSORIONSCAN','CONSTANTNEUTRALLOSS','SINGLEREACTIONMONITORING','MULTIPLEREACTIONMONITORING','SINGLEIONMONITORING','MULTIPLEIONMONITORING') NOT NULL default 'UNKNOWN',
