...
namespace OpenMS
{
	...
	void PILISSequenceDB::addPeptidesFromFile(const String& filename)
	{
		...
		while (seq_db_in.getline(line, 1000))
  	{
    	...
    	s.split(' ', split);
    		...
    		peptides_[(UInt)(new_peptide.weight * factor_)].push_back(new_peptide);
			}
	...
	void PILISSequenceDB::digestProteinsTryptic(UInt missed_cleavages)
	{
	...
	void PILISSequenceDB::addPeptide_(const String& peptide)
	{
		if (!has(peptide))
		{
			...
			p.weight = weight;
			p.charge = 2;
			peptides_[(UInt)(weight * factor_)].push_back(p);
	...
	bool PILISSequenceDB::has(const String& peptide) const
	{
		...
		try 
		{
			AASequence peptide_sequence(peptide);
			double weight = peptide_sequence.getAverageWeight(Residue::Full, 2)/2.0;
			if (!peptides_.has((UInt)(weight * factor_)))
			{
				return false;
			}

			for (vector<PepStruct>::const_iterator it = peptides_[(UInt)(weight * factor_)].begin(); it != peptides_[(UInt)(weight * factor_)].end(); ++it)
			{
	...
	void PILISSequenceDB::getPeptides(std::vector<PepStruct>& peptides, double range_start, double range_stop)
	{
		...
		for (UInt i = (UInt)(range_start * factor_); i <= (UInt)(range_stop * factor_); ++i)
		{
	...
	void PILISSequenceDB::digestTryptic_(const String& seq, vector<String>& peptides, UInt missedcleavages)
	{
    ...
    for (UInt i=0; i != seq.size(); ++i)
    {
     	...
     	{
       	UInt j = i + 1;
       	for (UInt k = 0; k <= missedcleavages; ++k)
       	{
