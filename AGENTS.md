...
- Prefer `OpenMS::StringUtils` free functions (e.g. `toStr()`, `toDouble()`) for numeric formatting and parsing (precision and speed).
...
- Type casters in `bindings/type_casters/` handle C++ ↔ Python type conversion (std::string/std::string_view ↔ str, DPosition, DataValue, etc.).
